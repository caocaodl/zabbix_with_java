...
public class IaaSClient {
	...
	public Glance getImageClient() {
		...
		Glance glanceClient = new Glance(endpoint+"/v1");
	...
	public Cinder getVolumeClient() {
		String endpoint = KeystoneUtils.findEndpointURL(this.access.getServiceCatalog(), "volume", this.region, Facing.ADMIN);
		Cinder volumeClient = new Cinder(endpoint);
		volumeClient.token(access.getToken().getId());
		return volumeClient;
	}
