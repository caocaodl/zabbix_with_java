...
//			}
	...
	public float getLastValue(int _num) {
			...
			for (int i = sizeX - 1; i >= 0; i--) {
				if (!empty(_data.getNested("count", i))) {
					switch (Nest.value(items, _num, "calc_fnc").asInteger()) {
						case CALC_FNC_MIN:
							return Nest.value(_data, "min", i).asFloat();
						case CALC_FNC_MAX:
							return Nest.value(_data, "max", i).asFloat();
						case CALC_FNC_ALL:
						case CALC_FNC_AVG:
						default:
							return Nest.value(_data, "avg", i).asFloat();
					}
				}
			}
		}
