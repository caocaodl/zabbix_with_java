	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		/*if (!empty(Nest.value(data,"hostid").$())) {
			if (!empty(Nest.value(data,"parent_discoveryid").$())) {
				triggersWidget.addItem(get_header_host_table(executor, "triggers", Nest.value(data,"hostid").asLong(true), Nest.value(data,"parent_discoveryid").asLong(true)));
			} else {
		...
		CButton addExpressionButton = new CButton(
			...
			"return PopUp(\"popup_trexpr.action?dstfrm="+triggersForm.getName()+
				...
				"&srcfld1=expression&expression=\" + encodeURIComponent(jQuery(\"[name='"+Nest.value(data,"expression_field_name").$()+"']\").val()), 650, 330);",
			"formlist"
		);
		...
		if (Nest.value(data,"input_method").asInteger() == IM_TREE) {
			...
			expressionTable.setHeader(array(
				...
				(Nest.value(data,"limited").$() == "yes") ? null : _("Operations")
			));
			...
			if (!empty(Nest.value(data,"eHTMLTree").$())) {
				for(Entry<Object,Map> t : ((CArray<Map>)Nest.value(data,"eHTMLTree").asCArray()).entrySet()) {
					...
					if (Nest.value(data,"limited").$() != "yes") {
						deleteUrl  = new CSpan(null, "link_menu remove");
						...
						deleteUrl.setAttribute("onclick","showModalWindow('"+_("Caption")+"',\""+_("Delete expression?")+"\",[{"
								+ "text:'"+_("Sure")+"',click:function(){  delete_expression(\""+Nest.value(tree,"id").$() +"\");"+
									" document.forms[\""+triggersForm.getName()+"\"].submit();}"
					...
					if (empty(Nest.value(data,"parent_discoveryid").$())) {
						...
						if (!isset(Nest.value(tree,"expression","levelErrors").$())) {
							errorImg = new CImg("styles/themes/originalblue/images/iradar/right_.png", "expression_no_errors");
		...
		triggersFormList.addRow(_("URL"), new CTextBox("url", Nest.value(data,"url").asString(), RDA_TEXTBOX_STANDARD_SIZE), true);
		triggersFormList.addRow(_("Priority"), new CSeverity(idBean, executor, map("name", "priority", "value", Nest.value(data,"priority").$())));
		 ...
		 */
		if (empty(Nest.value(data,"parent_discoveryid").$())) {
			CFormList dependenciesFormList = new CFormList("dependenciesFormList", IMonConsts.STYLE_CLASS_MULTLINE);
			...
			dependenciesTable.setHeader(array(_("Name"), _("Operations")));
			...
			for(Map dependency : (CArray<Map>)Nest.value(data,"db_dependencies").asCArray()) {
				...
				row = new CRow(array(
					...
					new CButton("remove", _("Remove"), "javascript: removeDependency(\""+Nest.value(dependency,"triggerid").$()+"\");", "link_menu icon remove")
				));
				row.setAttribute("id", "dependency_"+Nest.value(dependency,"triggerid").$());
				dependenciesTable.addRow(row);
			}
			dependenciesTable.addRow(new CRow(
					new CCol(
			...
			(
				_("Dependencies Trigger"),
				new CDiv(array(dependenciesTable),"objectgroup inlineblock border_dotted ui-corner-all")
			);
