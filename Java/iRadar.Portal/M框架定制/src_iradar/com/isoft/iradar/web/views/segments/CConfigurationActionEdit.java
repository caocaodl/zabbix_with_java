...
import com.isoft.iradar.inc.UsersUtil;
...
//		actionWidget.addPageHeader(_("CONFIGURATION OF ACTIONS"));
		...
		for (Map condition : conditions) {
			...
			conditionTable.addRow(
					array(
						...
						array(
							new CButton("remove", "", "javascript: removeCondition("+i+");", "link_menu remove"),
		...
		conditionTable.addRow(array(new CSubmit("add_condition", _("Add"), null, "link_menu add"), SPACE, SPACE));
		...
		for (Entry<Object, Map> e : operations.entrySet()) {
			...
			if (eventsource == EVENT_SOURCE_TRIGGERS || eventsource == EVENT_SOURCE_INTERNAL) {
				...
				operationRow = array(
						...
						esc_period_txt,
						array(
							new CSubmit("edit_operationid["+operationid+"]", "", null, "link_menu edit"),
							SPACE, SPACE, SPACE,
							array(
								new CButton("remove", "", "javascript: removeOperation("+operationid+");", "link_menu remove"),
		...
		if (empty(data.get("new_operation"))) {
			footer.add(new CSubmit("new_operation", _("New"), null, "link_menu add"));
		}
			...
			case OPERATION_TYPE_MESSAGE:
				if (!isset(new_operation,"opmessage")) {
					...
					if (eventsource == EVENT_SOURCE_TRIGGERS) {
						...
						opmessage.put("message",_("ACTION_DEFAULT_MSG_TRIGGER"));
				...
				usrgrpList.setHeader(array(_("User group"), _("Action operations")));
				usrgrpList.attr("style", "min-width: 310px;");
				usrgrpList.setAttribute("id", "opmsgUsrgrpList");

				CButton addUsrgrpBtn = new CButton("add", _("Add"), "return PopUp(\"popup.action?dstfrm=action.edit&srctbl=usrgrp&srcfld1=usrgrpid&srcfld2=name&multiselect=0\", 450, 450)", "link_menu add");
				...
				userList.setHeader(array(_("User"), _("Action operations")));
				userList.attr("style", "min-width: 310px;");
				userList.setAttribute("id", "opmsgUserList");

				CButton addUserBtn = new CButton("add", _("Add"), "return PopUp(\"popup.action?dstfrm=action.edit&srctbl=users&srcfld1=userid&srcfld2=fullname&multiselect=1\", 450, 450)", "link_menu add");
...
//				CArray<Map> usrgrps = API.UserGroup(idBean, executor).get(ugget);
				...
				CArray<Map> users = CArray.array();
				CArray<Map> usersTmp = API.User(idBean, executor).get(uget);
				for (Map user : usersTmp) {
					user.put("fullname", UsersUtil.getUserFullname(user));
					users.push(user);
				}
	...
	//			newOperationsTable.addRow(array(_("Send to User groups"), new CDiv(usrgrpList, "objectgroup inlineblock border_dotted ui-corner-all")));
				...
				String query = "SELECT mt.mediatypeid,mt.description FROM media_type mt where mt.tenantid='-' AND mt.status=0";
				CArray<Map> dbMediaTypes = DBselect(executor, query, params);
			...
			case OPERATION_TYPE_COMMAND:
				...
				cmdList.setHeader(array(_("Target"), _("Action operations")));

				CButton addCmdBtn = new CButton("add", _("New"), "javascript: showOpCmdForm(0, \"new\");", "link_menu add");
				...
				CButton userScriptSelect = new CButton("select_opcommand_script", _("Select"), null, "link_menu select-popup");
			...
			case OPERATION_TYPE_GROUP_REMOVE:
				...
				groupList.addRow(new CCol(new CButton("add", _("Add"), "return addDiscoveryHostGroup();", "link_menu add"), null, "2"));
			...
			case OPERATION_TYPE_TEMPLATE_REMOVE:
				...
				templateList.addRow(new CCol(new CButton("add", _("Add"), "return addDiscoveryTemplates();", "link_menu add"), null, "2"));
			...
			if (Nest.value(data,"eventsource").asInteger() == 0) {
				...
				if (!isset(_REQUEST, "new_opcondition")) {
					operationConditionsTable.addRow(new CCol(new CSubmit("new_opcondition", _("New"), null, "link_menu add")));
				}
			...
			if (isset(_REQUEST, "new_opcondition")) {
				...
				CArray newOperationConditionFooter = array(
					new CSubmit("add_opcondition", _("Add"), null, "link_menu add"),
					SPACE+SPACE,
					new CSubmit("cancel_new_opcondition", _("Cancel"), null, "link_menu cancel")
				);
			...
			footer = array(
					new CSubmit("add_operation", (Nest.value(data,"new_operation","action").$() == "update") ? _("Update") : _("Add"), null, "link_menu add"),
					SPACE+SPACE,
