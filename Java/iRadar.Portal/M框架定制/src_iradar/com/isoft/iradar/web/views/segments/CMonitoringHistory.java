...
public class CMonitoringHistory extends CViewSegment {
	...
	//	CArray header = map("left", count(Nest.value(data,"items").$())+SPACE+_("ITEMS"), "right", array());
		...
		//	Nest.value(header,"left").$(array(Nest.value(data,"item","hostname").$(), NAME_DELIMITER, Nest.value(data,"item","name_expanded").$()));
    ...
    */
		...
		CSpan hostname = new CSpan(array(Nest.value(data,"item","hostname").$(), NAME_DELIMITER, Nest.value(data,"item","name_expanded").$()),"name_font");
		actionForm.addItem(hostname);
		//array_unshift(Nest.value(header,"right").asCArray(), actionForm, SPACE);
		historyWidget.addHeader(actionForm);
		// create filter
