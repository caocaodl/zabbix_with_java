...
public class CAdministrationUsersList extends CViewSegment {
	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		// append userSwitchForm
				CForm userSwitchForm = new CForm("get");
				userSwitchForm.cleanItems();
				...
				userSwitchForm.addItem(configurationComboBox);
				
		usersWidget.addHeader(userGroupListForm,userSwitchForm);
				
		// create form
		CForm usersForm = new CForm();
		usersForm.setName("userForm");
		
		CToolBar tb = new CToolBar(usersForm);
		tb.addSubmit("form", _("Create user"), "", "orange create");
		...
		usersWidget.addItem(headerActions);
		...
		for(Map user : (CArray<Map>)Nest.value(data,"users").asCArray()) {
			...
			usersTable.addRow(array(
				...
				Nest.value(data,"displayNodes").asBoolean() ? Nest.value(user,"nodename").$() : null,
		...
		usersForm.addItem(array(usersTable, Nest.value(data,"paging").$()));
