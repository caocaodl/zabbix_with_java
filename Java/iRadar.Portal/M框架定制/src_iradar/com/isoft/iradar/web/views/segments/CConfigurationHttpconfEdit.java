...
public class CConfigurationHttpconfEdit extends CViewSegment {
	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		/*if (!empty(Nest.value(data,"hostid").$())) {
			httpWidget.addItem(get_header_host_table(executor, "web", Nest.value(data,"hostid").asLong(true)));
		}*/
		...
		/* Step tab */
		CFormList httpStepFormList = new CFormList("httpFormList",IMonConsts.STYLE_CLASS_MULTLINE);
		...
		for (Entry<Object, Map> e : ((CArray<Map>)Nest.value(data,"steps").asCArray()).entrySet()) {
			...
			if (Nest.value(data,"templated").asBoolean()) {
				...
				removeButton = new CButton("remove_"+stepid, _("Remove"), "javascript: removeStep(this);", "link_menu icon remove");
		...
		if (!Nest.value(data,"templated").asBoolean()) {
			stepsTable.addRow(new CRow(new CCol(new CButton("add_step", _("Add"), null, "link_menu add"),null,8),"cmd_row"));
		}
