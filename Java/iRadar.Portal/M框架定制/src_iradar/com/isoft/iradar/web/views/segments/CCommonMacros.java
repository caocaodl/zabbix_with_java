...
public class CCommonMacros extends CViewSegment {
	...
	public CFormList doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		for (Entry<Object, Map> e : macros.entrySet()) {
			...
			if (!readonly) {
				deleteButtonCell = array(new CButton("macros_"+i+"_remove", "", null, "link_menu macroRemove remove"));
		...
		// buttons
		if (!readonly) {
			CButton addButton = new CButton("macro_add", _("Add"), null, "link_menu add");
			...
			buttonRow.setAttribute("class", "cmd_row");
			buttonRow.addItem(buttonColumn);
			macrosTable.addRow(buttonRow);
		}
		macrosTable.addRow(array(_("Note:the path of Windows:")));
		// form list
		CFormList macrosFormList = new CFormList("macrosFormList");
