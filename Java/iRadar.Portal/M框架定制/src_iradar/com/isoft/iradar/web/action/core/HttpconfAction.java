...
public class HttpconfAction extends RadarBaseAction {
	...
	protected void doInitPage() {
		...
		page("css", new String[] {"lessor/strategy/httpconf.css"});
	}
...
//			"hostid", 					array(T_RDA_INT, O_OPT, P_SYS, DB_ID+NOT_ZERO,          "isset({form})||isset({save})"),
			"hostid", 					array(T_RDA_INT, O_OPT, P_SYS, DB_ID+NOT_ZERO,          "isset({save})"),
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"delete") && isset(_REQUEST,"httptestid")) {
			...
			if (result) {
				add_audit(getIdentityBean(), executor,AUDIT_ACTION_DELETE, AUDIT_RESOURCE_SCENARIO, "删除"+_("WEB PRIFEREWS")+" ["+Nest.value(httptest_data,"name").$()+"]于"+_("By host")+" ["+Nest.value(host,"host").$()+"]");
			}
			...
			try {
				...
				add_audit(getIdentityBean(), executor,action, AUDIT_RESOURCE_SCENARIO,_("Create scenario")+" ["+Nest.value(_REQUEST,"name").asString()+"] 于设备 ["+Nest.value(host,"host").$()+"]");
			...
			for(final Object id : groupHttpTestId) {
				...
				if (result) {
					...
					add_audit(getIdentityBean(), executor, auditAction, AUDIT_RESOURCE_SCENARIO,
						"更新"+_("WEB PRIFEREWS")+" ["+Nest.value(httpTestData,"name").$()+"] 于设备 ["+Nest.value(host,"host").$()+"] "+statusName
					);
				}
				updated++;
			}
			...
			for(final Object id : group_httptestid) {
				...
				if (Call(new Wrapper<Boolean>() {
					...
					add_audit(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCENARIO,"清除"+ _("WEB PRIFEREWS")+" ["+Nest.value(httptest_data,"name").$()+"]"+"历史数据于"+_("By host")+" ["+Nest.value(host,"host").$()+"]");
				}
			}
		...
		if (isset(_REQUEST, "form")) {
			...
			CPageFilter pageFilter = new CPageFilter(getIdentityBean(), executor, map(
				...
				"hostid", get_request("statusflag")==null? get_request("hostid") : 0,
			...
			if (pageFilter.$("hostsSelected").asBoolean()) {
				...
				if (pageFilter.$("hostid").asInteger() > 0 && Nest.value(_REQUEST,"statusflag").asLong()!=1)  {
