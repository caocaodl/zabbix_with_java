...
public class ScriptsAction extends RadarBaseAction {
	...
	protected void doInitPage() {
		...
		page("css", new String[] { "lessor/toolmanage/script.css" });
	}
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"groupid",						array(T_RDA_INT, O_OPT, null,			DB_ID,		"isset({save})&&{hgstype}!=0",_("Group")),
		...
		if (isset(_REQUEST,"clone") && isset(_REQUEST,"scriptid")) {
			...
			if (enableConfirmation && rda_empty(confirmation)) {
				...
				if (isset(_REQUEST,"scriptid")) {
					...
					scriptId=Nest.value(_REQUEST, "scriptid").asLong();
				} else {
					result = Call(new Wrapper<CArray<Long[]>>() {
						@Override
						protected CArray<Long[]> doCall() throws Throwable {
					...
					auditAction = AUDIT_ACTION_ADD;
					 scriptId = isset(result,"scriptids") ? reset(result.get("scriptids")) : null;
				}

				

				if (!empty(result)) {
					add_audit(getIdentityBean(), executor, auditAction, AUDIT_RESOURCE_SCRIPT, _("Name")+" ["+Nest.value(_REQUEST,"name").asString()+"] "+_("id")+" ["+scriptId+"]");
