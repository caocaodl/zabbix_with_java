...
public class ActionconfAction extends RadarBaseAction {
	...
	protected void doInitPage() {
		page("title", _("Configuration of Action rules"));
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"edit_operationid",				array(null,		O_OPT,	P_ACT,	null, null),
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"clone") && isset(_REQUEST,"actionid")) {
			...
			CArray<Map> operations = get_request("operations", array());
			int triggerValueCount = 0;
			boolean add_opcondition = true;
			boolean result;
			if(Defines.ACTION_EVAL_TYPE_AND == get_request("evaltype", 0)){
				for(Map condition:conditions){
					if(Nest.value(condition, "conditiontype").asInteger() == Defines.CONDITION_TYPE_TRIGGER_VALUE){
						++triggerValueCount;
					}
				}
			}
			for(Map operation : operations){
				if(!add_opcondition)
					continue;
				int opconditionValueCount = 0;
				if(isset(operation,"opconditions")){
					CArray<Map> opconditions = Nest.value(operation, "opconditions").asCArray();
					int evaltype = Nest.value(operation, "evaltype").asInteger();
					for(Map opcondition:opconditions){
						if(Nest.value(opcondition, "conditiontype").asInteger() == 14){//14:opconditiontype
							++opconditionValueCount;
						}
					}
					if(opconditionValueCount>1&&1==evaltype){
						add_opcondition = false;
					}
				}
			}
			if(triggerValueCount>1||!add_opcondition)
				result = false;
			else
				result = true;
			if(result){
				...
				info(_(triggerValueCount>1?"Conditions contains opposed condition":"Opconditions contains opposed condition"));
				show_error_message(_("Cannot save action"));
			}
			...
			if (isset(Nest.value(data,"action","actionid").$()) && !hasRequest("form_refresh")) {
				...
				if (!empty(Nest.value(data,"actionid").$()) && hasRequest("form_refresh")) {
					...
					if (Nest.value(data,"eventsource").asInteger() == EVENT_SOURCE_TRIGGERS) {
						...
						Nest.value(data,"action","def_longdata").$(get_request("def_longdata", _("ACTION_DEFAULT_MSG_TRIGGER")));
						Nest.value(data,"action","r_shortdata").$(get_request("r_shortdata", ACTION_DEFAULT_SUBJ_TRIGGER));
						Nest.value(data,"action","r_longdata").$(get_request("r_longdata", _("ACTION_DEFAULT_MSG_TRIGGER")));
