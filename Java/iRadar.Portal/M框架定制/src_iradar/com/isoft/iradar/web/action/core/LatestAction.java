...
public class LatestAction extends RadarBaseAction {
	
	@Override
	protected void doInitPage() {
		page("title", _("Latest data"));
		page("file", getAction());
		page("hist_arg", new String[] { "groupid", "hostid", "show", "select", "open", "applicationid" });
		page("type", detect_page_type(PAGE_TYPE_HTML));

		define("RDA_PAGE_MAIN_HAT", "hat_latest");
	...
	protected boolean doAjax(SQLExecutor executor) {
		if (hasRequest("favobj")) {
			if ("filter".equals(Nest.value(_REQUEST,"favobj").$())) {
				CProfile.update(getIdentityBean(), executor, "web.latest.filter.state", Nest.value(_REQUEST,"favstate").asInteger(), PROFILE_TYPE_INT);
				...
				if (!isArray(Nest.value(_REQUEST,"toggle_ids").$())) {
					if(!empty(Nest.value(_REQUEST,"toggle_ids").asString())){
						if ('_' == Nest.value(_REQUEST,"toggle_ids").asString().charAt(1)) {
							...
							CProfile.update(getIdentityBean(), executor, "web.latest.toggle_other", Nest.value(_REQUEST,"toggle_open_state").asInteger(), PROFILE_TYPE_INT, hostId);
						} else {
							Long applicationId = Nest.value(_REQUEST,"toggle_ids").asLong();
							CProfile.update(getIdentityBean(), executor, "web.latest.toggle", Nest.value(_REQUEST,"toggle_open_state").asInteger(), PROFILE_TYPE_INT, applicationId);
						}
					}
				} else {
	...
	public void doAction(SQLExecutor executor) {
		...
		if (hasRequest("filter_set") || hasRequest("filter_rst")) {
			CProfile.update(getIdentityBean(), executor, "web.latest.filter.select", filterSelect, PROFILE_TYPE_STR);
			CProfile.update(getIdentityBean(), executor, "web.latest.filter.show_without_data", filterShowWithoutData, PROFILE_TYPE_INT);
			CProfile.update(getIdentityBean(), executor, "web.latest.filter.show_details", filterShowDetails, PROFILE_TYPE_INT);
		} else {
			filterSelect = (String)CProfile.get(getIdentityBean(), executor, "web.latest.filter.select", "");
			filterShowWithoutData = (Integer)CProfile.get(getIdentityBean(), executor, "web.latest.filter.show_without_data", 0);
			filterShowDetails = (Integer)CProfile.get(getIdentityBean(), executor, "web.latest.filter.show_details", 0);
		}
		
		Long curGroupId = (Long)getGroupId(executor);
		CPageFilter pageFilter = new CPageFilter(getIdentityBean(), executor, map(
			...
			"groupid", curGroupId,
			"groups", CArray.map("groupIds", new Long[] {curGroupId})
		));		
		...
		CArray<CArray<Map>> hostScripts= array();
		
		// get hosts
		Long[] availableHostIds = null;
		if (!empty(Nest.value(_REQUEST,"hostid").$())) {
			availableHostIds = Nest.array(_REQUEST,"hostid").asLong();
		} else if(pageFilter.$("hostsSelected").asBoolean()){
			availableHostIds = pageFilter.$("hosts").asCArray().keysAsLong();
		} else {
			availableHostIds = new Long[0];
		}
		
		CHostGet options = new CHostGet();
		options.setOutput(new String[] { "name", "hostid", "status" });
		options.setHostIds(availableHostIds);
		...
		if (!empty(items)) {
			...
			if (!empty(items)) {
				...
				if (Nest.value(_REQUEST,"hostid").asInteger() == 0) {
					// get host scripts
					hostScripts = API.Script(getIdentityBean(), executor).getScriptsByHosts(hostIds.valuesAsLong());
		...
		doPageFilterCB(pageFilter, form);

		latestWidget.addHeader(SPACE, form);
		...
		latestWidget.addFlicker(filterForm, (Integer)CProfile.get(getIdentityBean(), executor, "web.latest.filter.state", 1));
		...
		if (!empty(filterShowDetails)) {
			...
			CCol infoHeader = new CCol(new CSpan(_("Info")), "latest-info");
		...
		for (Entry<Object, Map> e : items.entrySet()) {
			...
			if (!empty(filterShowDetails)) {
				CTag itemKey = (Nest.value(item,"type").asInteger() == ITEM_TYPE_HTTPTEST || Nest.value(item,"flags").asInteger() == RDA_FLAG_DISCOVERY_CREATED)
					...
					: new CLink(Nest.value(item,"key_expanded").$(), "monitor_items.action?form=update&itemid="+Nest.value(item,"itemid").$(), "enabled");
				...
				row  = new CRow(array(
					SPACE,
					hostColumn,
					new CCol(new CDiv(array(Nest.value(item,"name_expanded").$(), BR(), itemKey), stateCss+" item")),
					new CCol(new CSpan(
			...
			for(Map itemApplication:itemApplications) {
				...
				Nest.value(applications, applicationId,"item_cnt").plus(1);
		...
		for (Entry<Object, Map> e : applications.entrySet()) {
			...
			Object openState = CProfile.get(getIdentityBean(), executor, "web.latest.toggle", null, Nest.value(dbApp,"applicationid").asLong());
			...
			if (Nest.value(_REQUEST,"hostid").asLong() == 0L) {
				hostName = new CSpan(Nest.value(host,"name").$(),
					"link_menu menu-host"+((Nest.value(host,"status").asInteger() == HOST_STATUS_NOT_MONITORED) ? " not-monitored" : "")
				);
				hostName.setMenuPopup(getMenuPopupHost(host, hostScripts.get(host.get("hostid"))));
			}
			...
			table.addRow(array(
				...
				hostName,
				new CCol(array(
						bold(Nest.value(dbApp,"name").asString()),
		...
		for (Map item : items) {
			...
			Nest.value(hosts, item.get("hostid"),"item_cnt").plus(1);
			Nest.value(tab_rows, item.get("hostid")).push(row);
		}
		...
		for (Entry<Object, Map> e : hosts.entrySet()) {
			...
			Object openState = CProfile.get(getIdentityBean(), executor, "web.latest.toggle_other", null, Nest.value(host,"hostid").asLong());
	...
	protected Object getGroupId(SQLExecutor executor){
		return FuncsUtil.get_request("groupid", null);
	}
