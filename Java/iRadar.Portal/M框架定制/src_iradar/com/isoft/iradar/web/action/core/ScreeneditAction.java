...
public class ScreeneditAction extends RadarBaseAction {
	...
	protected void doInitPage() {
		...
		page("css", new String[] {"lessor/strategy/screen_common.css", "lessor/strategy/screenedit.css"});
	}
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"width",			array(T_RDA_INT, O_OPT, null,	BETWEEN(100, 1000), null, _("Width")),
			"height",			array(T_RDA_INT, O_OPT, null,	BETWEEN(100, 1000), null, _("Height")),
	...
	protected boolean doAjax(SQLExecutor executor) {
		...
		if (!empty(Nest.value(_REQUEST,"ajaxAction").$()) && "sw_pos".equals(Nest.value(_REQUEST,"ajaxAction").asString())) {
			...
			if (count(sw_pos) > 3) {
				...
				add_audit_details(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(), "");
			}
			echo("{\"result\": true}");
		}
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"save")) {
			...
			if (!empty(Nest.value(_REQUEST,"screenitemid").$())) {
				...
				show_messages(result, _("Screen element updated"), _("Cannot update screen element"));
			} else {
				...
				show_messages(result, _("Screen element added"), _("Cannot add screen element"));
			}
			DBend(executor, result);

			if (result) {
				add_audit_details(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(),
					(isset(_REQUEST,"screenitemid") ? _("Cell changed screen itemid")+"\""+Nest.value(_REQUEST,"screenitemid").asString()+"\"" : "")+
					(isset(_REQUEST,"x") && isset(Nest.value(_REQUEST,"y").$()) ? _("Cell changed coordinates")+" \""+Nest.value(_REQUEST,"x").asString()+","+Nest.value(_REQUEST,"y").asString()+"\"" : "")+
					(isset(_REQUEST,"resourcetype") ? " "+_("resource type")+" \""+Nest.value(_REQUEST,"resourcetype").asString()+"\"" : "")
				);
				unset(_REQUEST,"form");
			}
			...
			show_messages(result, _("Screen element deleted"), _("Cannot delete screen element"));
			if (result) {
			...
			add_audit_details(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(), _("Row added"));
			...
			add_audit_details(getIdentityBean(), executor, AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(), _("Column added"));
			...
			if (Nest.value(screen,"vsize").asInteger() > 1) {
				...
				add_audit_details(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(), _("Row deleted"));
			} else {
			...
			if (Nest.value(screen,"hsize").asInteger() > 1) {
				...
				add_audit_details(getIdentityBean(), executor,AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_SCREEN, Nest.value(screen,"screenid").asLong(), Nest.value(screen,"name").asString(), _("Column deleted"));
			} else {
				error(_("Screen should contain at least one row and column."));
				show_error_message(_("Impossible to remove last row and column."));
			}
