	...
	public void doAction(SQLExecutor executor) {
		...
		if (isset(_REQUEST,"unlink") || isset(_REQUEST,"unlink_and_clear")) {
			...
			try {
				...
				if (createNew) {
					...
					CArray<Map> infaces=API.HostInterface(getIdentityBean(), executor).get(inface);
					host.put("interfaces", infaces);
					//CHostGroupGet hg=new CHostGroupGet();
		...
		if ("massupdate".equals(Nest.value(_REQUEST,"go").asString()) && isset(_REQUEST,"hosts")) {
			...
			for (Map host : hosts) {
				...
				CArray applications = array(new CLink(_("Applications"), "monitor_apps.action?groupid="+groupid+"&hostid="+hostid),
					" ("+Nest.value(host,"applications").$()+")");
				CArray items = array(new CLink(_("Items"), "monitor_items.action?filter_set=1&hostid="+hostid),
				...
				CArray graphs = array(new CLink(_("Graphs"), "monitor_graphs.action?groupid="+groupid+"&hostid="+hostid),
