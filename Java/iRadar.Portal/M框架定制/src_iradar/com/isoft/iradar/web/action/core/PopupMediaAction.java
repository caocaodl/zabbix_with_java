...
import static com.isoft.iradar.inc.ValidateUtil.*;
...
public class PopupMediaAction extends RadarBaseAction {
	...
	String emailReg = "^(\\w)+(\\.\\w+)*@([a-zA-Z0-9]+[-|\\-|\\.]?)*[a-zA-Z0-9]+\\.[a-zA-Z]{2,3}$";
	
	@Override
	protected void doInitPage() {
		Nest.value(page, "title").$(_("Media Setting"));
		Nest.value(page, "file").$(_("popup_media.action"));
		page("js", new String[] {"class.usersedit.js"});
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"sendto",			array(T_RDA_STR, O_OPT,    null,	NOT_EMPTY,		"isset({add})",_("Popup Media Send to")),
			"period",			array(T_RDA_STR, O_OPT,	null,	NOT_EMPTY,		"isset({add})",_("When active")),
	...
	protected void doAction(SQLExecutor executor) {
		...
		if (isset(_REQUEST,"add")) {
			...
			boolean periodResult = validator.validate(getIdentityBean(), Nest.value(_REQUEST,"period").asString());
			boolean emailResult = emailValidate(Nest.value(_REQUEST, "sendto").asString());
			if (periodResult&&emailResult) {
				int severity = 0;
				...
				if(!emailResult){
					show_messages(false, null,_("Incorrect media sendto"));
					doShow(executor);
					return;
				}
				if(!periodResult){
					show_messages(false, null,_("Incorrect media period"));
	...
	protected void doShow(SQLExecutor executor){
		...
		CFormTable frmMedia = new CFormTable(null);
		frmMedia.setTableClass("formtable messagerecive");
		...
		frmMedia.addRow(_("Media types"), cmbType);
		frmMedia.addRow(_("Popup Media Send to"), new CTextBox("sendto", sendto, 48,false,50));
//		frmMedia.addRow(_("Send to"), new CTextBox("sendto", sendto, 48,false,50));
		...
		CSubmit btn = new CSubmit("add", (media > -1) ? _("Save") : _("Add"),null,"input buttonorange");
//		btn.setAttribute("onClick", "return onSubmitFotBtn();");
