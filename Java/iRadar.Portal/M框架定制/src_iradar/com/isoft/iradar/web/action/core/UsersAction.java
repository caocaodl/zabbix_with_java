...
public class UsersAction extends RadarBaseAction {
	...
	protected void doInitPage() {
		...
		page("css", new String[] {"lessor/toolmanage/userparameter.css"});
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST, "save")) {
			CArray<Map> keptMedias = new CArray<Map>();
			CArray<Map> userMedias = Nest.value(_REQUEST, "user_medias").asCArray();
			if (userMedias != null) {
				for (Entry<Object, Map> e : userMedias.entrySet()) {
					Map media = e.getValue();
					media.put("userid", userId);
					keptMedias.push(media);
				}
			}
			DBstart(executor);
			userDao.doDeleteMediaByUserId(userIds);
			for (Entry<Object, Map> e : keptMedias.entrySet()) {
				Map media = e.getValue();
				media.put("tenantid", getIdentityBean().getTenantId());
				userDao.doAddMediaByUserId(media);
			}
			show_messages(true, "设置成功");
			DBend(executor);
			/**
			 * bug12198,还原原始代码,去除非空判断
			 */
//				else{
//				show_messages(false, null,_("the user medias are necessarily"));
//			}
		}
		if (isset(_REQUEST, "new_media")) {
			...
			if (isset(_REQUEST, "enable_media")) {
				...
				data.put("user_medias", Nest.value(_REQUEST, "user_medias").asCArray());
			} else if (isset(_REQUEST, "disable_media")) {
				// id.put("mediaid", Nest.value(_REQUEST,
				...
				if (isset(_REQUEST, "user_medias_to_del")) {
					...
					show_messages(true, "请先选择数据");
				}
            } else if (empty(get_request("user_medias", array()))) {
		...
		cnf_wdgt.addItem(usersView.render(getIdentityBean(), executor));
		cnf_wdgt.show();
