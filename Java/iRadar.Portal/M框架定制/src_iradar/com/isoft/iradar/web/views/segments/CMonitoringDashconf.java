...
public class CMonitoringDashconf extends CViewSegment {
	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		//用户选择要查看面板
		CTweenBox panelsComboBox = new CTweenBox(dashconfForm, "panels", Nest.value(data,"u_panels").$(), 25);
		Set<Entry> set =  Nest.value(data,"t_panels").asCArray().entrySet();
		for(Entry<Object, Object> entry : set){
			panelsComboBox.addItem(entry.getKey().toString(), entry.getValue().toString());
		}	
//		dashconfFormList.addRow("面板", panelsComboBox.get("已选面板", "可选面板"));
		
		
		// create second tab
		CFormList panelFormList = new CFormList("panelFormList");
		panelFormList.addRow("面板", panelsComboBox.get("已选面板", "可选面板"));
		
		// create tab
		CTabView dashconfTab = new CTabView();
		dashconfTab.addTab("dashconfTab", _("Filter"), array(dashconfFormList,panelFormList));
