...
//				// percent left
//				CTextBox percentLeftTextBox = new CTextBox("percent_left", String.valueOf(Nest.value(data,"percent_left").asFloat()), 6, false, 7);
//				CCheckBox percentLeftCheckbox = new CCheckBox("visible[percent_left]", true, "javascript: showHideVisible(\"percent_left\");", 1);
//
//				if (Nest.value(data,"percent_left").asFloat() == 0f) {
...
//
//				// percent right
//				CTextBox percentRightTextBox = new CTextBox("percent_right", String.valueOf(Nest.value(data,"percent_right").asFloat()), 6, false, 7);
//				CCheckBox percentRightCheckbox = new CCheckBox("visible[percent_right]", true, "javascript: showHideVisible(\"percent_right\");", 1);
//
//				if (Nest.value(data,"percent_right").asFloat() == 0f) {
...
//			}
		...
		itemsTable.setHeader(array(
			...
			new CCol(_("Operations"), null, null, 80)
		));
		...
		CButton addButton = new CButton("add_item", _("Add"),
			"return PopUp(\"popup.action?writeonly=1&multiselect=1&dstfrm="+graphForm.getName()+
				...
				"&srctbl=items&srcfld1=itemid&srcfld2=name&numeric=1\" + getOnlyHostParam(), 800, 0);",
			"link_menu add"
		);
		...
		if (!empty(Nest.value(data,"parent_discoveryid").$())) {
			addPrototypeButton = new CButton("add_protoitem", _("Add prototype"),
				"return PopUp(\"popup.action?writeonly=1&multiselect=1&dstfrm="+graphForm.getName()+
					...
					(Nest.value(data,"normal_only").asBoolean() ? "&normal_only=1" : "")+
					"&srctbl=prototypes&srcfld1=itemid&srcfld2=name&numeric=1\", 800, 0);",
				"link_menu add"
			);
		}
		...
		if (Cphp.empty(data.get("form_refresh"))) {
		...
		if (!empty(Nest.value(data,"graphid").$())) {
			...
			graphForm.addItem(makeFormFooter(saveButton, array(cloneButton,  cancelButton)));
		} else {
		...
		includeSubView("js/configuration.graph.edit.js", data);
