...
public class FormsUtil {
	...
	public static Map getUserFormData(IIdentityBean idBean, SQLExecutor executor, String userid, boolean isProfile) {
		...
		if (!empty(Nest.value(data,"user_medias").$())) {
			...
			CArray<Map> dbMediaTypes = DBselect(executor,
				"SELECT mt.mediatypeid,mt.description FROM media_type mt WHERE mt.tenantid='-'"+
	...
	public static CForm getItemFilterForm(IIdentityBean idBean, SQLExecutor executor, CArray<Map> items) {
		return getItemFilterForm(idBean,executor,items,false);
	}
	
	public static CForm getItemFilterForm(IIdentityBean idBean, SQLExecutor executor, CArray<Map> items,boolean isTemplate) {
		CMap<Object, Object> _REQUEST = RadarContext._REQUEST();
		...
		form.addVar("flag_from_template", isTemplate);
		...
		String parameters = "srctbl=host_templates&dstfrm="+form.getName()+"&dstfld1=filter_hostid&srcfld1=hostid&writeonly=1";
		if(isTemplate){
			parameters = "srctbl=templates&dstfrm="+form.getName()+"&dstfld1=filter_hostid&srcfld1=hostid&srcfld2=host&templated_hosts=1&writeonly=1";
		}
...
//			new CCol(bold(_("Triggers")+NAME_DELIMITER), "label"),
			new CCol(bold(_("Triggers relevance")+NAME_DELIMITER), "label"),
			new CCol(new CComboBox("filter_with_triggers", filter_with_triggers, null, (CArray)map(
				-1, _("all"),
//				1 , _("With triggers"),
//				0 , _("Without triggers")
				1 , _("With triggers relevance"),
				0 , _("Without triggers relevance")
			)))
		), "item-list-row");
		// row 4
		table.addRow(array(
...
//					Nest.value(item_params,"with_triggers",0).$(map("name", _("Without triggers"), "count", 0));
					Nest.value(item_params,"with_triggers",0).$(map("name", _("Without triggers relevance"), "count", 0));
				} else if (count(Nest.value(item,"triggers").$()) > 0 && !isset(Nest.value(item_params,"with_triggers",1).$())) {
//					Nest.value(item_params,"with_triggers",1).$(map("name", _("With triggers"), "count", 0));
					Nest.value(item_params,"with_triggers",1).$(map("name", _("With triggers relevance"), "count", 0));//无报警改为有报警
				}
				show_item = true;
				for (Entry<Object, Boolean> e : ((CArray<Boolean>)Nest.value(item,"subfilters").asCArray()).entrySet()) {
				    Object name = e.getKey();
...
//			table_subfilter.addRow(array(_("With triggers"), with_triggers_output));
			table_subfilter.addRow(array(_("With triggers relevance"), with_triggers_output));
		}
	...
	public static Map getCopyElementsFormData(IIdentityBean idBean, SQLExecutor executor, String elementsField, String title) {
		...
		Iterator<Map> groupIt = groups.iterator();
		while(groupIt.hasNext()){
			Map group = groupIt.next();
			if(Feature.hostGroupsNeedHide.containsKey(Nest.value(group,"groupid").$())){
				groupIt.remove();
			}
		}
	...
	public static CTable get_timeperiod_form() {
		...
		if (Nest.value(new_timeperiod,"timeperiod_type").asInteger() == TIMEPERIOD_TYPE_DAILY) {
			...
			tblPeriod.addRow(array(_("Every day(s)"), new CNumericBox("new_timeperiod[every]", Nest.value(new_timeperiod,"every").asString(), 3)), "hidden");
		} else if (Nest.value(new_timeperiod,"timeperiod_type").asInteger() == TIMEPERIOD_TYPE_WEEKLY) {
			tblPeriod.addItem(new CVar("new_timeperiod[month]", bindec(bit_month)));
			tblPeriod.addItem(new CVar("new_timeperiod[day]", Nest.value(new_timeperiod,"day").$()));
			tblPeriod.addItem(new CVar("new_timeperiod[start_date]", Nest.value(new_timeperiod,"start_date").$()));
			tblPeriod.addItem(new CVar("new_timeperiod[month_date_type]", Nest.value(new_timeperiod,"month_date_type").$()));
			tblPeriod.addRow(array(_("Every week(s)"), new CNumericBox("new_timeperiod[every]", Nest.value(new_timeperiod,"every").asString(), 2)), "hidden");
			...
			tabMonths.addRow(array(
				new CCheckBox("new_timeperiod[month_jan]", true, null, 1), _("January"),
				SPACE, SPACE,
				new CCheckBox("new_timeperiod[month_jul]", true, null, 1), _("July")
			));
			tabMonths.addRow(array(
			...
			tblPeriod.addRow(array(_("Month"), tabMonths), "hidden");
