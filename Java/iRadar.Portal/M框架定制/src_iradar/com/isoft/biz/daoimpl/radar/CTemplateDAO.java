	...
	public CArray<Long[]> delete(Long... templateids) {
		...
		try {
			...
			for(Map template : delTemplates) {
				...
				add_audit_ext(this.idBean, getSqlExecutor(), AUDIT_ACTION_DELETE, AUDIT_RESOURCE_TEMPLATE, Nest.value(template,"templateid").asLong(), Nest.value(template,"host").asString(), "hosts", null, null);
			}
