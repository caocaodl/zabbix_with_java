...
public class CHostGroupDAO extends CCoreLongKeyDAO<CHostGroupGet> {
	...
	public <T> T get(CHostGroupGet params) {
		...
		if (userType != USER_TYPE_SUPER_ADMIN && !params.getNopermissions()) {
			...
			sqlParts.where.put("EXISTS ("+
				"SELECT NULL"+
				" FROM rights r"+
				" WHERE (g.tenantid=r.tenantid or (g.tenantid='-' and r.tenantid=#{tenantId}))"+
					" AND g.groupid=r.id"+
		...
		if (!is_null(params.getOnlyHostGroup())) {
			sqlParts.where.put("groupid", "g.groupid>100");
		}
		...
		if (!is_null(params.getHostIds())) {
			...
			sqlParts.where.put("hgg.tenantid", "(hg.tenantid=g.tenantid or (g.tenantid='-' and hg.tenantid=#{tenantId}))");
		...
		if (!is_null(params.getMaintenanceIds())) {
			...
			sqlParts.where.put("hmh.tenantid", "(g.tenantid=mg.tenantid or (g.tenantid='-' and mg.tenantid=#{tenantId}))");
		...
		if (!is_null(params.getMonitoredHosts())) {
			...
			sqlParts.where.put("hgg.tenantid","(hg.tenantid=g.tenantid or (g.tenantid='-' and h.tenantid=#{tenantId}))");
			sqlParts.where.put("hgg","hg.groupid=g.groupid");
		...
		if (!is_null(params.getWithItems())) {
			sqlParts.from.put("hosts_groups","hosts_groups hg");
			sqlParts.where.put("hgg.tenantid","(g.tenantid=hg.tenantid or (g.tenantid='-' and hg.tenantid=#{tenantId}))");
			sqlParts.where.put("hgg","g.groupid=hg.groupid");
			sqlParts.where.put("EXISTS ("+
					"SELECT NULL"+
					" FROM items i"+
		...
		if (!is_null(params.getWithGraphs())) {
			...
			sqlParts.where.put("EXISTS ("+
					...
					" AND (gi.tenantid=g.tenantid or g.tenantid='-')"+
					" AND hg.hostid=i.hostid"+
	...
	public CArray<Long[]> delete(boolean nopermissions, Long... ids) {
		...
		try {
			...
			SqlBuilder opgroupSqlParts = new SqlBuilder();//设备类型删除时，如果关联发现策略则不允许删除
			Map opgroup = DBfetch(DBselect(getSqlExecutor(),
				"SELECT o.groupid "+
				" FROM opgroup o,groups g "+
				" WHERE g.tenantid = o.tenantid and o.groupid= g.groupid and "
				+opgroupSqlParts.dual.dbConditionInt("g.groupid", ids),
				1,
				opgroupSqlParts.getNamedParams()
			));
			
			if (opgroup != null) {
				throw CDB.exception(RDA_API_ERROR_PARAMETERS,
					_s("Host group \"%1$s\" cannot be deleted, because some discovery depend on it.",
						Nest.value(delGroups,opgroup.get("groupid"), "name").asString()
					)
				);
			}
	...
	public CArray<Long[]> massUpdate(CArray data) {
		...
		sqlParts.where.dbConditionString("h.tenantid", new String[] {this.idBean.getTenantId()});
...
//		long count = get(params);
	...
	protected void applyQueryTenantOptions(String tableName, String tableAlias,
		...
		if (Role.isTenant(this.idBean.getTenantRole()) || params.getEditable()) {
			String fieldKey = this.fieldId("tenantid", tableName);
			String fieldId = this.fieldId("tenantid", tableAlias);
			sqlParts.where.dbConditionStringAhead(fieldKey, fieldId, new String[] { idBean.getTenantId(), "-" });
		}
	}
}
