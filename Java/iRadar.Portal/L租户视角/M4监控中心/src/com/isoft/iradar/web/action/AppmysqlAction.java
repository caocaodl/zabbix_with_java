...
public class AppmysqlAction extends RadarBaseAction {
	...
	private final static String SQL_UPDATE_APPLICATION = " update applications set type="+IMonConsts.A_TYPE_MYSQL+" where applicationid= #{applicationid} and tenantid=#{tenantid} ";
	
	@Override
	protected void doInitPage() {
		page("title","Mysql"+_("Service application"));
		page("file", "app_mysql.action");
		page("hist_arg", new String[] { "groupid", "type" });
		page("type", detect_page_type(PAGE_TYPE_HTML));
		page("css", new String[] {"tenant/edit.css","tenant/supervisecenter/apptomcat.css"});
		page("js", new String[] {"imon/common.mysql.js"});
	}
	...
	protected void doCheckFields(SQLExecutor executor) {
		CArray fields = map(
				...
				"dbusername",		   array(T_RDA_STR, O_OPT, null,	null,		null),
				"dbuserpass",		   array(T_RDA_STR, O_OPT, null,	null,		null),
				"dbip",			       array(T_RDA_STR, O_OPT, null,	null,		null),
				"dbport",		       array(T_RDA_STR, O_OPT, null,	null,		null),
				"dbjavahome",		   array(T_RDA_STR, O_OPT, null,	null,		null),
	...
	protected void doAction(final SQLExecutor executor) {
		...
		templateid.put("templateid", IMonConsts.A_TYPE_MYSQL);
		...
		for (Map map : monitors) {
		 ...
		 CArray data = array();
	        if(hasRequest("tsave")){
	        	...
	        	String dbusername = get_request("dbusername");
				String dbuserpass = get_request("dbuserpass");
				String dbip = get_request("dbip", "127.0.0.1");
				String dbport = get_request("dbport", "3306");
				String dbjavahome = get_request("dbjavahome", "");
	        	String testkey = "mysql.threads_running["+dbusername+","+dbuserpass+","+dbip+","+dbport+","+dbjavahome+"]";
	        	String msgOk= _("saveSuccess"), msgFail =_("saveFail");
	        	String hostid = get_request("belongVirt");
	        	if (TvmUtil.checkSameport(executor,getIdentityBean(),Long.valueOf(hostid),applicationid,testkey,IMonConsts.A_TYPE_MYSQL)) {
					resutl=false;
					info(_s("Cannot deploy database username , password, ip,database port,and he java path of the same application in the sanme hosts,please fill in again"));
				}
				if(resutl){
		        	...
		        	try {
						...
						final Map application = map(
								"name", APP_NAME_MYSQL+Nest.value(_REQUEST,"name").$(),
								"hostid", hostid);
						...
						int delay=Nest.value(_REQUEST, "monitoringName").asInteger();//更新频率
						 ...
						 if (!empty(applicationid)) {
							 ...
							 for(Map item : items){
								 ...
								 String newkey = oldkey.substring(0,oldkey.indexOf("[")+1)+dbusername+","+dbuserpass+","+dbip+","+dbport+","+dbjavahome+"]";
							 ...
							 for(Map.Entry<String, String> entry:keymap.entrySet()){
									String key = Cphp.str_replace(
											CArray.array("{$USER}", "{$PSWD}", "{$DBIP}", "{$PORT}", "{$JAVAHOME}"),
											CArray.array(dbusername, dbuserpass,dbip,dbport,dbjavahome),entry.getKey());
				    		    ...
				    		    alarmlist=JsonUtil.JsonToJavas(new AlarmUtil(),jsonstr);
				    			 for(AlarmUtil alarm:alarmlist){
				    					...
				    					if (!empty(applicationid)) {
				    					    ...
				    					    type = oldtype.substring(0,oldtype.indexOf("[")+1)+dbusername+","+dbuserpass+","+dbip+","+dbport+","+dbjavahome+"]";
				    					}else{
				    						oldtype = namemap.get(alarm.getType());
				    						type = Cphp.str_replace(
				    								CArray.array("{$USER}", "{$PSWD}", "{$DBIP}", "{$PORT}","{$JAVAHOME}"),
				    								CArray.array(dbusername, dbuserpass,dbip,dbport,dbjavahome),
													oldtype);
				    					}
        ...
        if(isset(_REQUEST,"form")){
    		...
    		if(empty(applicationid)){
    			...
    			data=map("applicationid",get_request("applicationid",0L),
    					...
    					"dbusername",get_request("dbusername"),
    					"dbuserpass",get_request("dbuserpass"),
    					"dbip",get_request("dbip","127.0.0.1"),
    					"dbport",get_request("dbport","3306"),
    					"dbjavahome",get_request("dbjavahome"),
            	...
            	String dbusername=arrs[0].substring(arrs[0].indexOf("[")+1, arrs[0].length());
            	String dbuserpass=arrs[1];
            	String dbip=arrs[2];
            	String dbport=arrs[3];
            	String dbjavahome = arrs[4].substring(0,arrs[4].length()-1);
            	
            	Nest.value(data, "dbusername").$(dbusername);
           		Nest.value(data, "dbuserpass").$(dbuserpass);
           		Nest.value(data, "dbip").$(dbip);
           		Nest.value(data, "dbport").$(dbport);
           		Nest.value(data, "dbjavahome").$(dbjavahome);
    		...
    		CView editView = new CView("configuration.mysql.edit", data);
	...
	public CArray getHostTable(SQLExecutor executor,CForm form){
		...
		String sql="select applicationid,hostid,name from applications where tenantid=#{tenantid} and type=2 order by applicationid desc ";
		...
		Map apptypes= DBfetch(DBselect(executor, "select templateid,name "
				+ "from i_t_template where templateid=#{templateid}", map("templateid",IMonConsts.A_TYPE_MYSQL)));
		String apptype = Nest.value(apptypes, "name").asString();
		
		for(Map app:apps){
			String appname = removeDefaultProfix(Nest.value(app, "name").asString(),APP_NAME_MYSQL);
    		...
    		CArray edit = array(new CLink(_("monitorConfig"), "app_mysql.action?form=edit&applicationid="+ appid
			...
			CRow row = new CRow(array(
					checkBox,
					new CLink(appname, "mysqlinfor.action?form=update&templateid="+IMonConsts.A_TYPE_MYSQL+"&applicationid="+appid+"&hostid="+hostid),
...
/*	private boolean updateItemStatus(SQLExecutor executor,boolean status,Long itemid){
	...
	boolean result=true;
	  try {
		    int delta=0;
		    if("mysql.questions".equals(key.substring(0, key.indexOf("[")))){
		    	delta=1;
		    }
    ...
    /**
   	...
   	public static void main(String[] agrs){
		...
		System.out.println(st.substring(0, st.indexOf("[")+1));*/
   		String appname = "mysql_mmm11";
   		if(appname.contains("mysql_")){
   			appname = appname.substring(6, appname.length());
   		}
