...
public class EventIpcManagerDefaultImpl implements EventIpcManager{
    ...
    private Integer m_handlerQueueLength = Integer.MAX_VALUE;
    ...
    private static class EventListenerExecutor {
        ...
        public void addEvent(final Event event) {
            m_delegateThread.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        LOG.info("run: calling onEvent on {} for event {} with time {}", m_listener.getName(), event.getUei(), event.getTime());
    ...
    private void broadcastNow(Event event) {
        LOG.debug("Event ID {} to be broadcasted: {}", event, event.getUei());
        ...
        if (event.getUei() == null) {
            LOG.debug("Event ID {} does not have a UEI, so skipping UEI matching", event);
            return;
        }
        ...
        if (sentToListeners.isEmpty()) {
            LOG.debug("No listener interested in event ID {}: {}", event, event.getUei());
        }
    }
