package com.isoft.web.bean.subscribeStatistical;
...
public class SubscribeStatisticalAction extends IaasPageAction {
	...
	public void doReportCSV() throws Exception {
		...
		//获得要导出数据
		RequestEvent request = new RequestEvent();
		request.setCallDAOIF(ISubscribeStatisticalDAO.class);
		request.setCallHandlerIF(ISubscribeStatisticalHandler.class);
		request.setCallHandlerMethod(ISubscribeStatisticalHandler.doSubscribeStatisticalPage);
		request.setModuleName(ModuleConstants.MODULE_SUBSCRIBESTATISTICAL);
		request.setDataSource(DataSourceEnum.IRADAR);
	    ...
	    response.setHeader("Content-disposition","attachment;filename=subscribeStatistical.csv");
		...
		if(dataList.size()>0){
			...
			for(int i=0; i<dataList.size(); i++){
				...
				data[0] = map.get("templateName").toString(); //监控模型名称
				data[1] = map.get("subscribeSum").toString(); //订阅数量
				wr.writeRecord(data);
			}
		}
		wr.close(); 
