...
public class CConfigurationReportFormList extends CViewSegment {
	@Override
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		CDiv titleDiv = new CDiv("资产监控类型统计表");
		titleDiv.setAttribute("class", "hostTitleDiv");
		CTableInfo table = new CTableInfo();
		table.setAttribute("class", "tableinfo");
		table.setHeader(array(_("Group"),"数量"));
		for(Map map:list){
			table.addRow(array(map.get("label"),map.get("value")));
		}
		CDiv toolbar = new CDiv();
		toolbar.setAttribute("class", "toolbar");
		toolbar.addItem(dataexport);
		
		hostDiv.addItem(array(titleDiv,table));
		CForm btnForm = new CForm("get", "intvoisport.action");	
		CWidget mainWidget = inverntForReport(idBean,executor);
	 ...
	 * @return
	 */
	public Map inverntForReportFirm(IIdentityBean idBean, SQLExecutor executor,boolean isFirm){
		...
		if(isFirm){
			...
			if(manufacturers.size() != 0 || !manufacturers.isEmpty()){
				String manuClass = manufacturerTable.getAttribute("class").toString();
				manuClass += " manufacturClass";
				manufacturerTable.setAttribute("class", manuClass);
				manufacturerTable.setHeader(array(
						_("Belong company"),
						_("SERVER"),
						_("Net Hosts"),
						_("Stroge Hosts")
			...
			if(manufacturers.size() != 0 || !manufacturers.isEmpty()){				
				manufacturerTable.addRow(array(_("Total"),hostTotalNum,netTotalNum,storgeTotalNum));
			}
			widget.addItem(manufacturerTable);
			result.put("data", widget);
		}else{
			CArray manufacturerTable = array();
			manufacturerTable.add(array(_("Belong company"),_("SERVER"),_("Net Hosts"),_("Stroge Hosts")));
			//生成表格
			CArray dataArray = null;
			Integer hostTotalNum = 0;
			Integer netTotalNum = 0;
			...
			for(Map manufacturer:manufacturers){//遍历所有的厂商
				...
				netTotalNum += Integer.parseInt(dataArray.get(2).toString());
				storgeTotalNum += Integer.parseInt(dataArray.get(3).toString());
				manufacturerTable.add(dataArray);
			}
			manufacturerTable.add(array("合计",hostTotalNum+"",netTotalNum+"",storgeTotalNum+""));
			result.put("data", manufacturerTable);
		}
		return result;
	}
	...
	public CArray countForHostGroups(IIdentityBean idBean, SQLExecutor executor,Object manufacturerName,Object dkey,boolean isFirm){
		 ...
		 * 系统屏蔽默认的存储设备类型
		 */
//		Long[] groupIds = {IMonConsts.MON_SERVER_LINUX.longValue(),IMonConsts.MON_SERVER_WINDOWS.longValue(),
//				IMonConsts.MON_STORAGE.longValue(),IMonConsts.MON_COMMON_NET.longValue(),IMonConsts.MON_NET_CISCO.longValue()};
...
//				}
	 ...
	 * @return
	 */
	private CWidget inverntForReport(IIdentityBean idBean, SQLExecutor executor){
		CWidget maintenanceWidget = new CWidget();
		CForm createForm = new CForm("get");
		CTableInfo _table = new CTableInfo();
		String tbClass = _table.getAttribute("class").toString();
		tbClass += " table_inverntories";
		_table.setAttribute("class", tbClass);
        CJSScript _fusionScript = new CJSScript(get_js("inite();initeInventoryHostType();"));
        
		CDiv newdiv = new CDiv();
		newdiv.setAttribute("id", "todayMonItemChart");
		//厂商列表
//		Map map = inverntForReportFirm(idBean,executor,true);
//		CTableInfo table = (CTableInfo)map.get("data");
//		CDiv titleDiv = new CDiv("资产物理类型统计表");
//		titleDiv.setAttribute("class", "caption");
//		CDiv div = new CDiv();
//		div.setAttribute("class", "manufacturCon");
//		div.addItem(array(titleDiv,table));
//		CDiv newdiv1 = new CDiv();
//		newdiv1.setAttribute("id", "newdata");
		CDiv inventoryHostTypeChart = new CDiv();
		inventoryHostTypeChart.setAttribute("id", "inventoryHostTypeChart");
		_table.addRow(array(newdiv, inventoryHostTypeChart,_fusionScript));
		createForm.addItem(_table);
		//table.addItem(newdiv1);
//		createForm.addItem(newdiv1);
