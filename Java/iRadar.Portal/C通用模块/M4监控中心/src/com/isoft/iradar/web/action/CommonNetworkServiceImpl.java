...
public class CommonNetworkServiceImpl implements IHostDetailService {
	@Override
	public CTableInfo getTargetKey(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		titleRowFrist.addItem(_("net up error"));
		titleRowFrist.addItem(_("net down error"));
		target_key.addRow(titleRowFrist,"speed_center");
		
		CRow firstRow = new CRow();
		firstRow.addItem(viewService.create_Favorite_Graphs_Prototype(idBean, executor, hostid,ItemsKey.COMMON_NET_IFOUTERRORS));
		firstRow.addItem(viewService.create_Favorite_Graphs_Prototype(idBean, executor, hostid,ItemsKey.COMMON_NET_IFINERRORS));
	...
	public CFormList getOverviewForm(IIdentityBean idBean, SQLExecutor executor,Map data,String hostid){
		//获取数据
		String hostname = LatestValueHelper.buildByNormalKey(EasyObject.asLong(hostid), ItemsKey.COMMON_NET_SYSNAME.getValue()).value().out().format();//设备名称
		...
		String sysUpTime = LatestValueHelper.buildByNormalKey(EasyObject.asLong(hostid), ItemsKey.COMMON_NET_SYSUPTIME.getValue()).value().out().format();//cpu空闲率
		...
		leftData.put(_("Runing Time"), sysUpTime);
		
		Map rightData = array();
		rightData.put(_("OS Info"), type_full);
