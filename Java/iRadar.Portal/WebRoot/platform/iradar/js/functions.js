...
function call_ins_macro_menu(ev) {
	show_popup_menu(ev, [
		[locale['S_INSERT_MACRO'], null, null, {
			'outer': ['pum_oheader'],
			'inner': ['pum_iheader']
		}],
		['TRIGGER.VALUE=0', function() {
				set_macro(0);
			},
			null, {
				'outer': ['pum_o_item'],
...
function call_triggerlog_menu(evnt, id, name, menu_options) {
	...
	if (typeof(menu_options) != 'undefined') {
		show_popup_menu(evnt, [
			...
			[tname,function(){ return openWinCentered('tr_logform.action?sform=1&itemid='+id, 'TriggerLog', 880, 540, 'titlebar=no, resizable=yes, scrollbars=yes, dialog=no')},"", {
				'outer': ['pum_o_item'],
				'inner': ['pum_i_item']
			}],
			menu_options
		...
		show_popup_menu(evnt, [
			...
			[tname,function(){return openWinCentered('tr_logform.action?sform=1&itemid='+id, 'ServiceForm', 880, 540, 'titlebar=no, resizable=yes, scrollbars=yes, dialog=no')},"", {
				'outer': ['pum_o_item'],
				'inner': ['pum_i_item']
			}]
		], 140);
...
function copy_expression(id) {
	...
	if (expr_temp.value.length > 0) {
		showModalWindow('提示', locale['DO_YOU_REPLACE_CONDITIONAL_EXPRESSION_Q'], [{
			text: '确定',
			click: function() {
				var src = document.getElementById(id);
				if (typeof src.textContent != 'undefined') {
	...
	}else if(!expr_temp.value.length){
		var src = document.getElementById(id);
		if (typeof src.textContent != 'undefined') {
			expr_temp.value = src.textContent;
		} else {
...
function cloneRow(elementid, count) {
	...
	var emptyEntry = tpl.evaluate({
		'id': cloneRow.count
	});

	var newEntry = $(elementid).insert({
		'before': emptyEntry
	}).previousSibling;
...
// dashboard js menu
function create_page_menu(e, id, width) {
	if (!e) {
		e = window.event;
	}
	id = 'menu_' + id;
	...
	show_popup_menu(e, dbrd_menu, width);
}

// triggers js menu
function create_mon_trigger_menu(e, args, items) {
	var tr_menu = [
		[t('Triggers'), null, null, {
			'outer': ['pum_oheader'],
			'inner': ['pum_iheader']
		}],
		[t('Events'), 'events.action?triggerid=' + args[0].triggerid + '&nav_time=' + args[0].lastchange + '&source=0', null]
	];
	...
	tr_menu.push([history_section_caption, null, null, {
		'outer': ['pum_oheader'],
		'inner': ['pum_iheader']
	}]);
	...
	for (var itemid in items) {
		...
		tr_menu.push([items[itemid].name, 'history.action?action=' + items[itemid].action + '&itemid=' + items[itemid].itemid, null]);
	}
...
function testUserSound(idx) {
	...
	if (repeat == 1) {
		...
		AudioList.loop(sound, {
			'seconds': repeat
		});
	} else {
		AudioList.loop(sound, {
			'seconds': $('messages_timeout').value
		});
	}
}
...
function getNextColor(paletteType) {
	...
	hexColor = ('0' + parseInt(r, 10).toString(16)).slice(-2) + ('0' + parseInt(g, 10).toString(16)).slice(-2) + ('0' + parseInt(b, 10).toString(16)).slice(-2);
...
function moveListBoxSelectedItem(formname, objname, from, to, action) {
	...
	jQuery('#' + from).find('option:selected').each(function(i, fromel) {
		...
		if (action.toLowerCase() == 'add') {
			jQuery(document.forms[formname]).append("<input name='" + objname + '[' + fromel.val() + ']' + "' id='" + objname + '_' + fromel.val() + "' value='" + fromel.val() + "' type='hidden'>");
...
function formatTimestamp(timestamp, isTsDouble, isExtend) {
	...
	str += (isExtend && isTsDouble) ? days + locale['S_DAY_SHORT'] + ' ' : ((days == 0) ? '' : days + locale['S_DAY_SHORT'] + ' ');
...
function executeScript(hostId, scriptId, confirmation) {
	var execute = function() {
		if (!empty(hostId)) {
			openWinCentered('scripts_exec.action?hostid=' + hostId + '&scriptid=' + scriptId, 'Tools', 560, 470,
