...
var chkbxRange = {
	...
	init: function() {
		var path = new Curl();
		var filename = basename(path.getPath(), '.action');
...
var AudioList = {
	...
	startLoop: function(audiofile, loop) {
		...
		this.list[audiofile].onEnded = this.loop.bind(this, audiofile, {
			'repeat': 0
		});
...
var hintBox = {
	...
	HintWraper: function(e, target, hintText, width, className) {
		...
		if (typeof hintText === 'string') {
			hintText = "<div class='hintbox-tableCon'>" + hintText + "</div>";
		}
	...
	positionHint: function(e, target) {
		var wWidth = document.body.clientWidth,//jQuery(window).width(),
		...
		if ((top - scrollTop + target.hintBoxItem.height() > wHeight) && (target.clientX - 10 > target.hintBoxItem.width() || wWidth - target.clientX - 10 > target.hintBoxItem.width())) {
...
function show_color_picker(id) {
	...
	//第二个参数鼠标事件event
	var pos = getPosition(curr_lbl, arguments.callee.caller.arguments[0]);
