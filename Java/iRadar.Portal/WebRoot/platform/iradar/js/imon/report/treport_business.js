...
jQuery(function($) {
	...
	$("#csv_export").click(function() {
		...
		var url = "tbusiness_report.action?csv_export=true&topnperiod=" + period + "&groupid=" + groupid;
		location.href = url;
	});
...
function getJsonTrenddata() {
	var url = "iradar/tbusiness_report.action";
	var data = getParameterdata();
	showloading();
...
function Trenddatashow(item) {
	...
	if (groupid == 1) {
		...
		titlearray = ["系统会话数", "每秒错误次数", "告警次数"];
	} else if (groupid == 3) {
		titlearray = ["响应时间", "可用率", "告警次数"];
	}
	...
	for (var i = 1; i < 4; i++) {
		if (titlearray[i - 1] == "") {
			...
			if (titlearray[i - 1] == "告警次数") {
				jQuery('#area' + i + '_title').text(titlearray[i - 1]).attr("title", "单个应用产生的告警数");
			} else {
...
function getJsondata(data) {
	...
	if (groupid == 1) {
		titlearray = ["链接数", "应用告警数", ""];
		unitarray = ["个", "个", ""];
	} else if (groupid == 2) {
		titlearray = ["系统会话数", "每秒错误次数", "应用告警数"];
		unitarray = ["个", "次", "个"];
	} else if (groupid == 3) {
		titlearray = ["响应时间", "应用告警数", "可用率"];
		unitarray = ["", "个", ""];
	}
	...
	for (var i = 1; i < 4; i++) {
		if (titlearray[i - 1] == "") {
			...
			jQuery('#area' + i + '_title').show();
			jQuery('#area' + i + '_title').text(titlearray[i - 1]);
			if (titlearray[i - 1] == "应用告警数") {
				jQuery('#area' + i + '_title').text(titlearray[i - 1]).attr("title", " 正常-周期内应用未产生告警\n 异常-周期内应用未产生告警\n 注：一个应用算一个，跟产生告警数无关");
			} else {
				jQuery('#area' + i + '_title').text(titlearray[i - 1]).removeAttr("title");
				jQuery('#area' + i + '_title').text(titlearray[i - 1]);
			}
	...
	jQuery.getJSON(url, data, function(dataObj) {
		jQuery.each(dataObj, function(i, item) {
			...
			if (groupid == 1) {
				...
				jQuery.each(item.left2, function(i, item) {
					Ydata3.push(item.name);
					Xdata3.push(item.value);
				});
				jQuery.each(item.fleft2, function(i, item) {
					fYdata3.push(item.name);
					fXdata3.push(-item.value);
				});
			}
		...
		if (groupid == 1) {
			...
			showPieImage(area[1], Ydata2, Xdata2);
			showdataTop5(area[2], Ydata3.reverse(), Xdata3.reverse(), fYdata3.reverse(), fXdata3.reverse(), titlearray[2], unitarray[2]);
		}
		hideloading();
	});
...
function addHosts() {
	var AUTOCOMPLETE = "autocomplete",
		businessURL = "iradar/tbusiness_report.action",
