...
function getMenuPopupHost(options) {
	...
	if (options.hasGoTo) {
		...
		gotos[gotos.length] = {
			label: t('Latest data'),
			url: new Curl('host_list.action?hostid=' + options.hostid +"&groupid="+groupid).getUrl()
		};

		//host detail
		gotos[gotos.length] = {
			label: t('Host detail'),
			url: new Curl('host_detail.action?hostid=' + options.hostid+"&groupid="+groupid).getUrl()
		};
		
		// inventory
		gotos[gotos.length] = {
			label: t('Host inventory'),
			url: new Curl('hostinventories.action?hostid=' + options.hostid).getUrl()
		};
		...
		if (options.hasScreens) {
			gotos[gotos.length] = {
				...
				url: new Curl('host_screen.action?hostid=' + options.hostid).getUrl()
			};
		}
...
function getMenuPopupMap(options) {
	...
	if (typeof options.gotos !== 'undefined') {
		...
		if (typeof options.gotos.inventory !== 'undefined') {
			var url = new Curl('hostinventories.action');
		...
		if (typeof options.gotos.screens !== 'undefined') {
			var url = new Curl('host_screen.action');
		...
		if (typeof options.gotos.triggerStatus !== 'undefined') {
			var url = new Curl('tr_status.action?filter_set=1');
		...
		if (typeof options.gotos.events !== 'undefined') {
			var url = new Curl('events.action?source=0');
...
function getMenuPopupTrigger(options) {
	...
	// events
	var url = new Curl('events.action?triggerid=' + options.triggerid + '&source=0');
	...
	if (!empty(options.acknowledge)) {
		var url = new Curl('acknow.action');
	...
	if (!empty(options.configuration)) {
		var url = new Curl('triggers.action?triggerid=' + options.triggerid +
			'&hostid=' + options.configuration.hostid + '&form=update');
	...
	if (!empty(options.items)) {
		...
		jQuery.each(options.items, function(i, item) {
			var url = new Curl('history.action');
...
function getMenuPopupHistory(options) {
	...
	if (typeof options.hasLatestGraphs !== 'undefined' && options.hasLatestGraphs) {
		items[items.length] = {
			...
			url: new Curl('history.action?itemid=' + options.itemid + '&action=showgraph&period=3600').getUrl()
		};

		items[items.length] = {
			label: t('Last week graph'),
			url: new Curl('history.action?itemid=' + options.itemid + '&action=showgraph&period=604800').getUrl()
		};

		items[items.length] = {
			label: t('Last month graph'),
			url: new Curl('history.action?itemid=' + options.itemid + '&action=showgraph&period=2678400').getUrl()
		};
	}
	...
	items[items.length] = {
		...
		url: new Curl('history.action?itemid=' + options.itemid + '&action=showvalues&period=3600').getUrl()
	};
