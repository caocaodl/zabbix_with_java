...
function create_calendar(time, timeobjects, id, utime_field_id, parentNodeid, options) {
	id = id || CLNDR.length;
	if ('undefined' == typeof(utime_field_id)) {
		utime_field_id = null;
	}
	CLNDR[id] = new Object;
	CLNDR[id].clndr = new calendar(id, time, timeobjects, utime_field_id, parentNodeid, options);
	return CLNDR[id];
}
...
calendar.prototype = {
	...
	initialize: function(id, stime, timeobjects, utime_field_id, parentNodeid, options) {
	...
	setSDateFromOuterObj: function() {
		...
		if (!is_null(this.clndr_utime_field)) {
			this.clndr_utime_field.value = this.sdt.getRDADate();
		}
		return true;
	},
	...
	setDone: function() {
		if (this.before === true && this.cdt.getTime() < this.sdt.getTime()) {
			return false;
		}
		this.syncSDT();
		this.syncBSDateBySDT();
	...
	createDaysTab: function() {
		...
		if (this.before) {
			...
			if (this.before === true) {
				...
				cDate = this.before();
			} else if (this.before instanceof Date) {
				cDate = this.before;
			} else {
				cDate = new Date(this.before * 1);
			}
			beforeUtc = Date.UTC(cDate.getFullYear(), cDate.getMonth(), cDate.getDate())
		}
		...
		for (var y = 0; y < 6; y++) {
			...
			for (var x = 0; x < 7; x++) {
				...
				if(this.cdt.getFullYear()>YEAR_LIMIT){
					continue;
				}
				...
				if (this.sdt.getFullYear() == this.cdt.getFullYear() && this.sdt.getMonth() == this.cdt.getMonth() && this.sdt.getDate() == this.cdt.getDate()) {
					td.addClassName('selected');
					this.clndr_selectedday = td;
				}
				if (this.before) {
					/* 判断客户端时间与日历插件显示日期大小*/
					if (beforeUtc > Date.UTC(this.cdt.getFullYear(), this.cdt.getMonth(), this.cdt.getDate())) {
	...
	calendarcreate: function(parentNodeid) {
		...
		column1.setAttribute('style', 'padding-left: 10px;');
		...
		this.clndr_now.setAttribute('style', '');
		column3.appendChild(this.clndr_now);
		...
		this.clndr_done.setAttribute('style', 'margin:0 10px;');
		column3.appendChild(this.clndr_done);
		this.clndr_calendar.appendChild(line_div);
	}
}