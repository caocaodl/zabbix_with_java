...
var timeControl = {
		...
		for (var id in this.objectList) {
			if (!empty(this.objectList[id]) && !this.objectList[id].processed && this.objectList[id].loadScroll) {
				...
				obj.time.starttime = (!isset('starttime', obj.time) || is_null(obj.time['starttime']))
					...
					: nowDate.setRDADate(obj.time.starttime) / 1000;

				obj.time.usertime = (!isset('usertime', obj.time))
					? obj.time.endtime
					: nowDate.setRDADate(obj.time.usertime) / 1000;
	...
	refreshImage: function(id) {
		var obj = this.objectList[id],
			...
			stime = new CDate((this.timeline.usertime() - this.timeline.period()) * 1000).getRDADate();
	...
	objectUpdate: function() {
		...
		if (isNaN(usertime) || isNaN(period)) {
			for (var id in this.objectList) {
				if (isset(id, RDA_SBOX)) {
					RDA_SBOX[id].clearParams();
				}
			}
		...
		if (this.refreshPage) {
			...
			url.setArgument('stime', date.getRDADate());
			...
			flickerfreeScreen.refreshAll(period, date.getRDADate(), this.timeline.isNow());
		}
	},
...
var CScrollBar = Class.create(CDebug, {
	...
	setCalendarRight: function(time) {
		...
		// dynamic
			...
			var originPeriod = timeControl.timeline.period();
			var originStarttime = timeControl.timeline.starttime();
			//crack to jump the method of "timeControl.timeline.usertime" which'll check the new usertime whether less than starttime, so begin set the "right starttime" then restore origin value
			var tmpStarttime = time - originPeriod;
			timeControl.timeline.starttime(tmpStarttime);
			
			var startusertime = timeControl.timeline.usertime() - timeControl.timeline.period();
			timeControl.timeline.usertime(time);
			
			//restore origin value
			timeControl.timeline.starttime(originStarttime);
	...
	appendCalendars: function() {
		var beforeTime = timeControl.timeline.starttime()*1000;
		this.clndrLeft = create_calendar(timeControl.timeline.usertime() - timeControl.timeline.period(), this.dom.info_left, null, null, 'scrollbar_cntr', {before: beforeTime});
		this.clndrRight = create_calendar(timeControl.timeline.usertime(), this.dom.info_right, null, null, 'scrollbar_cntr', {before: function(){
			return new Date((timeControl.timeline.usertime()-timeControl.timeline.period()) * 1000);
		}});
	...
	addListeners: function() {
		...
		if (IE) {
			...
			jQuery('#flickerfreescreen_' + this.sbox_id + ' a').click(function() {
				RDA_SBOX[this.sbox_id].ieMouseClick();
			});
		}
		else {
...
function sboxGlobalMove() {
	for (var id in RDA_SBOX) {
		if (!empty(RDA_SBOX[id])) {
			RDA_SBOX[id].moveSBoxByObj();
		}
	}
}
...
function sboxGlobalMousemove(e) {
	for (var id in RDA_SBOX) {
		if (!empty(RDA_SBOX[id]) && RDA_SBOX[id].is_active) {
			RDA_SBOX[id].mouseMove(e);
		}
	}
}
