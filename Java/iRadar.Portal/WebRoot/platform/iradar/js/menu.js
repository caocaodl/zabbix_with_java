	...
	for (var i = 0; i < content.length; i++) {
		...
		if (content[i].length > 4) {
			for (var j = 4; j < content[i].length; j++) {
				content[i][j].push(content[i][j][0]);
...
function popup_menu(a_items, a_tpl, x, y) {
	...
	this.n_scr_width = (document.body.clientWidth) ? document.body.clientWidth : document.width;
...
function menu_collapse(n_id) {
	...
	if (-1 == n_tolevel) {
		for (n_id = 0; n_id < this.a_index.length; n_id++) {
			...
			if (o_curritem) {
				var parent = document.getElementById(o_curritem.e_oelement.id + "&" + o_curritem.n_id);
		...
		for (n_id = 0; n_id < this.a_index.length; n_id++) {
			...
			if (o_curritem && o_curritem.n_depth > n_tolevel && o_curritem.b_visible) {
				...
				var parent = document.getElementById(o_curritem.o_parent.e_oelement.id + "&" + o_curritem.o_parent.n_id);
				if (parent) {
...
function menu_expand(n_id) {
	...
	// direct child items parent
	var parentId = o_item.e_oelement.id + "&" + o_item.n_id,
		parent = document.getElementById(parentId);

	if (o_item.a_children.length) {
		var _e_oelement = o_item.e_oelement;
		if (!parent) {
			...
			parent.onmouseover = function() {
				...
				this.onmouseover = o_item.e_oelement.onmouseover;
				this.onmouseover.call(o_item.e_oelement);
				this.onmouseover = null;
			};
			parent.onmouseout = function(event) {
				var even = event || window.event,
					toElement = even.relatedTarget || even.toElement;
				if (toElement && (toElement == this || (toElement.className.indexOf("remove") > -1 || toElement.className.indexOf("pum_i_item") > -1 || toElement.className.indexOf("pum_o_item") > -1))) {

				} else {
					this.onmouseout = o_item.e_oelement.onmouseout;
					this.onmouseout.call(o_item.e_oelement);
				}
			};
		};
		parent.style.visibility = 'visible';
		parent.style.position = 'absolute';
		parent.className = "pum_i_item_con";
		var firstChild = o_item.a_children[0],
			_top = firstChild.n_y,
			_left = firstChild.n_x,
			_winHeight = jQuery(window).height(),
			_e_oelementLeft = parseInt(_e_oelement.style.left),
			_e_oelementTop = parseInt(_e_oelement.style.top),
			_e_oelementWidth = parseInt(_e_oelement.style.width);
		if (_e_oelementLeft < _e_oelementWidth) {
			_left = _e_oelementWidth + _e_oelementLeft + 10;
			_top = _e_oelementTop;
		}
		parent.style.top = _top + 'px';
		parent.style.left = _left + 'px';
		parent.style.width = o_item.getprop('width') + 'px';
		parent.style.maxHeight = '250px';
		parent.style.overflowX = 'hidden';
		parent.style.overflowY = 'auto';
		parent.o_root_n_id = o_item.e_oelement.o_root_n_id;
		parent.this_n_id = o_item.e_oelement.this_n_id;
	...
	for (var n_order = 0; n_order < o_item.a_children.length; n_order++) {
		...
		o_curritem.e_oelement.style.position = "static";
		o_curritem.b_visible = true;
		o_curritem.e_oelement.onmouseout = function() {
			return false;
		};
		o_curritem.e_oelement.onmouseover = function() {
			return false;
		};

		parent.appendChild(o_curritem.e_oelement);
	}
	var $parent = jQuery(parent),
		_result = _winHeight - ($parent.height() + _top);
	if (_result < 0) {
		_result -= 20;
		$parent.css("top", _top + _result);
	}
}
...
function menu_item(o_parent, n_order) {
	...
	this.n_x = n_order ? o_parent.a_children[n_order - 1].n_x + this.getprop('left') * o_parent.get_x_direction() : o_parent.n_x + this.getprop('block_left') * o_parent.get_x_direction();
	...
	// top
	this.n_y = n_order ? o_parent.a_children[n_order - 1].n_y + this.getprop('top') : o_parent.n_y + this.getprop('block_top') * (o_parent == o_root ? o_parent.get_y_direction() : 1);
	...
	if (typeof(this.a_config[0]) == 'string' && this.a_config[0].length > 35) {
		...
		eldiv.setAttribute('title', this.a_config.last());
	}
