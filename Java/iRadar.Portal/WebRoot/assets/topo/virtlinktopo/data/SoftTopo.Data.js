...
SoftTopo.Data.prototype.doLayout = function() {
    var _this = this;
    /*
     *控制子网内的设备显示与隐藏（子网内的设备单独布局）
     */
    function subNetIsVisible(subNets, visible) {
        ...
        for (var i = 0, len = subNets.length; i < len; i++) {
            var subnetwork = subNets[i];
            if (subnetwork.childrenCount) {
                childrenIsVisible(subnetwork.children.datas, visible);
            };
        };
    };
    subNetIsVisible(_this.subNets, false);
    this.layout.doLayout({
        callback: function() {
            subNetIsVisible(_this.subNets, true);
            _this.graph.zoomToOverview();
        }
    });
}