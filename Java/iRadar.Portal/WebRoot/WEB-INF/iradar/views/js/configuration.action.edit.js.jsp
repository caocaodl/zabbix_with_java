...
<%@ page import="static com.isoft.iradar.Cphp._"%>
<%@ page import="static com.isoft.iradar.inc.Defines.*"%>
<%@ page import="static com.isoft.iradar.utils.CJs.encodeJson"%>
<!-- Discovery Actions-->
...
<tr id="opGroupRow_#{groupid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpGroupRow('#{groupid}');" />
    </td>
</tr>
</script>
...
<tr id="opTemplateRow_#{templateid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpTemplateRow('#{templateid}');" />
    </td>
</tr>
</script>
...
<tr id="opmsgUsrgrpRow_#{usrgrpid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpmsgUsrgrpRow('#{usrgrpid}');" />
    </td>
</tr>
</script>
...
<tr id="opmsgUserRow_#{userid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpmsgUserRow('#{userid}');" />
    </td>
</tr>
</script>
...
<tr id="opCmdGroupRow_#{groupid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpCmdRow('#{groupid}', 'groupid');" />
    </td>
</tr>
</script>
...
<tr id="opCmdHostRow_#{hostid}">
    ...
    <td>
        <input type="button" class="input link_menu icon remove" name="remove" value="<%=_("Remove")%>" onclick="removeOpCmdRow('#{hostid}', 'hostid');" />
    </td>
</tr>
</script>
...
<div id="opcmdEditForm">
    <table class="objectgroup border_dotted ui-corner-all inlineblock" style="min-width: 310px;">
        ...
        <tr>
            ...
            <td>
                <select name="opCmdTarget" class="input select">
                    <option value="current"><%=_("Current host")%></option>
                    <option value="host"><%=_("Host")%></option>
                    <option value="hostGroup"><%=_("Host group")%></option>
                </select>
            </td>
        ...
        <tr>
            <td colspan="3">
                <input type="button" class="input link_menu save" name="save" value="#{operationName}" />&nbsp;
                <input type="button" class="input link_menu cancel" name="cancel" value="<%=_("Cancel")%>" />
            </td>
        </tr>
        </tbody>
    </table>
    ...
    <td>
        <div class="objectgroup inlineblock border_dotted ui-corner-all" id="uniqList">
            <div>
                ...
                <label for="execute_on_agent"><%=_("iRadar agent")%></label>
            </div>
            <div>
                <input type="radio" id="execute_on_server" name="execute_on" value="1" class="input radio">
                <label for="execute_on_server"><%=_("iRadar server")%></label>
            </div>
        </div>
    </td>
</tr>
...
<script type="text/javascript">
    var RDA_SCRIPT_TYPES = {
        script: <%=RDA_SCRIPT_TYPE_CUSTOM_SCRIPT%>,
        ipmi: <%=RDA_SCRIPT_TYPE_IPMI%>,
        telnet: <%=RDA_SCRIPT_TYPE_TELNET%>,
        ssh: <%=RDA_SCRIPT_TYPE_SSH%>,
        userscript: <%=RDA_SCRIPT_TYPE_GLOBAL_SCRIPT%>
    };
        ...
        for (i = 0; i < list.values.length; i++) {
            ...
            switch (list.object) {
                ...
                case 'groupid':
                    ...
                    value.objectCaption = <%=encodeJson(_("Host group")+NAME_DELIMITER)%>;
                ...
                case 'hostid':
                    ...
                    if (value.hostid.toString() != '0') {
                        value.objectCaption = <%=encodeJson(_("Host")+NAME_DELIMITER)%>;
                    }
                    else {
                        value.name = <%=encodeJson(_("Current host"))%>;
                    }
    ...
    function showOpCmdForm(opCmdId) {
        ...
        objectTPL.operationName = <%=encodeJson(_("Add"))%>;
    ...
    function saveOpCmdForm() {
        ...
        if (object.target == 'hostGroup') {
            ...
            if (empty(values)) {
                alert(<%=encodeJson(_("You did not specify host group for operation."))%>);
        ...
        // host
            ...
            if (object.target != 'current' && empty(values)) {
                alert(<%=encodeJson(_("You did not specify host for operation."))%>);
    ...
    function showOpTypeForm() {
        ...
        opTypeFields = {
            class_opcommand_userscript: [RDA_SCRIPT_TYPES.userscript],
            class_opcommand_execute_on: [RDA_SCRIPT_TYPES.script],
            class_opcommand_port: [RDA_SCRIPT_TYPES.ssh, RDA_SCRIPT_TYPES.telnet],
            class_opcommand_command: [RDA_SCRIPT_TYPES.script, RDA_SCRIPT_TYPES.ssh, RDA_SCRIPT_TYPES.telnet],
            class_opcommand_command_ipmi: [RDA_SCRIPT_TYPES.ipmi],
            class_authentication_method: [RDA_SCRIPT_TYPES.ssh],
            class_authentication_username: [RDA_SCRIPT_TYPES.ssh, RDA_SCRIPT_TYPES.telnet],
            class_authentication_publickey: [],
            class_authentication_privatekey: [],
            class_authentication_password: [RDA_SCRIPT_TYPES.ssh, RDA_SCRIPT_TYPES.telnet],
            class_authentication_passphrase: [RDA_SCRIPT_TYPES.ssh]
        };
    ...
    function processTypeOfCalculation() {
        ...
        if (count > 1) {
            ...
            if (jQuery('#evaltype').val() == <%=ACTION_EVAL_TYPE_AND%>) {
                groupOperator = <%=encodeJson(_("and"))%>;
                globalOperator = <%=encodeJson(_("and"))%>;
            }
            else if (jQuery('#evaltype').val() == <%=ACTION_EVAL_TYPE_OR%>) {
                groupOperator = <%=encodeJson(_("or"))%>;
                globalOperator = <%=encodeJson(_("or"))%>;
            }
            else {
                groupOperator = <%=encodeJson(_("or"))%>;
                globalOperator = <%=encodeJson(_("and"))%>;
    ...
    function addDiscoveryTemplates() {
        ...
        jQuery('#dsc_templateid').multiSelect.clean('discoveryTemplates');
    }
    ...
    function addDiscoveryHostGroup() {
        ...
        jQuery('#dsc_groupid').multiSelect.clean('discoveryHostGroup');
    }
...
//        });
        ...
        jQuery('#select_opcommand_script').click(function() {
            PopUp('popup.action?srctbl=scripts&srcfld1=scriptid&srcfld2=name&dstfrm=action.edit&dstfld1=new_operation_opcommand_scriptid&dstfld2=new_operation_opcommand_script', 480, 400);
        });
