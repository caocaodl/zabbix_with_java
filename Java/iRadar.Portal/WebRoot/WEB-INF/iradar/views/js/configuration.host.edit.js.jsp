...
<tr class="interfaceRow" id="hostInterfaceRow_#{iface.interfaceid}" data-interfaceid="#{iface.interfaceid}">
	...
	<td class="interface-connect-to">
		<div class="jqueryinputset">
			...
			<label for="radio_ip_#{iface.interfaceid}"><%=Cphp._("IP")%></label><label for="radio_dns_#{iface.interfaceid}"><%=Cphp._("DNS")%></label>
		</div>
	</td>
	...
	<td class="interface-control">
		<button type="button" id="removeInterface_#{iface.interfaceid}" data-interfaceid="#{iface.interfaceid}" class="link_menu icon remove #{*attrs.cls}" #{*attrs.disabled}></button>
	</td>
</tr>
</script>
	...
	var hostInterfacesManager = (function() {
		...
		function addNotDraggableIcon(domElement) {
			...
			jQuery('.ui-icon', domElement).hover(
				function (event) {
					jQuery('<div>' + <%=CJs.encodeJson(Cphp._("Interface is used by items that require this type of the interface."))%> + '</div>')
		...
		function getHostInterfaceNumericType(typeName) {
			...
			switch (typeName) {
				case 'agent':
					typeNum = '<%=Defines.INTERFACE_TYPE_AGENT%>';
					break;
				case 'snmp':
					typeNum = '<%=Defines.INTERFACE_TYPE_SNMP%>';
					break;
				case 'jmx':
					typeNum = '<%=Defines.INTERFACE_TYPE_JMX%>';
					break;
				case 'ipmi':
					typeNum = '<%=Defines.INTERFACE_TYPE_IPMI%>';
					break;
				default:
	...
	jQuery(document).ready(function() {
		...
		jQuery('div.jqueryinputset input[name=inventory_mode]').click(function() {
			...
			switch (jQuery(this).val()) {
				case '<%=Defines.HOST_INVENTORY_DISABLED%>':
					inventoryFields.prop('disabled', true);
					jQuery('.populating_item').hide();
					break;
				case '<%=Defines.HOST_INVENTORY_MANUAL%>':
					inventoryFields.prop('disabled', false);
					jQuery('.populating_item').hide();
					break;
				case '<%=Defines.HOST_INVENTORY_AUTOMATIC%>':
					inventoryFields.prop('disabled', false);
