...
<script type="text/javascript">
	jQuery(function() {
		...
		jQuery('#algorithm').change(function() {
			var statusDisabled = (jQuery(this).val() == <%=Defines.SERVICE_ALGORITHM_NONE%>);
	...
	function add_child_service(name, serviceid, trigger) {
		if (jQuery('#children_' + serviceid + '_serviceid').attr('id') == null) {
			...
			url.setAttribute('href', 'services.action?form=1&serviceid=' + serviceid);
			...
			inputRemove.setAttribute('value', <%=CJs.encodeJson(Cphp._("Remove"))%>);
