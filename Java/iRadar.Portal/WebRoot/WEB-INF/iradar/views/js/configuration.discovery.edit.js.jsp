...
<script type="text/x-jquery-tmpl" id="dcheckRowTPL">
	<tr id="dcheckRow_#{dcheckid}">
		...
		<td>
			<input type="button" class="input link_menu icon edit" name="edit" value="<%=CHtml.encode(Cphp._("Edit"))%>"
				onclick="javascript: showNewCheckForm(null, null, '#{dcheckid}');" />
		</td>
		<td>
			<input type="button" class="input link_menu icon remove" name="remove" value="<%=CHtml.encode(Cphp._("Remove"))%>"
		...
		<div class="objectgroup inlineblock border_dotted ui-corner-all">
			<table class="formElementTable">
				...
				<tr id="newCheckPortsRow" class="hidden">
					...
					<td>
						<input type="text" onkeyup="value=value.replace(/[^\d]/g,'') " onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" id="ports" name="ports" value="" class="input text" size="16" maxlength="255">
					</td>
				</tr>
			...
			<table class="formElementTable cmd_table">
				<tr>
					<td>
						<input type="button" id="add_new_dcheck" name="add_new_dcheck" class="input button link_menu add"
							value="<%=CHtml.encode(Cphp._("Add"))%>" />
					</td>
					<td>
						<input type="button" id="cancel_new_dcheck" name="cancel_new_dcheck" class="input button link_menu cancel"
...
<script type="text/javascript">
	...
	var DEFAULTAGENTCHECKKEY = "agent.ping";
	var DEFAULTSNMPCHECKKEY = "sysDescr.0";
	...
	function checkSubmit(){
		var index = jQuery('#tabs').tabs('option', 'active');
		jQuery("#cabIndexFlag").val(index);
		return true;
}	
	 function selectNext(){
		 jQuery.cookie("tab", 1);
	 ...
	 function selectQQTwo(){
		 jQuery.cookie("tab", 1);
	  	 jQuery('#tabs').tabs('option', 'active', 1);
	     } 
	 function selectNextTwo(){
		 jQuery.cookie("tab", 2);
	...
	function addPopupValues_discovery(list) {
		// templates
		...
		for (var i = 0; i < list.length; i++) {
			...
			if(checkName.length>35){
				checkName = checkName.substring(0,35)+"...";
				value['name'] = checkName;
			}
			...
			// update
				...
				// update check name
				jQuery('#dcheckCell_' + value.dcheckid + ' .bold').text(checkName);
			}
			...
			if (jQuery.inArray(parseInt(value.type, 10), availableDeviceTypes) > -1) {
				if (uniquenessCriteria.length) {
					jQuery('label[for=uniqueness_criteria_' + value.dcheckid + ']').text(checkName);
				}
				else {
	...
	function showNewCheckForm(e, dcheckType, dcheckId) {
		...
		if (jQuery('#new_check_form').length == 0) {
			...
			jQuery('#add_new_dcheck')[isUpdate?'addClass':'removeClass']('update');
			...
			jQuery('#dcheckCell_' + dcheckId + ' input').each(function(i, item) {
				...
				if (name !== 'name') {
					...
					if (radioObj.attr('type') == 'radio') {
						...
						jQuery('#' + name + '_' + itemObj.val()).prop('checked', 'checked');
					}
				}
			});
		}
	...
	function updateNewDCheckType(dcheckId) {
		...
		if (isset(dcheckType, keyRowTypes)) {
			...
			var defkeys = (dcheckType == RDA_SVC.agent)? DEFAULTAGENTCHECKKEY:DEFAULTSNMPCHECKKEY;
			if(isUpdate){
				var defkeyOld = jQuery('#dcheckList input[type="hidden"][name="dchecks['+dcheckId+'][key_]"]').val();
				if(DEFAULTAGENTCHECKKEY!=defkeyOld && DEFAULTSNMPCHECKKEY!=defkeyOld){
					defkeys = defkeyOld;
				}
			}
			jQuery('#newCheckKeyRow input[type="text"]').val(defkeys);
		}
	...
	function saveNewDCheckForm(dcheckId) {
		...
		switch (parseInt(dCheck.type, 10)) {
			...
			case RDA_SVC.snmpv3:
				...
				if (dCheck.snmpv3_securityname == '') {
					validationErrors.push(<%=CJs.encodeJson(Cphp._("Incorrect SNMP securityname."))%>);
				}
				break;
		}
		...
		jQuery.when(jqxhr).done(function() {
			...
			if (validationErrors.length) {
				...
				addPopupValues_discovery([dCheck]);
	...
	function selectUniquenessCriteriaDefault() {
		jQuery('#uniqueness_criteria_ip').prop('checked', 'checked');
	}

	jQuery(document).ready(function() {
		addPopupValues_discovery(<%=JsUtil.rda_jssvalue(Cphp.array_values(Nest.value(data,"drule","dchecks").asCArray()))%>);

		jQuery("input:radio[name='uniqueness_criteria'][value=<%=JsUtil.rda_jsvalue(Nest.value(data,"drule","uniqueness_criteria").$())%>]").prop('checked', 'checked');
