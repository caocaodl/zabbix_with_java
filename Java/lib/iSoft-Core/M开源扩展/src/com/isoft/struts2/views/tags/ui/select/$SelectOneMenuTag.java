...
public class $SelectOneMenuTag extends AndurilUITag{
	...
	public Component getBean(ValueStack stack, HttpServletRequest req,
		...
		return new $SelectOneMenu(stack, req, res);
	}
	
    @Override
    protected void setProperties(AndurilComponent component) {
        super.setProperties(component);
        $SelectOneMenu tagBean = ($SelectOneMenu)component;
        if(this.name != null && this.name.length()>0){
        	tagBean.setName(this.getPropertyStringValue(this.name));
        }
        if(this._disabled){
