	...
	if (typeof(menu_options) != 'undefined') {
		show_popup_menu(evnt,
			[
				...
				[tname, "javascript: openWinCentered('tr_logform.action?sform=1&itemid=" + id + "', 'TriggerLog', 760, 540, 'titlebar=no, resizable=yes, scrollbars=yes, dialog=no');", {'outer' : ['pum_o_item'], 'inner' : ['pum_i_item']}],
				menu_options
			], 240);
	}
	else {
		show_popup_menu(evnt,
			[
				[name, null, null, {'outer' : ['pum_oheader'], 'inner' : ['pum_iheader']}],
				[tname, "javascript: openWinCentered('tr_logform.action?sform=1&itemid=" + id + "', 'ServiceForm', 760, 540, 'titlebar=no, resizable=yes, scrollbars=yes, dialog=no');", {'outer' : ['pum_o_item'], 'inner' : ['pum_i_item']}]
			], 140);
	}
	return false;
}
...
function create_mon_trigger_menu(e, args, items) {
	var tr_menu = [[t('Triggers'), null, null, {'outer' : ['pum_oheader'], 'inner' : ['pum_iheader']}], [t('Events'), 'events.action?triggerid=' + args[0].triggerid + '&nav_time=' + args[0].lastchange + '&source=0', null]];
