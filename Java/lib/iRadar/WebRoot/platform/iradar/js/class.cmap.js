...
IRADAR.namespace = function(namespace) {
...
IRADAR.namespace('classes.Observer');

IRADAR.classes.Observer = (function() {
	var Observer = function() {
		this.listeners = {};
	};

	Observer.prototype = {
		constructor: IRADAR.classes.Observer,
...
IRADAR.namespace('apps.map');

IRADAR.apps.map = (function($) {
	// dependencies
	var Observer = IRADAR.classes.Observer;
	...
	function createMap(containerId, mapData) {
		...
		CMap.prototype = {
			...
			updateImage: function() {
				var url = new Curl(),
					urlText = 'map.action?sid=' + url.getArgument('sid'),
					ajaxRequest;
				...
				ajaxRequest = $.ajax({
					...
					success: $.proxy(function(data) {
						if (this.base64image) {
							...
							this.mapimg.attr('src', 'imgstore.action?imageid=' + data.result);
						}
...
/**
 * Function that is executed by popup.action to ass selected values to destination.
 * It uses a sysmap global variable that created in sysmap.action file via 'var sysmap = IRADAR.apps.map.run();'
 *
...
function addPopupValues(list) {
	if (list.object === 'linktrigger') {
		IRADAR.apps.map.object.linkForm.addNewTriggers(list.values);
	}
}
