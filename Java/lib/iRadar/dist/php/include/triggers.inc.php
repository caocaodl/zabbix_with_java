...
function getSeverityCaption($severity = null) {
	...
	$severities = CArray.array(
		TRIGGER_SEVERITY_NOT_CLASSIFIED => _(Nest.value($config,"severity_name_0").$()),
		TRIGGER_SEVERITY_INFORMATION => _(Nest.value($config,"severity_name_1").$()),
		TRIGGER_SEVERITY_WARNING => _(Nest.value($config,"severity_name_2").$()),
		TRIGGER_SEVERITY_AVERAGE => _(Nest.value($config,"severity_name_3").$()),
		TRIGGER_SEVERITY_HIGH => _(Nest.value($config,"severity_name_4").$()),
		TRIGGER_SEVERITY_DISASTER => _(Nest.value($config,"severity_name_5").$())
	);
...
function getSeverityColor($severity, $value = TRIGGER_VALUE_TRUE) {
	...
	switch ($severity) {
		case TRIGGER_SEVERITY_DISASTER:
			$color = Nest.value($config,"severity_color_5").$();
			break;
		case TRIGGER_SEVERITY_HIGH:
			$color = Nest.value($config,"severity_color_4").$();
			break;
		case TRIGGER_SEVERITY_AVERAGE:
			$color = Nest.value($config,"severity_color_3").$();
			break;
		case TRIGGER_SEVERITY_WARNING:
			$color = Nest.value($config,"severity_color_2").$();
			break;
		case TRIGGER_SEVERITY_INFORMATION:
			$color = Nest.value($config,"severity_color_1").$();
			break;
		case TRIGGER_SEVERITY_NOT_CLASSIFIED:
			$color = Nest.value($config,"severity_color_0").$();
			break;
		default:
			$color = Nest.value($config,"severity_color_0").$();
	}
...
function get_service_status_of_trigger($triggerid) {
	...
	return !empty(Nest.value($rows,"priority").$()) ? Nest.value($rows,"priority").$() : 0;
}
...
function addTriggerValueStyle($object, $triggerValue, $triggerLastChange, $isAcknowledged) {
	...
	if ($triggerValue == TRIGGER_VALUE_TRUE && !$isAcknowledged) {
		$color = Nest.value($config,"problem_unack_color").$();
		$blinks = Nest.value($config,"problem_unack_style").$();
	}
	elseif ($triggerValue == TRIGGER_VALUE_TRUE && $isAcknowledged) {
		$color = Nest.value($config,"problem_ack_color").$();
		$blinks = Nest.value($config,"problem_ack_style").$();
	}
	elseif ($triggerValue == TRIGGER_VALUE_FALSE && !$isAcknowledged) {
		$color = Nest.value($config,"ok_unack_color").$();
		$blinks = Nest.value($config,"ok_unack_style").$();
	}
	elseif ($triggerValue == TRIGGER_VALUE_FALSE && $isAcknowledged) {
		$color = Nest.value($config,"ok_ack_color").$();
		$blinks = Nest.value($config,"ok_ack_style").$();
	}
	if (isset($color) && isset($blinks)) {
		...
		if ($blinks && $timeSinceLastChange < Nest.value($config,"blink_period").$()) {
			$object.addClass("blink"); // elements with this class will blink
			$object.setAttribute("data-time-to-blink", Nest.value($config,"blink_period").$() - $timeSinceLastChange);
		}
	}
	else {
...
function getParentHostsByTriggers($triggers) {
	$hosts = CArray.array();
	$triggerParent = CArray.array();

	while (!empty($triggers)) {
		for($triggers as $tnum => $trigger) {
			if (Nest.value($trigger,"templateid").$() == 0) {
				if (isset($triggerParent[$trigger["triggerid"]])) {
					for($triggerParent[$trigger["triggerid"]] as $triggerid => $state) {
						$hosts[$triggerid] = Nest.value($trigger,"hosts").$();
					}
				}
				else {
					$hosts[$trigger["triggerid"]] = Nest.value($trigger,"hosts").$();
				}
				unset($triggers[$tnum]);
			}
			else {
				if (isset($triggerParent[$trigger["triggerid"]])) {
					if (!isset($triggerParent[$trigger["templateid"]])) {
						$triggerParent[$trigger["templateid"]] = CArray.array();
					}
		...
		$triggers = API.Trigger().get(CArray.array(
			...
			"selectHosts" => CArray.array("hostid", "host", "name", "status"),
			"output" => CArray.array("triggerid", "templateid"),
...
function get_hosts_by_triggerid($triggerids) {
	zbx_value2CArray.array($triggerids);
...
function copyTriggersToHosts($srcTriggerIds, $dstHostIds, $srcHostId = null) {
	$options = CArray.array(
		...
		"output" => CArray.array("triggerid", "expression", "description", "url", "status", "priority", "comments", "type"),
		"filter" => CArray.array("flags" => ZBX_FLAG_DISCOVERY_NORMAL),
		"selectDependencies" => API_OUTPUT_REFER
	);
	if ($srcHostId) {
		$srcHost = API.Host().get(CArray.array(
	...
	// if no $srcHostId provided we will need trigger host "host"
		...
		Nest.value($options,"selectHosts").$() = CArray.array("host");
	}
	$dbSrcTriggers = API.Trigger().get($options);

	$dbDstHosts = API.Host().get(CArray.array(
	...
	for($dbDstHosts as $dstHost) {
		for($dbSrcTriggers as $srcTrigger) {
			...
			// if $srcHostId not provided, use source trigger first host "host"
				...
				if (count(Nest.value($srcTrigger,"hosts").$()) > 1) {
					error(_s("Cannot copy trigger \"%1$s:%2$s\", because it has multiple hosts in the expression.",
						Nest.value($srcTrigger,"description").$(), explode_exp(Nest.value($srcTrigger,"expression").$())));
					return false;
				}
				$host = Nest.array($srcTrigger,"hosts").get(0)["host"];
				$srcTriggerContextHostId = Nest.array($srcTrigger,"hosts").get(0)["hostid"];
			}
			// get expression for the new trigger to be added
			Nest.value($srcTrigger,"expression").$() = explode_exp(Nest.value($srcTrigger,"expression").$(), false, false, $host, Nest.value($dstHost,"host").$());

			// the dependencies must be added after all triggers are created
			unset(Nest.value($srcTrigger,"dependencies").$());

			unset(Nest.value($srcTrigger,"templateid").$());

			if (!$result = API.Trigger().create($srcTrigger)) {
				return false;
			}

			$newTriggers[$srcTrigger["triggerid"]] = CArray.array(
				"newTriggerId" =>reset(Nest.value($result,"triggerids").$()),
	...
	$depIds = CArray.array();
	for($dbSrcTriggers as $srcTrigger) {
		if (Nest.value($srcTrigger,"dependencies").$()) {
			for(Nest.value($srcTrigger,"dependencies").$() as $depTrigger) {
				$depIds[] = Nest.value($depTrigger,"triggerid").$();
			}
		}
	}
	$depTriggers = API.Trigger().get(CArray.array(
		"triggerids" => $depIds,
			...
			if (Nest.value($srcTrigger,"dependencies").$()) {
				...
				for(Nest.value($srcTrigger,"dependencies").$() as $depTrigger) {
					...
					if (isset($newTriggers[$depTrigger["triggerid"]])) {
						...
						if (Nest.value($newTrigger,"srcTriggerContextHostId").$() == $newTriggers[$depTrigger["triggerid"]]["srcTriggerContextHostId"]) {
					...
					elseif (in_CArray.array(CArray.array("hostid"=>Nest.value($newTrigger,"srcTriggerContextHostId").$()), $depTriggers[$depTrigger["triggerid"]]["hosts"])) {
						// get all possible $depTrigger matching triggers by description
						$targetHostTriggersByDescription = API.Trigger().get(CArray.array(
							"hostids" => Nest.value($newTrigger,"newTriggerHostId").$(),
							"output" => CArray.array("hosts", "triggerid", "expression"),
							"filter" => CArray.array("description" => $depTriggers[$depTrigger["triggerid"]]["description"]),
							"preservekeys" => true
						));
						...
						$depTriggerId = null;
						for($targetHostTriggersByDescription as $potentialTargetTrigger) {
							$expr2 = explode_exp(Nest.value($potentialTargetTrigger,"expression").$(), false, false, Nest.value($newTrigger,"newTriggerHost").$(), Nest.value($newTrigger,"srcTriggerContextHost").$());
							if ($expr2 == $expr1) {
						...
						if (is_null($depTriggerId)) {
							$expr1 = explode_exp(Nest.value($srcTrigger,"expression").$(), false, false, Nest.value($newTrigger,"srcTriggerContextHost").$(), Nest.value($newTrigger,"newTriggerHost").$());
							$expr2 = explode_exp($depTriggers[$depTrigger["triggerid"]]["expression"], false, false, Nest.value($newTrigger,"srcTriggerContextHost").$(), Nest.value($newTrigger,"newTriggerHost").$());
...
function construct_expression($itemid, $expressions) {
	...
	$functions = CArray.array("regexp" => 1, "iregexp" => 1);
	$expr_array = CArray.array();
	$cexpor = 0;
	$startpos = -1;

	for($expressions as $expression) {
		Nest.value($expression,"value").$() = preg_replace("/\s+(AND){1,2}\s+/U", "&", Nest.value($expression,"value").$());
		Nest.value($expression,"value").$() = preg_replace("/\s+(OR){1,2}\s+/U", "|", Nest.value($expression,"value").$());

		if (Nest.value($expression,"type").$() == REGEXP_INCLUDE) {
		...
		$expr = "&".Nest.value($expression,"value").$();
...
function explode_exp($expressionCompressed, $html = false, $resolveMacro = false, $sourceHost = null, $destinationHost = null) {
	$expressionExpanded = $html ? CArray.array() : "";
	$trigger = CArray.array();
	...
	for ($i = 0, $state = "", $max = zbx_strlen($expressionCompressed); $i < $max; $i++) {
		...
		elseif ($expressionCompressed[$i] == "}") {
			if ($state == "USERMACRO") {
				...
				if ($resolveMacro) {
					Nest.value($functionData,"expression").$() = $userMacro;
			...
			if (is_numeric($functionId)) {
				...
				if ($functionData) {
					...
					if ($resolveMacro) {
						...
						$items = CMacrosResolverHelper::resolveItemKeys(CArray.array($functionData));
						$item = reset($items);

						Nest.value($functionData,"key_").$() = Nest.value($item,"key_expanded").$();

						// expand macros in function parameter
						$functionParameters = CMacrosResolverHelper::resolveFunctionParameters(CArray.array($functionData));
						$functionParameter = reset($functionParameters);
						Nest.value($functionData,"parameter").$() = Nest.value($functionParameter,"parameter_expanded").$();
					}

					if ($sourceHost !== null && $destinationHost !== null && $sourceHost === Nest.value($functionData,"host").$()) {
						Nest.value($functionData,"host").$() = $destinationHost;
					}

					if ($html) {
						if (Nest.value($functionData,"status").$() == ITEM_STATUS_DISABLED) {
							$style = "disabled";
						}
						elseif (Nest.value($functionData,"status").$() == ITEM_STATUS_ACTIVE) {
							$style = "enabled";
						}
						else {
							$style = "unknown";
						...
						if (Nest.value($functionData,"flags").$() == ZBX_FLAG_DISCOVERY_CREATED || Nest.value($functionData,"type").$() == ITEM_TYPE_HTTPTEST) {
							$link = new CSpan($functionData["host"].":".Nest.value($functionData,"key_").$(), $style);
						}
						elseif (Nest.value($functionData,"flags").$() == ZBX_FLAG_DISCOVERY_PROTOTYPE) {
							$link = new CLink(
								$functionData["host"].":".Nest.value($functionData,"key_").$(),
								"disc_prototypes.php?form=update&itemid=".$functionData["itemid"]."&parent_discoveryid=".
									$trigger["discoveryRuleid"]."&switch_node=".id2nodeid(Nest.value($functionData,"itemid").$()),
								$style
							);
						}
						else {
							$link = new CLink(
								...
								"items.php?form=update&itemid=".$functionData["itemid"]."&switch_node=".
									id2nodeid(Nest.value($functionData,"itemid").$()),
								$style
							);
						}

						$expressionExpanded[] = CArray.array("{", $link,".", bold($functionData["function"]."("), Nest.value($functionData,"parameter").$(), bold(")"), "}");
					}
					else {
	...
	for ($i = 0, $state = "", $len = strlen($expression); $i < $len; $i++) {
		...
		elseif ($expression[$i] == "}") {
			...
			elseif (is_numeric($functionid) && isset($trigger["functions"][$functionid])) {
				...
				if ($html) {
					$style = (Nest.value($function_data,"status").$() == ITEM_STATUS_DISABLED) ? "disabled" : "unknown";
					if (Nest.value($function_data,"status").$() == ITEM_STATUS_ACTIVE) {
						$style = "enabled";
					}

					if (Nest.value($function_data,"flags").$() == ZBX_FLAG_DISCOVERY_CREATED || Nest.value($function_data,"type").$() == ITEM_TYPE_HTTPTEST) {
						$link = new CSpan($function_data["host"].":".CHtml::encode(Nest.value($function_data,"key_").$()), $style);
					}
					elseif (Nest.value($function_data,"flags").$() == ZBX_FLAG_DISCOVERY_PROTOTYPE) {
						$link = new CLink($function_data["host"].":".CHtml::encode(Nest.value($function_data,"key_").$()),
							"disc_prototypes.php?form=update&itemid=".$function_data["itemid"]."&parent_discoveryid=".
							Nest.value($trigger,"discoveryRuleid").$(), $style);
					}
					else {
						$link = new CLink($function_data["host"].":".CHtml::encode(Nest.value($function_data,"key_").$()),
							"items.php?form=update&itemid=".Nest.value($function_data,"itemid").$(), $style);
					}
					array_push(
						$exp,
						CArray.array(
							...
							CHtml::encode(Nest.value($function_data,"parameter").$()),
							bold(")"),
							"}"
						)
					);
 ...
 */
function implode_exp($expression, $triggerId, &$hostnames = CArray.array()) {
	$expressionData = new CTriggerExpression();
	if (!$expressionData.parse($expression)) {
		throw new Exception($expressionData.error);
	}

	$newFunctions = CArray.array();
	$functions = CArray.array();
	$items = CArray.array();
	$triggerFunctionValidator = new CTriggerFunctionValidator();

	for($expressionData.expressions as $exprPart) {
		...
		if (!isset($items[$exprPart["host"]][$exprPart["item"]])) {
			$result = DBselect(
				...
				" WHERE i.key_=".zbx_dbstr(Nest.value($exprPart,"item").$()).
					" AND ".dbConditionInt("i.flags", CArray.array(ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED, ZBX_FLAG_DISCOVERY_PROTOTYPE)).
					" AND h.host=".zbx_dbstr(Nest.value($exprPart,"host").$()).
					" AND h.hostid=i.hostid".
					andDbNode("i.itemid")
			);
			if ($row = DBfetch($result)) {
				$hostnames[] = Nest.value($row,"name").$();
				$items[$exprPart["host"]][$exprPart["item"]] = CArray.array(
		...
		if (!$triggerFunctionValidator.validate(CArray.array(
	...
	$num = 0;
	for($newFunctions as &$newFunction) {
	...
	do {
		$expression = substr_replace($expression, "{".$newFunctions[$exprPart["expression"]]."}",
				Nest.value($exprPart,"pos").$(), strlen(Nest.value($exprPart,"expression").$()));
	}
...
function getExpressionItems(CTriggerExpression $triggerExpression) {
	$items = CArray.array();
	$processedFunctions = CArray.array();
	$processedItems = CArray.array();

	for($triggerExpression.expressions as $expression) {
		...
		if (!isset($processedItems[$expression["host"]][$expression["item"]])) {
			$dbItems = DBselect(
				...
				" FROM items i,hosts h".
				" WHERE i.key_=".zbx_dbstr(Nest.value($expression,"item").$()).
					" AND ".dbConditionInt("i.flags", CArray.array(
						ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED, ZBX_FLAG_DISCOVERY_PROTOTYPE
					)).
					" AND h.host=".zbx_dbstr(Nest.value($expression,"host").$()).
					" AND h.hostid=i.hostid".
					andDbNode("i.itemid")
			);
...
function check_right_on_trigger_by_expression($permission, $expression) {
	...
	$hosts = API.Host().get(CArray.array(
		"filter" => CArray.array("host" => $expressionHosts),
...
function replace_template_dependencies($deps, $hostid) {
	for($deps as $id => $val) {
		...
		if ($db_new_dep = DBfetch(DBselect($sql))) {
			$deps[$id] = Nest.value($db_new_dep,"triggerid").$();
		}
	}
...
function getTriggersOverview($hostIds, $application, $pageFile, $viewMode = null, $screenId = null) {
	...
	$applicationIds = null;
	if ($application !== "") {
		$dbApplications = API.Application().get(CArray.array(
			"hostids" => $hostIds,
			"filter" => CArray.array("name" => $application),
			"output" => CArray.array("applicationid")
		));
	...
	for($dbTriggers as $trigger) {
		$host = reset(Nest.value($trigger,"hosts").$());

		$hostIds[$host["hostid"]] = Nest.value($host,"hostid").$();
	}

	$hosts = API.Host().get(CArray.array(
		"output" => CArray.array("name", "hostid", "status"),
		"hostids" => $hostIds,
		"preservekeys" => true,
	...
	$hostNames = CArray.array();

	for($dbTriggers as $trigger) {
		$host = reset(Nest.value($trigger,"hosts").$());

		Nest.value($host,"name").$() = get_node_name_by_elid(Nest.value($host,"hostid").$(), null, NAME_DELIMITER).Nest.value($host,"name").$();
		Nest.value($trigger,"description").$() = CMacrosResolverHelper::resolveTriggerReference(Nest.value($trigger,"expression").$(), Nest.value($trigger,"description").$());
		$hostNames[$host["hostid"]] = Nest.value($host,"name").$();

		// a little tricky check for attempt to overwrite active trigger (value=1) with
		// inactive or active trigger with lower priority.
		if (!isset($triggers[$trigger["description"]][$host["name"]])
				|| (($triggers[$trigger["description"]][$host["name"]]["value"] == TRIGGER_VALUE_FALSE && Nest.value($trigger,"value").$() == TRIGGER_VALUE_TRUE)
					|| (($triggers[$trigger["description"]][$host["name"]]["value"] == TRIGGER_VALUE_FALSE || Nest.value($trigger,"value").$() == TRIGGER_VALUE_TRUE)
						&& Nest.value($trigger,"priority").$() > $triggers[$trigger["description"]][$host["name"]]["priority"]))) {
			$triggers[$trigger["description"]][$host["name"]] = CArray.array(
	...
	if ($viewMode == STYLE_TOP) {
		// header
		$header = CArray.array(new CCol(_("Triggers"), "center"));

		for($hostNames as $hostName) {
		...
		// data
		for($triggers as $description => $triggerHosts) {
			$columns = CArray.array(nbsp($description));
		...
		// header
		$header = CArray.array(new CCol(_("Host"), "center"));
...
function getTriggerOverviewCells($trigger, $pageFile, $screenId = null) {
	$ack = $css = $style = null;
	$desc = $menuPopup = $triggerItems = $acknowledge = CArray.array();
	...
	if ($trigger) {
		...
		if (Nest.value($trigger,"value").$() == TRIGGER_VALUE_TRUE) {
			$css = getSeverityStyle(Nest.value($trigger,"priority").$());
			$ack = null;

			if (Nest.value($config,"event_ack_enable").$() == 1) {
				if ($event = get_last_event_by_triggerid(Nest.value($trigger,"triggerid").$())) {
					if ($screenId) {
						$acknowledge = CArray.array(
					...
					if (Nest.value($event,"acknowledged").$() == 1) {
		...
		$dbItems = DBfetchArray(DBselect(
			...
			" WHERE f.itemid=i.itemid".
				" AND f.triggerid=".zbx_dbstr(Nest.value($trigger,"triggerid").$())
		));

		$dbItems = CMacrosResolverHelper::resolveItemNames($dbItems);

		for($dbItems as $dbItem) {
			$triggerItems[] = CArray.array(
				"name" => Nest.value($dbItem,"name_expanded").$(),
				"params" => CArray.array(
					"action" => in_CArray.array(Nest.value($dbItem,"value_type").$(), CArray.array(ITEM_VALUE_TYPE_FLOAT, ITEM_VALUE_TYPE_UINT64))
		...
		$triggerId = empty(Nest.value($trigger,"triggerid").$()) ? 0 : Nest.value($trigger,"triggerid").$();
		...
		while ($dbDependency = DBfetch($dbDependencies)) {
			$dependencyTable.addRow(SPACE."-".SPACE.CMacrosResolverHelper::resolveTriggerNameById(Nest.value($dbDependency,"triggerid_up").$()));
		...
		while ($dbDependency = DBfetch($dbDependencies)) {
			$dependencyTable.addRow(SPACE."-".SPACE.CMacrosResolverHelper::resolveTriggerNameById(Nest.value($dbDependency,"triggerid_down").$()));
			$isDependencyFound = true;
		}
	...
	$column = ((is_array($desc) && count($desc) > 0) || $ack)
		? new CCol(CArray.array($desc, $ack), $css." hosts")
		: new CCol(SPACE, $css." hosts");

	$column.setAttribute("style", $style);

	if ($trigger && Nest.value($config,"blink_period").$() > 0 && time() - Nest.value($trigger,"lastchange").$() < Nest.value($config,"blink_period").$()) {
...
function calculate_availability($triggerid, $period_start, $period_end) {
	...
	if ($period_start > 0 && $period_start <= time()) {
		...
		if ($row = DBfetch(DBselect($sql, 1))) {
			$start_value = Nest.value($row,"value").$();
			$min = $period_start;
		}
	}
	...
	if (Nest.value($db_events,"cnt").$() > 0) {
		if (!isset($min)) {
			$min = Nest.value($db_events,"min_clock").$();
		}
		$max = Nest.value($db_events,"max_clock").$();
	}
	else {
		if ($period_start == 0 && $period_end == 0) {
			...
			Nest.value($ret,"true_time").$() = 0;
			Nest.value($ret,"false_time").$() = 0;
			Nest.value($ret,"true").$() = (TRIGGER_VALUE_TRUE == $start_value) ? 100 : 0;
			Nest.value($ret,"false").$() = (TRIGGER_VALUE_FALSE == $start_value) ? 100 : 0;
			return $ret;
		}
	}
	...
	if ($rows == 0) {
		...
		$state = Nest.value($trigger,"value").$();
	}
	...
	if ($total_time == 0) {
		Nest.value($ret,"true_time").$() = 0;
		Nest.value($ret,"false_time").$() = 0;
		Nest.value($ret,"true").$() = 0;
		Nest.value($ret,"false").$() = 0;
	}
	else {
		Nest.value($ret,"true_time").$() = $true_time;
		Nest.value($ret,"false_time").$() = $false_time;
		Nest.value($ret,"true").$() = (100 * $true_time) / $total_time;
		Nest.value($ret,"false").$() = (100 * $false_time) / $total_time;
	}
...
function get_triggers_unacknowledged($db_element, $count_problems = null, $ack = false) {
	...
	if ($ack) {
		Nest.value($options,"withAcknowledgedEvents").$() = 1;
	}
	else {
		Nest.value($options,"withUnacknowledgedEvents").$() = 1;
	}

	if ($count_problems) {
		Nest.value($options,"filter","value").$() = TRIGGER_VALUE_TRUE;
	}
	if (!empty(Nest.value($elements,"hosts_groups").$())) {
		Nest.value($options,"groupids").$() = array_unique(Nest.value($elements,"hosts_groups").$());
	}
	if (!empty(Nest.value($elements,"hosts").$())) {
		Nest.value($options,"hostids").$() = array_unique(Nest.value($elements,"hosts").$());
	}
	if (!empty(Nest.value($elements,"triggers").$())) {
		Nest.value($options,"triggerids").$() = array_unique(Nest.value($elements,"triggers").$());
	}
...
function make_trigger_details($trigger) {
	$hosts = reset(Nest.value($trigger,"hosts").$());
	$hostId = Nest.value($hosts,"hostid").$();
	...
	if (is_show_all_nodes()) {
		$table.addRow(CArray.array(_("Node"), get_node_name_by_elid(Nest.value($trigger,"triggerid").$())));
	}

	$table.addRow(CArray.array(_("Host"), $hostName));
	$table.addRow(CArray.array(_("Trigger"), CMacrosResolverHelper::resolveTriggerName($trigger)));
	$table.addRow(CArray.array(_("Severity"), getSeverityCell(Nest.value($trigger,"priority").$())));
	$table.addRow(CArray.array(_("Expression"), explode_exp(Nest.value($trigger,"expression").$(), true, true)));
	$table.addRow(CArray.array(_("Event generation"), _("Normal").((TRIGGER_MULT_EVENT_ENABLED == Nest.value($trigger,"type").$())
		? SPACE."+".SPACE._("Multiple PROBLEM events") : "")));
	$table.addRow(CArray.array(_("Disabled"), ((TRIGGER_STATUS_ENABLED == Nest.value($trigger,"status").$())
...
function buildExpressionHtmlTree(array $expressionTree, array &$next, &$letterNum, $level = 0, $operand = null) {
	$treeList = CArray.array();
	$outline = "";

	end($expressionTree);
	$lastKey = key($expressionTree);

	for($expressionTree as $key => $element) {
		switch (Nest.value($element,"type").$()) {
			case "operand":
				$next[$level] = ($key != $lastKey);
				$expr = expressionLevelDraw($next, $level);
				$expr[] = SPACE;
				$expr[] = italic(Nest.value($element,"operand").$() == "&" ? _("AND") : _("OR"));
				$levelDetails = CArray.array(
					"list" => $expr,
					"id" => Nest.value($element,"id").$(),
					"expression" => CArray.array(
						"value" => $element["expression"]
					)
				);

				$levelErrors = expressionHighLevelErrors(Nest.value($element,"expression").$());
				if (count($levelErrors) > 0) {
					Nest.value($levelDetails,"expression","levelErrors").$() = $levelErrors;
				}
				$treeList[] = $levelDetails;

				list($subOutline, $subTreeList) = buildExpressionHtmlTree(Nest.value($element,"elements").$(), $next, $letterNum,
						$level + 1, Nest.value($element,"operand").$());
			...
			case "expression":
				...
				if (defined("NO_LINK_IN_TESTING")) {
					...
					$expressionId = "expr_".Nest.value($element,"id").$();
	...
	return CArray.array($outline, $treeList);
}
...
function expressionHighLevelErrors($expression) {
	...
	if (!isset($errors)) {
		$definedErrorPhrases = CArray.array(
	...
	if (!isset($errors[$expression])) {
		$errors[$expression] = CArray.array();
		$expressionData = new CTriggerExpression();
		if ($expressionData.parse($expression)) {
			for($expressionData.expressions as $exprPart) {
				$info = get_item_function_info(Nest.value($exprPart,"expression").$());
...
function getExpressionElementsNum(CTriggerExpression $expressionData, $start, $end) {
	for ($i = $start, $level = 0, $expressionElementsNum = 1; $i <= $end; $i++) {
		switch ($expressionData.expression[$i]) {
			...
			case "{":
				for($expressionData.expressions as $exprPart) {
					if (Nest.value($exprPart,"pos").$() == $i) {
						$i += strlen(Nest.value($exprPart,"expression").$()) - 1;
						break;
					}
				}
				break;
...
function getExpressionTree(CTriggerExpression $expressionData, $start, $end) {
	$expressionTree = CArray.array();

	for(CArray.array("|", "&") as $operand) {
		$operandFound = false;
		$lParentheses = -1;
		$rParentheses = -1;
...
function rebuildExpressionTree(array &$expressionTree, $expressionId, $action, $newExpression, $operand = null) {
	for($expressionTree as $key => $expression) {
					...
					switch ($expressionTree[$key]["type"]) {
						case "operand":
							if ($expressionTree[$key]["operand"] == $action) {
								$expressionTree[$key]["elements"][] = CArray.array(
									"expression" => $newExpression,
									"type" => "expression"
								);
							}
							else {
								$element = CArray.array(
									"type" => "operand",
									"operand" => $action,
									"elements" => CArray.array(
										$expressionTree[$key],
...
function makeExpression(array $expressionTree, $level = 0, $operand = null) {
	...
	for($expressionTree as $key => $element) {
		switch (Nest.value($element,"type").$()) {
			case "operand":
				$subExpression = makeExpression(Nest.value($element,"elements").$(), $level + 1, Nest.value($element,"operand").$());
...
function get_item_function_info($expr) {
	...
	$type_of_value_type = CArray.array(
	...
	$function_info = CArray.array(
		"band" =>		CArray.array("value_type" => _("Numeric (integer 64bit)"),	"type" => T_ZBX_INT, "validation" => NOT_EMPTY),
		"abschange" =>	CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"avg" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"change" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"count" =>		CArray.array("value_type" => _("Numeric (integer 64bit)"), "type" => T_ZBX_INT, "validation" => NOT_EMPTY),
		"date" =>		CArray.array("value_type" => "YYYYMMDD",	"type" => T_ZBX_INT,			"validation" => "{}>=19700101&&{}<=99991231"),
		"dayofmonth" =>	CArray.array("value_type" => "1-31",		"type" => T_ZBX_INT,			"validation" => "{}>=1&&{}<=31"),
		"dayofweek" =>	CArray.array("value_type" => "1-7",		"type" => T_ZBX_INT,			"validation" => IN("1,2,3,4,5,6,7")),
		"delta" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"diff" =>		CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"fuzzytime" =>	CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"iregexp" =>	CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"last" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"logeventid" =>	CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"logseverity" =>CArray.array("value_type" => _("Numeric (integer 64bit)"), "type" => T_ZBX_INT, "validation" => NOT_EMPTY),
		"logsource" =>	CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"max" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"min" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"nodata" =>		CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"now" =>		CArray.array("value_type" => _("Numeric (integer 64bit)"), "type" => T_ZBX_INT, "validation" => NOT_EMPTY),
		"prev" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"regexp" =>		CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"str" =>		CArray.array("value_type" => _("0 or 1"),	"type" => T_ZBX_INT,			"validation" => IN("0,1")),
		"strlen" =>		CArray.array("value_type" => _("Numeric (integer 64bit)"), "type" => T_ZBX_INT, "validation" => NOT_EMPTY),
		"sum" =>		CArray.array("value_type" => $value_type,	"type" => $type_of_value_type,	"validation" => NOT_EMPTY),
		"time" =>		CArray.array("value_type" => "HHMMSS",		"type" => T_ZBX_INT,			"validation" => "zbx_strlen({})==6")
	);
	...
	if ($expressionData.parse($expr)) {
		...
		elseif (isset($expressionData.expressions[0])) {
			...
			$hostFound = API.Host().get(CArray.array(
				"filter" => CArray.array("host" => CArray.array(Nest.value($exprPart,"host").$())),
				"templated_hosts" => true
			));
			...
			$itemFound = API.Item().get(CArray.array(
				"output" => CArray.array("value_type"),
				"hostids" => zbx_objectValues($hostFound, "hostid"),
				"filter" => CArray.array(
					"key_" => CArray.array(Nest.value($exprPart,"item").$())
				),
				"webitems" => true
			));

			if (!$itemFound) {
				$itemFound = API.ItemPrototype().get(CArray.array(
					"output" => CArray.array("value_type"),
					"hostids" => zbx_objectValues($hostFound, "hostid"),
					"filter" => CArray.array(
						"key_" => CArray.array(Nest.value($exprPart,"item").$())
			...
			if (is_array(Nest.value($result,"value_type").$())) {
				Nest.value($result,"value_type").$() = $result["value_type"][$itemFound["value_type"]];
				Nest.value($result,"type").$() = $result["type"][$itemFound["value_type"]];

				if (Nest.value($result,"type").$() == T_ZBX_INT) {
					Nest.value($result,"type").$() = T_ZBX_STR;
					Nest.value($result,"validation").$() = "preg_match(\"/^".ZBX_PREG_NUMBER."$/u\",{})";
				}
			}
		}
		else {
...
function evalExpressionData($expression, $rplcts) {
	...
	if (!empty($ZBX_MESSAGES)) {
		$messageList = CArray.array();

		for($ZBX_MESSAGES as $zbxMessage) {
			if (strpos(Nest.value($zbxMessage,"message").$(), "eval()") !== false) {
				$error = substr(Nest.value($zbxMessage,"message").$(), 0, strpos(Nest.value($zbxMessage,"message").$(), "["));
				$result = "NULL";
			}
			else {
...
function resolveTriggerUrl($trigger) {
	return str_replace("{TRIGGER.ID}", Nest.value($trigger,"triggerid").$(), Nest.value($trigger,"url").$());
}
...
function convert($value) {
	...
	$value = Nest.value($arr,"value").$();
	switch (Nest.value($arr,"mult").$()) {
		case "T":
...
function orderTriggersByStatus(array &$triggers, $sortorder = ZBX_SORT_UP) {
	...
	for($triggers as $key => $trigger) {
		if (Nest.value($trigger,"status").$() == TRIGGER_STATUS_ENABLED) {
			$statusOrder = (Nest.value($trigger,"state").$() == TRIGGER_STATE_UNKNOWN) ? 2 : 0;
		}
		elseif (Nest.value($trigger,"status").$() == TRIGGER_STATUS_DISABLED) {
			$statusOrder = 1;
		}
