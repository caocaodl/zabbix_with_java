...
class CArrayHelper {
	...
	public static function sort(array &$array, array $fields) {
		for($fields as $fid => $field) {
			if (!is_array($field)) {
				$fields[$fid] = CArray.array("field" => $field, "order" => ZBX_SORT_UP);
			}
		}
		self::$fields = $fields;
		uasort($array, CArray.array("self", "compare"));
	}
	...
	protected static function compare($a, $b) {
		for(self::$fields as $field) {
			...
			if ($cmp != 0) {
				return $cmp * (Nest.value($field,"order").$() == ZBX_SORT_UP?1:-1);
			}
		}
		return 0;
	}
	 ...
	 * @return array
	 */
	public static function unsetEqualValues(array $a1, array $a2, array $skipKeys = CArray.array()) {
		// ignore given fields
		for($skipKeys as $key) {
			unset($a2[$key]);
		}
	 ...
	 * Example 1:
	 * $data = CArray.array(
	 *     CArray.array("name" => "CPU load"),
	 * 	   CArray.array("name" => "CPU load"),
	 * 	   CArray.array("name" => "Free memory")
	 * );
	 * var_dump(CArrayHelper::findDuplicate($data, "name")); // returns array with index 1
	 *
	 * Example 2:
	 * $data = CArray.array(
	 *     CArray.array("host" => "Zabbix server", "name" => "CPU load"),
	 * 	   CArray.array("host" => "Zabbix server", "name" => "Free memory"),
	 * 	   CArray.array("host" => "Linux server", "name" => "CPU load"),
	 * 	   CArray.array("host" => "Zabbix server", "name" => "CPU load")
	 * );
