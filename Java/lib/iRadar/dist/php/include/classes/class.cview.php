...
class CView {
	 ...
	 */
	public function __construct($view, $data = CArray.array()) {
		assign($view);
		data = $data;
	}
	...
	public function assign($view) {
		...
		filePath = self::viewsDir."/".$view.".php";

		if (!file_exists(filePath)) {
			throw new Exception(_s("File provided to a view does not exist. Tried to find \"%s\".", filePath));
		}
	}
	...
	public function getArray($var) {
		return isset(data[$var]) && is_array(data[$var]) ? data[$var] : CArray.array();
	}
	...
	public function render() {
		...
		if (template === false) {
			throw new Exception(_s("Cannot include view file \"%s\".", filePath));
		}
