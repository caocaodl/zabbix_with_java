...
class CMacrosResolverGeneral {
	 ...
	 * @var array
	 */
	protected $interfacePriorities = CArray.array(
	...
	protected function getFunctionMacroValues(array $macroValues, array $fNums, $triggerId, $macro, $replace) {
		for($fNums as $fNum) {
			$macroValues[$triggerId][getFunctionMacroName($macro, $fNum)] = $replace;
		}
	...
	protected function getItemValueMacro($lastValue, array $item, array $trigger) {
		if (config == "eventDescription") {
			$value = item_get_history($item, Nest.value($trigger,"clock").$(), Nest.value($trigger,"ns").$());

			return ($value === null) ? UNRESOLVED_MACRO_STRING : formatHistoryValue($value, $item);
		}
		else {
			return getItemLastValueMacro($lastValue, $item);
		}
	}
		...
		if ($macros) {
			...
			while ($dbInterface = DBfetch($dbInterfaces)) {
				if (isset($interfaces[$dbInterface["functionid"]])
						&& interfacePriorities[$interfaces[$dbInterface["functionid"]]["type"]] > interfacePriorities[$dbInterface["type"]]) {
					continue;
				}
			...
			for($interfaces as $interface) {
				for($macros[$interface["functionid"]] as $macro => $fNums) {
					switch ($macro) {
						case "IPADDRESS":
						case "HOST.IP":
							$replace = Nest.value($interface,"ip").$();
							break;
						case "HOST.DNS":
							$replace = Nest.value($interface,"dns").$();
							break;
						case "HOST.CONN":
							$replace = Nest.value($interface,"useip").$() ? Nest.value($interface,"ip").$() : Nest.value($interface,"dns").$();
							break;
						case "HOST.PORT":
							$replace = Nest.value($interface,"port").$();
							break;
					}

					$macroValues = getFunctionMacroValues($macroValues, $fNums, Nest.value($interface,"triggerid").$(), $macro, $replace);
				}
			}
		}
	...
	protected function getItemMacros(array $macros, array $triggers, array $macroValues) {
		if ($macros) {
			...
			for($functions as $func) {
				for($macros[$func["functionid"]] as $macro => $fNums) {
					$lastValue = isset($history[$func["itemid"]]) ? $history[$func["itemid"]][0]["value"] : null;

					switch ($macro) {
						case "ITEM.LASTVALUE":
							$replace = getItemLastValueMacro($lastValue, $func);
							break;
						case "ITEM.VALUE":
							$replace = getItemValueMacro($lastValue, $func, $triggers[$func["triggerid"]]);
							break;
					}

					$macroValues = getFunctionMacroValues($macroValues, $fNums, Nest.value($func,"triggerid").$(), $macro, $replace);
				}
			}
		}
			...
			while ($func = DBfetch($dbFuncs)) {
				for($macros[$func["functionid"]] as $macro => $fNums) {
					switch ($macro) {
						case "HOSTNAME":
						case "HOST.HOST":
							$replace = Nest.value($func,"host").$();
							break;
						case "HOST.NAME":
							$replace = Nest.value($func,"name").$();
							break;
					}
	...
	protected function isTypeAvailable($type) {
		return in_CArray.array($type, configs[config]["types"]);
	}
	...
	protected function getSource() {
		return configs[config]["source"];
	}

	/**
	 * Get macros with values.
	 *
	 * @param array $data			Macros to resolve (CArray.array(hostids => CArray.array(hostid), macros => CArray.array(macro => null)))
	 *
	 * @return array
	 */
	protected function getUserMacros(array $data) {
		...
		for($data as $element) {
			for(Nest.value($element,"hostids").$() as $hostId) {
		...
		// hostid => CArray.array(templateid)
		$hostTemplates = CArray.array();

		// hostid => CArray.array(macro => value)
		$hostMacros = CArray.array();
		...
		do {
			$dbHosts = API.Host().get(CArray.array(
				...
				"selectParentTemplates" => CArray.array("templateid"),
				"selectMacros" => CArray.array("macro", "value")
			));

			$hostIds = CArray.array();

			if ($dbHosts) {
				for($dbHosts as $dbHost) {
					$hostTemplates[$dbHost["hostid"]] = zbx_objectValues(Nest.value($dbHost,"parentTemplates").$(), "templateid");

					for(Nest.value($dbHost,"macros").$() as $dbMacro) {
						if (!isset($hostMacros[$dbHost["hostid"]])) {
							$hostMacros[$dbHost["hostid"]] = CArray.array();
						}

						$hostMacros[$dbHost["hostid"]][$dbMacro["macro"]] = Nest.value($dbMacro,"value").$();
					}
				}

				for($dbHosts as $dbHost) {
					// only unprocessed templates will be populated
					for($hostTemplates[$dbHost["hostid"]] as $templateId) {
		...
		for($data as &$element) {
			...
			for(Nest.value($element,"macros").$() as $macro => &$value) {
				$value = getHostUserMacros($hostIds, $macro, $hostTemplates, $hostMacros);
		 ...
		 * Global macros
		 */
		$dbGlobalMacros = API.UserMacro().get(CArray.array(
			"output" => CArray.array("macro", "value"),
			"globalmacro" => true
		));
		if ($dbGlobalMacros) {
	 ...
	 * @param array  $hostTemplates	The list of linked templates (hostid => CArray.array(templateid))
	 * @param array  $hostMacros	The list of macros on hosts (hostid => CArray.array(macro => value))
	 *
	 * @return array
	 */
	protected function getHostUserMacros(array $hostIds, $macro, array $hostTemplates, array $hostMacros) {
		for($hostIds as $hostId) {
		...
		$templateIds = CArray.array();

		for($hostIds as $hostId) {
			if (isset($hostTemplates[$hostId])) {
				for($hostTemplates[$hostId] as $templateId) {
		...
		if ($templateIds) {
			...
			return getHostUserMacros($templateIds, $macro, $hostTemplates, $hostMacros);
		}
