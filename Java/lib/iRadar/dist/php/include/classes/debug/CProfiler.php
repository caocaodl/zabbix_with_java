...
class CProfiler {
	 ...
	 * @var array
	 */
	protected $sqlQueryLog = CArray.array();
	...
	public function show() {
		...
		$totalScriptTime = stopTime - startTime;
		$totalTimeStr = _s("Total time: %s", round($totalScriptTime, 6));
		if ($totalTimeStr > slowScriptTime) {
			$totalTimeStr = "<b>".$totalTimeStr."</b>";
		}
		$debug_str .= $totalTimeStr."<br>";

		$sqlTotalTimeStr = _s("Total SQL time: %s", sqlTotalTime);
		if ($sqlTotalTimeStr > slowTotalSqlTime) {
			$sqlTotalTimeStr = "<b>".$sqlTotalTimeStr."</b>";
		}
		$debug_str .= $sqlTotalTimeStr."<br>";

		if (isset($DB) && isset(Nest.value($DB,"SELECT_COUNT").$())) {
			$debug_str .= _s("SQL count: %s (selects: %s | executes: %s)",
				count(sqlQueryLog), Nest.value($DB,"SELECT_COUNT").$(), Nest.value($DB,"EXECUTE_COUNT").$())."<br>";
		}

		$debug_str .= _s("Peak memory usage: %s", mem2str(getMemoryPeak()))."<br>";
		$debug_str .= _s("Memory limit: %s", ini_get("memory_limit"))."<br>";

		$debug_str .= "<br>";

		for(apiLog as $i => $apiCall) {
		...
		for(sqlQueryLog as $query) {
			...
			if ($time > slowSqlQueryTime) {
				$sqlString = "<b>".$sqlString."</b>";
			}
			$debug_str .= $sqlString;

			$callStackString = "<span style=\"font-style: italic;\">".formatCallStack($query[2])."</span>"."<br>"."<br>";
		...
		$debug.addItem(CArray.array(BR(), new CJSscript($debug_str), BR()));
		$debug.show();
	}
	...
	public function profileSql($time, $sql) {
		if (!is_null(CWebUser::$data) && isset(CWebUser::Nest.value($data,"debug_mode").$())
				&& CWebUser::Nest.value($data,"debug_mode").$() == GROUP_DEBUG_MODE_DISABLED) {
			return;
		}
	...
	public function formatCallStack(array $callStack = null) {
		...
		$callStackString = "";
		$callWithFile = CArray.array();

		$callStack = array_reverse($callStack);
		$firstCall = reset($callStack);

		for($callStack as $call) {
			// do not show the call to the error handler function
			if (Nest.value($call,"function").$() != "zbx_err_handler") {
				if (isset(Nest.value($call,"class").$())) {
					$callStackString .= $call["class"].Nest.value($call,"type").$();
				}
			...
			if (isset(Nest.value($call,"file").$())) {
				$callWithFile = $call;
			}
		}

		if ($callStackString) {
			$path = pathinfo(Nest.value($firstCall,"file").$());
			$callStackString = $path["basename"].":".Nest.value($firstCall,"line").$() . " &rarr; ".rtrim($callStackString, "&rarr; ");
		}

		if ($callWithFile) {
			$callStackString .= " in ".$callWithFile["file"].":".Nest.value($callWithFile,"line").$();
		}
