	...
	public function get() {
		...
		if (Nest.value(screenitem,"resourceid").$() > 0) {
			$hosts = API.Host().get(CArray.array(
				"hostids" => Nest.value(screenitem,"resourceid").$(),
			...
			for($hosts as $host) {
				$hostsComboBox.addItem(Nest.value($host,"hostid").$(), get_node_name_by_elid(Nest.value($host,"hostid").$(), null, NAME_DELIMITER).Nest.value($host,"host").$());
			}
			...
			$item.addItem(CArray.array(SPACE._("Host").SPACE, $hostsComboBox));
		}

		$output = new CUIWidget("hat_trstatus", make_latest_issues($params));
		$output.setDoubleHeader(CArray.array(_("HOST ISSUES"), SPACE, zbx_date2str(_("[H:i:s]")), SPACE), $item);
