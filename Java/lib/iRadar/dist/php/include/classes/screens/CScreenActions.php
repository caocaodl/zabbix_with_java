...
class CScreenActions extends CScreenBase {
	...
	public function get() {
		...
		if (CWebUser::getType() != USER_TYPE_SUPER_ADMIN) {
			$userid = CWebUser::Nest.value($data,"userid").$();
		...
		$alerts = DBfetchArray(DBselect($sql, Nest.value(screenitem,"elements").$()));

		order_result($alerts, $sortfield, $sortorder);

		// indicator of sort field
		$sortfieldSpan = new CSpan(CArray.array($sorttitle, SPACE));
		...
		$actionTable.setHeader(CArray.array(
			is_show_all_nodes() ? _("Nodes") : null,
			($sortfield == "clock") ? CArray.array($sortfieldSpan, $sortorderSpan) : _("Time"),
			($sortfield == "description") ? CArray.array($sortfieldSpan, $sortorderSpan) : _("Type"),
			($sortfield == "status") ? CArray.array($sortfieldSpan, $sortorderSpan) : _("Status"),
			($sortfield == "retries") ? CArray.array($sortfieldSpan, $sortorderSpan) : _("Retries left"),
			($sortfield == "sendto") ? CArray.array($sortfieldSpan, $sortorderSpan) : _("Recipient(s)"),
			_("Message"),
			_("Error")
		));
		...
		for($alerts as $alert) {
			...
			$actionTable.addRow(CArray.array(
				get_node_name_by_elid(Nest.value($alert,"alertid").$()),
				new CCol(zbx_date2str(HISTORY_OF_ACTIONS_DATE_FORMAT, Nest.value($alert,"clock").$()), "top"),
