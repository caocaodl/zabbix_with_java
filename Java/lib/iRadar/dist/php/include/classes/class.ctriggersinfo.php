...
class CTriggersInfo extends CTable {
	...
	public function __construct($groupid = null, $hostid = null, $style = STYLE_HORIZONTAL) {
		...
		hostid = is_null($hostid) ? 0 : $hostid;
	}
	...
	public function bodyToString() {
		...
		if (hostid > 0) {
			Nest.value($options,"hostids").$() = hostid;
		}
		elseif (groupid > 0) {
		...
		while ($row = DBfetch($db_priority)) {
			switch (Nest.value($row,"value").$()) {
				case TRIGGER_VALUE_TRUE:
					switch (Nest.value($row,"priority").$()) {
						case TRIGGER_SEVERITY_NOT_CLASSIFIED:
							$uncl += Nest.value($row,"cnt").$();
							break;
						case TRIGGER_SEVERITY_INFORMATION:
							$info += Nest.value($row,"cnt").$();
							break;
						case TRIGGER_SEVERITY_WARNING:
							$warn += Nest.value($row,"cnt").$();
							break;
						case TRIGGER_SEVERITY_AVERAGE:
							$avg += Nest.value($row,"cnt").$();
							break;
						case TRIGGER_SEVERITY_HIGH:
							$high += Nest.value($row,"cnt").$();
							break;
						case TRIGGER_SEVERITY_DISASTER:
							$dis += Nest.value($row,"cnt").$();
							break;
					}
					break;
				case TRIGGER_VALUE_FALSE:
					$ok += Nest.value($row,"cnt").$();
					break;
			}
		}
		...
		if (STYLE_HORIZONTAL == style) {
			addRow(CArray.array($trok, $uncl, $info, $warn, $avg, $high, $dis));
		}
		else {
			addRow($trok);
			addRow($uncl);
