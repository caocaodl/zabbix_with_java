...
class CScreenImporter extends CAbstractScreenImporter {
	...
	public function import(array $screens) {
		...
		do {
			...
			$screensToCreate = CArray.array();
			$screensToUpdate = CArray.array();
			for($independentScreens as $name) {
				$screen = $screens[$name];
				unset($screens[$name]);

				$screen = resolveScreenReferences($screen);

				if ($screenId = referencer->resolveScreen(Nest.value($screen,"name").$())) {
			...
			if (Nest.value(options,"screens","createMissing").$() && $screensToCreate) {
				$newScreenIds = API.Screen().create($screensToCreate);
				for($screensToCreate as $num => $newScreen) {
					$screenidId = $newScreenIds["screenids"][$num];
					referencer->addScreenRef(Nest.value($newScreen,"name").$(), $screenidId);
				}
			}
			if (Nest.value(options,"screens","updateExisting").$() && $screensToUpdate) {
		...
		for($screens as $screen) {
			...
			for(Nest.value($screen,"screenitems").$() as $screenItem) {
				if (Nest.value($screenItem,"resourcetype").$() == SCREEN_RESOURCE_SCREEN
						&& !referencer->resolveScreen(Nest.value($screenItem,"resource","name").$())) {
					$unresolvedReferences[] = Nest.value($screenItem,"resource","name").$();
				}
			}
	...
	protected function checkCircularScreenReferences(array $screens) {
		for($screens as $screenName => $screen) {
			if (empty(Nest.value($screen,"screenitems").$())) {
				continue;
			}

			for(Nest.value($screen,"screenitems").$() as $screenItem) {
				$checked = CArray.array($screenName);
				if ($circScreens = checkCircularRecursive($screenItem, $screens, $checked)) {
	...
	protected function checkCircularRecursive(array $screenItem, array $screens, array $checked) {
		...
		if (!empty($screens[$screenName]["screenitems"])) {
			for($screens[$screenName]["screenitems"] as $sItem) {
	...
	protected function getIndependentScreens(array $screens) {
		for($screens as $num => $screen) {
			...
			for(Nest.value($screen,"screenitems").$() as $screenItem) {
				if (Nest.value($screenItem,"resourcetype").$() == SCREEN_RESOURCE_SCREEN) {
					if (!referencer->resolveScreen(Nest.value($screenItem,"resource","name").$())) {
						unset($screens[$num]);
						continue 2;
					}
				}
