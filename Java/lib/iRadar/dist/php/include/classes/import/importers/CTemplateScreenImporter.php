		...
		for($allScreens as $template => $screens) {
			...
			for($screens as $screen) {
				...
				if (isset(Nest.value($screen,"screenid").$())) {
					...
					Nest.value($screen,"templateid").$() = referencer->resolveTemplate($template);
					$screensToCreate[] = $screen;
				}
			}
		}

		if (Nest.value(options,"templateScreens","createMissing").$() && $screensToCreate) {
			API.TemplateScreen()->create($screensToCreate);
		}
		if (Nest.value(options,"templateScreens","updateExisting").$() && $screensToUpdate) {
