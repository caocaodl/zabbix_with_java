...
class CSeverity extends CTag {
	...
	public function __construct(array $options = CArray.array()) {
		...
		attr("id", isset(Nest.value($options,"id").$()) ? Nest.value($options,"id").$() : zbx_formatDomId(Nest.value($options,"name").$()));
		addClass("jqueryinputset control-severity");

		if (!isset(Nest.value($options,"value").$())) {
			Nest.value($options,"value").$() = TRIGGER_SEVERITY_NOT_CLASSIFIED;
		}
		...
		for(getSeverityCaption() as $severity => $caption) {
			$items[] = new CRadioButton(
				Nest.value($options,"name").$(),
				$severity,
				null,
				$options["name"]."_".$severity,
				(Nest.value($options,"value").$() == $severity)
			);
			...
			if (Nest.value($options,"value").$() == $severity) {
		...
		insert_js('
			...
			.mouseleave(function() {
				jQuery(\"#".getAttribute("id")." [aria-pressed=\\"true\\"]\").trigger(\"mouseenter\");
			});
