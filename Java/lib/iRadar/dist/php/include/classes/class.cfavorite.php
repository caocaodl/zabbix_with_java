...
class CFavorite {
	...
	public static function get($idx) {
		...
		while ($profile = DBfetch($db_profiles)) {
			$result[] = CArray.array("value" => Nest.value($profile,"value_id").$(), "source" => Nest.value($profile,"source").$());
		}
	...
	public static function add($idx, $favid, $favobj = null) {
		...
		if (!is_null($favobj)) {
			Nest.value($values,"source").$() = zbx_dbstr($favobj);
		}
	...
	public static function exists($idx, $favid, $favobj = null) {
		...
		for($favorites as $favorite) {
			if (idcmp($favid, Nest.value($favorite,"value").$()) && Nest.value($favorite,"source").$() == $favobj) {
				return true;
			}
		}
