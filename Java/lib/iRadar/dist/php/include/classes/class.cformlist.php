...
class CFormList extends CDiv {
	...
	protected $formInputs = CArray.array("ctextbox", "cnumericbox", "ctextarea", "ccombobox", "ccheckbox", "cpassbox", "cipbox");

	public function __construct($id = null, $class = null, $editable = true) {
		editable = $editable;
		formList = new CList(null, "formlist");
	...
	public function addRow($term, $description = null, $hidden = false, $id = null, $class = null) {
		...
		if (is_object($description)) {
			...
			if (in_CArray.array($inputClass, formInputs)) {
		...
		if ($description === null) {
			formList->addItem(CArray.array(new CDiv(SPACE, "dt right"), new CDiv($label, "dd")), $class, $id);
		}
		else {
			formList->addItem(CArray.array(new CDiv($label, "dt right"), new CDiv($description, "dd")), $class, $id);
		}
	}
	...
	public function addVar($name, $value, $id = null) {
		if ($value !== null) {
			return addItem(new CVar($name, $value, $id));
		}
	}
}
