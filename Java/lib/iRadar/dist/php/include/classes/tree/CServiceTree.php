...
class CServiceTree extends CTree {
	...
	protected function makeCol($rowId, $colName) {
		...
		if ($colName == "status" && zbx_is_int(tree[$rowId][$colName]) && tree[$rowId]["id"] > 0) {
			$status = tree[$rowId][$colName];

			// do not show the severity for information and unclassified triggers
			if (in_CArray.array($status, CArray.array(TRIGGER_SEVERITY_INFORMATION, TRIGGER_SEVERITY_NOT_CLASSIFIED))) {
				tree[$rowId][$colName] = new CSpan(_("OK"), "green");
			}
			else {
				tree[$rowId][$colName] = getSeverityCaption($status);
