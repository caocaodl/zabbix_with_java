...
class Db2DbBackend extends DbBackend {
	...
	protected function checkDbVersionTable() {
		...
		$tabSchema = zbx_dbstr(!empty(Nest.value($DB,"SCHEMA").$()) ? Nest.value($DB,"SCHEMA").$() : zbx_strtoupper(Nest.value($DB,"USER").$()));
		$tableExists = DBfetch(DBselect("SELECT 1 FROM SYSCAT.TABLES".
			\" WHERE TABNAME="DBVERSION"\".
				\" AND TABSCHEMA=\".$tabSchema));

		if (!$tableExists) {
			setError(_("The frontend does not match Zabbix database."));
			return false;
		}
