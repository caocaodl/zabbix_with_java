...
abstract class DbBackend {
	...
	public function checkDbVersion() {
		if (!checkDbVersionTable()) {
			return false;
		}

		$version = DBfetch(DBselect("SELECT dv.mandatory,dv.optional FROM dbversion dv"));
		if (Nest.value($version,"mandatory").$() != ZABBIX_DB_VERSION) {
			setError(_s("The frontend does not match Zabbix database. Current database version (mandatory/optional): %d/%d. Required mandatory version: %d. Contact your system administrator.",
				Nest.value($version,"mandatory").$(), Nest.value($version,"optional").$(), ZABBIX_DB_VERSION));
			return false;
		}
