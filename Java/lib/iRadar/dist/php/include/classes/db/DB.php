...
class DB {
	...
	protected static function reserveIds($table, $count) {
		...
		$id_name = Nest.value($tableSchema,"key").$();
		...
		if (Nest.value($DB,"TYPE").$() != ZBX_DB_SQLITE3) {
			$sql = $sql." FOR UPDATE";
		}

		$res = DBfetch(DBselect($sql));
		if ($res) {
			$maxNextId = bcadd(Nest.value($res,"nextid").$(), $count, 0);
			if (bccomp($maxNextId, self::$maxNodeId, 0) == 1 || bccomp($maxNextId, self::$minNodeId, 0) == -1) {
				...
				$nextid = bcadd(Nest.value($res,"nextid").$(), 1, 0);
			}
		}
		else {
			$nextid = self::refreshIds($table, $count);
	...
	private static function refreshIds($table, $count) {
		...
		$nextid = ($row && Nest.value($row,"id").$()) ? Nest.value($row,"id").$() : self::$minNodeId;
	...
	private static function addMissingFields($tableSchema, $values) {
		...
		if (Nest.value($DB,"TYPE").$() == ZBX_DB_MYSQL) {
			for(Nest.value($tableSchema,"fields").$() as $name => $field) {
				if (Nest.value($field,"type").$() == DB::FIELD_TYPE_TEXT && !Nest.value($field,"null").$()) {
					for($values as &$value) {
						if (!isset($value[$name])) {
							$value[$name] = "";
						}
	...
	public static function getDefaults($table) {
		...
		$defaults = CArray.array();
		for(Nest.value($table,"fields").$() as $name => $field) {
			if (isset(Nest.value($field,"default").$())) {
				$defaults[$name] = Nest.value($field,"default").$();
			}
		}
		return $defaults;
	}
	 ...
	 * @return array
	 */
	public static function find($tableName, array $criteria = CArray.array()) {
		// build the WHERE part
		$sqlWhere = CArray.array();
		for($criteria as $field => $value) {
	...
	public static function update($table, $data) {
		...
		for($data as $row) {
			// check
			self::checkValueTypes($table, Nest.value($row,"values").$());
			if (empty(Nest.value($row,"values").$())) {
				self::exception(self::DBEXECUTE_ERROR, _s("Cannot perform update statement on table \"%1$s\" without values.", $table));
			}

			// set creation
			$sqlSet = "";
			for(Nest.value($row,"values").$() as $field => $value) {
				$sqlSet .= " ".$field."=".$value.",";
			}
			$sqlSet = rtrim($sqlSet, ",");

			if (!isset(Nest.value($row,"where").$()) || empty(Nest.value($row,"where").$()) || !is_array(Nest.value($row,"where").$())) {
			...
			$sqlWhere = CArray.array();
			for(Nest.value($row,"where").$() as $field => $values) {
	...
	public static function updateByPk($tableName, $pk, array $values) {
		return self::update($tableName, CArray.array(
			"where" => CArray.array(self::getPk($tableName) => $pk),
			"values" => $values
		));
	}
	...
	public static function save($tableName, array $data) {
		...
		$newRecords = CArray.array();
		for($data as $key => $record) {
	...
	public static function replace($tableName, array $oldRecords, array $newRecords) {
		...
		$modifiedRecords = CArray.array();
		for($newRecords as $record) {
	 ...
	 *
	 * Example:
	 * DB::delete("applications", CArray.array("applicationid"=>CArray.array(1, 8, 6)));
	 * DELETE FROM applications WHERE applicationid IN (1, 8, 6)
	 *
	 * DB::delete("applications", CArray.array("applicationid"=>CArray.array(1), "templateid"=CArray.array(10)));
