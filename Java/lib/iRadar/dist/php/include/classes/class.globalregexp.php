...
class GlobalRegExp {
	 ...
	 * @var array
	 */
	private static $_cachedExpressions = CArray.array();
	...
	public function __construct($regExp) {
		if ($regExp[0] == "@") {
			...
			if (!isset(self::$_cachedExpressions[$regExp])) {
				self::$_cachedExpressions[$regExp] = CArray.array();
			...
			expression = self::$_cachedExpressions[$regExp];
		}
		else {
			isZabbixRegexp = false;
			expression = $regExp;
		}
	}
	...
	public function match($string) {
		if (isZabbixRegexp) {
			...
			$result = (bool) preg_match("/".expression."/", $string);
		}
	...
	public static function matchExpression(array $expression, $string) {
		if (Nest.value($expression,"expression_type").$() == EXPRESSION_TYPE_TRUE || Nest.value($expression,"expression_type").$() == EXPRESSION_TYPE_FALSE) {
	...
	private static function _matchRegular(array $expression, $string) {
		...
		if (!Nest.value($expression,"case_sensitive").$()) {
			$pattern .= "i";
		}

		$expectedResult = (Nest.value($expression,"expression_type").$() == EXPRESSION_TYPE_TRUE);
	...
	private static function _matchString(array $expression, $string) {
		...
		if (Nest.value($expression,"expression_type").$() == EXPRESSION_TYPE_ANY_INCLUDED) {
			$paterns = explode(Nest.value($expression,"exp_delimiter").$(), Nest.value($expression,"expression").$());
		}
		else {
			$paterns = CArray.array(Nest.value($expression,"expression").$());
		}

		$expectedResult = (Nest.value($expression,"expression_type").$() != EXPRESSION_TYPE_NOT_INCLUDED);

		for($paterns as $patern) {
			if (Nest.value($expression,"case_sensitive").$()) {
			...
			if (Nest.value($expression,"expression_type").$() == EXPRESSION_TYPE_ANY_INCLUDED && $tmp) {
				return true;
			}
			else {
				$result = ($result && $tmp);
