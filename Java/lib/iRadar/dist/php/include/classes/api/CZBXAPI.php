...
class CZBXAPI {
	 ...
	 * @var array
	 */
	protected $sortColumns = CArray.array();
	 ...
	 * @var array
	 */
	protected $globalGetOptions = CArray.array();
	 ...
	 * @var array
	 */
	protected $getOptions = CArray.array();
	 ...
	 * @var array
	 */
	protected $errorMessages = CArray.array();

	public function __construct() {
		// set the PK of the table
		pk = pk(tableName());

		globalGetOptions = CArray.array(
			"nodeids"				=> null,
			// filter
			"filter"				=> null,
			"search"				=> null,
	...
	protected function outputExtend($tableName, $fields, $output) {
		...
		for($fields as $field) {
			if ($output == API_OUTPUT_REFER) {
				$output = CArray.array(pk($tableName), $field);
			}

			if (is_array($output) && !in_CArray.array($field, $output)) {
				$output[] = $field;
			}
		}
	...
	protected function outputIsRequested($field, $output) {
		switch ($output) {
			...
			default:
				return in_CArray.array($field, $output);
		}
	}
	...
	protected function createRelationMap(array $objects, $baseField, $foreignField, $table = null) {
		...
		if ($table) {
			$res = DBselect(API.getApi().createSelectQuery($table, CArray.array(
				"output" => CArray.array($baseField, $foreignField),
				"filter" => CArray.array($baseField => array_keys($objects)),
	...
	protected function select($tableName, array $options) {
		$limit = isset(Nest.value($options,"limit").$()) ? Nest.value($options,"limit").$() : null;

		$sql = createSelectQuery($tableName, $options);

		$objects = DBfetchArray(DBSelect($sql, $limit));

		if (isset(Nest.value($options,"preservekeys").$())) {
			$rs = CArray.array();
			for($objects as $object) {
				$rs[$object[pk($tableName)]] = $object;
			}
	...
	protected function createSelectQuery($tableName, array $options) {
		$sqlParts = createSelectQueryParts($tableName, tableAlias(), $options);

		return createSelectQueryFromParts($sqlParts);
	}
	...
	protected function createSelectQueryParts($tableName, $tableAlias, array $options) {
		...
		$options = zbx_array_merge(globalGetOptions, $options);

		$sqlParts = CArray.array(
			"select" => CArray.array(fieldId(pk($tableName), $tableAlias)),
			"from" => CArray.array(tableId($tableName, $tableAlias)),
			"where" => CArray.array(),
			"group" => CArray.array(),
			"order" => CArray.array(),
			"limit" => null
		...
		// add filter options
		$sqlParts = applyQueryFilterOptions($tableName, $tableAlias, $options, $sqlParts);

		// add output options
		$sqlParts = applyQueryOutputOptions($tableName, $tableAlias, $options, $sqlParts);

		// add node options
		$sqlParts = applyQueryNodeOptions($tableName, $tableAlias, $options, $sqlParts);

		// add sort options
		$sqlParts = applyQuerySortOptions($tableName, $tableAlias, $options, $sqlParts);
	...
	protected function createSelectQueryFromParts(array $sqlParts) {
		// build query
		$sqlSelect = implode(",", array_unique(Nest.value($sqlParts,"select").$()));
		$sqlFrom = implode(",", array_unique(Nest.value($sqlParts,"from").$()));
		$sqlWhere = empty(Nest.value($sqlParts,"where").$()) ? "" : " WHERE ".implode(" AND ", array_unique(Nest.value($sqlParts,"where").$()));
		$sqlGroup = empty(Nest.value($sqlParts,"group").$()) ? "" : " GROUP BY ".implode(",", array_unique(Nest.value($sqlParts,"group").$()));
		$sqlOrder = empty(Nest.value($sqlParts,"order").$()) ? "" : " ORDER BY ".implode(",", array_unique(Nest.value($sqlParts,"order").$()));
	...
	protected function applyQueryOutputOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		$pkFieldId = fieldId(pk($tableName), $tableAlias);

		// count
		if (isset(Nest.value($options,"countOutput").$()) && !requiresPostSqlFiltering($options)) {
			Nest.value($sqlParts,"select").$() = CArray.array("COUNT(DISTINCT ".$pkFieldId.") AS rowscount");

			// select columns used by group count
			if (isset(Nest.value($options,"groupCount").$())) {
				for(Nest.value($sqlParts,"group").$() as $fields) {
					$sqlParts["select"][] = $fields;
				}
			}
		}
		// custom output
		elseif (is_array(Nest.value($options,"output").$())) {
			// the pk field must always be included for the API to work properly
			Nest.value($sqlParts,"select").$() = CArray.array($pkFieldId);
			for(Nest.value($options,"output").$() as $field) {
				if (hasField($field, $tableName)) {
					$sqlParts["select"][] = fieldId($field, $tableAlias);
				}
			}

			Nest.value($sqlParts,"select").$() = array_unique(Nest.value($sqlParts,"select").$());
		}
		// extended output
		elseif (Nest.value($options,"output").$() == API_OUTPUT_EXTEND) {
			// TODO: API_OUTPUT_EXTEND must return ONLY the fields from the base table
			$sqlParts = addQuerySelect(fieldId("*", $tableAlias), $sqlParts);
		}
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		$pkOption = pkOption($tableName);
		$tableId = tableId($tableName, $tableAlias);

		// pks
		if (isset($options[$pkOption])) {
			zbx_value2CArray.array($options[$pkOption]);
			$sqlParts["where"][] = dbConditionString(fieldId(pk($tableName), $tableAlias), $options[$pkOption]);
		}

		// filters
		if (is_array(Nest.value($options,"filter").$())) {
			dbFilter($tableId, $options, $sqlParts);
		}

		// search
		if (is_array(Nest.value($options,"search").$())) {
	...
	protected function applyQueryNodeOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		...
		if (!isset($options[$pkOption])) {
			$nodeIds = isset(Nest.value($options,"nodeids").$()) ? Nest.value($options,"nodeids").$() : get_current_nodeid();
			Nest.value($sqlParts,"where").$() = sqlPartDbNode(Nest.value($sqlParts,"where").$(), $pkFieldId, $nodeIds);
		}
	...
	protected function applyQuerySortOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		if (sortColumns && !zbx_empty(Nest.value($options,"sortfield").$())) {
			Nest.value($options,"sortfield").$() = is_array(Nest.value($options,"sortfield").$())
				? array_unique(Nest.value($options,"sortfield").$())
				: CArray.array(Nest.value($options,"sortfield").$());

			for(Nest.value($options,"sortfield").$() as $i => $sortfield) {
				// validate sortfield
				...
				$sortorder = "";
				if (is_array(Nest.value($options,"sortorder").$())) {
					if (!empty($options["sortorder"][$i])) {
						$sortorder = ($options["sortorder"][$i] == ZBX_SORT_DOWN) ? " ".ZBX_SORT_DOWN : "";
					}
				}
				else {
					$sortorder = (Nest.value($options,"sortorder").$() == ZBX_SORT_DOWN) ? " ".ZBX_SORT_DOWN : "";
				}

				$sqlParts = applyQuerySortField($sortfield, $sortorder, $tableAlias, $sqlParts);

				// add sort field to select if distinct is used
				if (count(Nest.value($sqlParts,"from").$()) > 1) {
					if (!str_in_array($tableAlias.".".$sortfield, Nest.value($sqlParts,"select").$()) && !str_in_array($tableAlias.".*", Nest.value($sqlParts,"select").$())) {
	 ...
	 * If Nest.value($sqlParts,"select").$() not present it is created and field appended.
	 *
	...
	protected function addQuerySelect($fieldId, array $sqlParts) {
		if (!isset(Nest.value($sqlParts,"select").$())) {
			return CArray.array("select" => CArray.array($fieldId));
		}

		list($tableAlias, $field) = explode(".", $fieldId);

		if (!in_CArray.array($fieldId, Nest.value($sqlParts,"select").$()) && !in_CArray.array(fieldId("*", $tableAlias), Nest.value($sqlParts,"select").$())) {
			// if we want to select all of the columns, other columns from this table can be removed
			if ($field == "*") {
				for(Nest.value($sqlParts,"select").$() as $key => $selectFieldId) {
	...
	protected function addQueryOrder($fieldId, array $sqlParts, $sortorder = null) {
		...
		$sqlParts = addQuerySelect($fieldId, $sqlParts);
	...
	protected function deleteByIds(array $ids) {
		DB::delete(tableName(), CArray.array(
			pk() => $ids
		));
	}
	...
	protected function extendObjects($tableName, array $objects, array $fields) {
		$dbObjects = API.getApi().select($tableName, CArray.array(
			"output" => $fields,
			pkOption($tableName) => zbx_objectValues($objects, pk($tableName)),
			"preservekeys" => true
		));

		for($objects as &$object) {
			$pk = $object[pk($tableName)];
	...
	protected function extendObject($tableName, array $object, array $fields) {
		$objects = extendObjects($tableName, CArray.array($object), $fields);
	...
	protected function checkObjectIds(array $objects, $idField, $messageRequired, $messageEmpty, $messageInvalid) {
		$idValidator = new CIdValidator(CArray.array(
			"messageEmpty" => $messageEmpty,
			"messageRegex" => $messageInvalid
		));
		for($objects as $object) {
			if (!isset($object[$idField])) {
				self::exception(ZBX_API_ERROR_PARAMETERS, _params($messageRequired, CArray.array($idField)));
			}

			checkValidator($object[$idField], $idValidator);
		}
	}
	 ...
	 * @throws APIException
	 */
	protected function checkUnsupportedFields($tableName, array $object, $error, array $extraFields = CArray.array()) {
		$extraFields = array_flip($extraFields);

		for($object as $field => $value) {
	 ...
	 *
	 * Example:
	 * checkNoParameters($item, CArray.array("templateid", "state"), _("Cannot set \"%1$s\" for item \"%2$s\"."), Nest.value($item,"name").$());
	 * If any of the parameters "templateid" or "state" are present in the object, it will be placed in \"%1$s\"
	 * and Nest.value($item,"name").$() will be placed in \"%2$s\".
	 *
	...
	protected function checkNoParameters(array $object, array $params, $error, $objectName) {
		for($params as $param) {
			if (array_key_exists($param, $object)) {
				$error = _params($error, CArray.array($param, $objectName));
				self::exception(ZBX_API_ERROR_PARAMETERS, $error);
			}
		}
	}
	...
	protected function dbFilter($table, $options, &$sqlParts) {
		...
		$filter = CArray.array();
		for(Nest.value($options,"filter").$() as $field => $value) {
		...
		if ($filter) {
			if (isset(Nest.value($sqlParts,"where","filter").$())) {
				$filter[] = Nest.value($sqlParts,"where","filter").$();
			}

			if (is_null(Nest.value($options,"searchByAny").$()) || Nest.value($options,"searchByAny").$() === false || count($filter) == 1) {
				Nest.value($sqlParts,"where","filter").$() = implode(" AND ", $filter);
			}
			else {
				Nest.value($sqlParts,"where","filter").$() = "(".implode(" OR ", $filter).")";
			}
	...
	protected function checkDeprecatedParam(array $params, $paramName, $value = null) {
		if (isset($params[$paramName])) {
			...
			elseif (is_array($params[$paramName]) && in_CArray.array($value, $params[$paramName]) || $params[$paramName] == $value) {
	 ...
	 */
	protected function checkPartialValidator(array $array, CPartialValidatorInterface $validator, $fullArray = CArray.array()) {
	...
	protected function customFetch($query, array $options) {
		if (requiresPostSqlFiltering($options)) {
			...
			$limit = Nest.value($options,"limit").$() ? 2 * Nest.value($options,"limit").$() : null;
			...
			do {
				...
				if (Nest.value($options,"limit").$() && count($allElements) + count($elements) >= Nest.value($options,"limit").$()) {
					$allElements += array_slice($elements, 0, Nest.value($options,"limit").$() - count($allElements), true);
					break;
				}
				...
				if ($limit) {
					...
					$limit = max($minLimit, round($limit / $elemCount * (Nest.value($options,"limit").$() - count($allElements)) * 2));
				}
			} while ($hasMore);
			...
			return DBfetchArray(DBselect($query, Nest.value($options,"limit").$()));
		}
	}
