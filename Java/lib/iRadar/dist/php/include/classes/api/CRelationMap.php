...
class CRelationMap {
	 ...
	 * @var array
	 */
	protected $relatedIds = CArray.array();
	...
	public function addRelation($baseObjectId, $relatedObjectId) {
		map[$baseObjectId][$relatedObjectId] = $relatedObjectId;
		relatedIds[$relatedObjectId] = $relatedObjectId;
	}
	...
	public function mapMany(array $baseObjects, array $relatedObjects, $name, $limit = null) {
		for($baseObjects as $baseObjectId => &$baseObject) {
			$baseObject[$name] = CArray.array();

			if (isset(map[$baseObjectId]) && map[$baseObjectId]) {
				$matchingRelatedObjects = array_values(array_intersect_key($relatedObjects, map[$baseObjectId]));
	...
	public function mapOne(array $baseObjects, array $relatedObjects, $name) {
		for($baseObjects as $baseObjectId => &$baseObject) {
			$matchingRelatedObject = CArray.array();

			if (isset(map[$baseObjectId])) {
				$matchingRelatedId = reset(map[$baseObjectId]);
