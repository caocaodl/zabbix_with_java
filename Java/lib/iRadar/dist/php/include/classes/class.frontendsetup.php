...
class FrontendSetup {
	...
	public function checkRequirements() {
		...
		$result[] = checkPhpVersion();
		$result[] = checkPhpMemoryLimit();
		$result[] = checkPhpPostMaxSize();
		$result[] = checkPhpUploadMaxFilesize();
		$result[] = checkPhpMaxExecutionTime();
		$result[] = checkPhpMaxInputTime();
		$result[] = checkPhpTimeZone();
		$result[] = checkPhpDatabases();
		$result[] = checkPhpBcmath();
		$result[] = checkPhpMbstring();
		$result[] = checkPhpSockets();
		$result[] = checkPhpGd();
		$result[] = checkPhpGdPng();
		$result[] = checkPhpGdJpeg();
		$result[] = checkPhpGdFreeType();
		$result[] = checkPhpLibxml();
		$result[] = checkPhpXmlWriter();
		$result[] = checkPhpXmlReader();
		$result[] = checkPhpCtype();
		$result[] = checkPhpSession();
		$result[] = checkPhpSessionAutoStart();
		$result[] = checkPhpGettext();
	...
	public function checkPhpDatabases() {
		$current = CArray.array();

		$databases = getSupportedDatabases();
		for($databases as $database => $name) {
			$current[] = $name;
			$current[] = BR();
		}
	...
	public function getSupportedDatabases() {
		$allowedDb = CArray.array();
		if (zbx_is_callable(CArray.array("mysqli_connect", "mysqli_connect_error", "mysqli_error", "mysqli_query",
				"mysqli_fetch_assoc", "mysqli_free_result", "mysqli_real_escape_string", "mysqli_close"))) {
			$allowedDb[ZBX_DB_MYSQL] = "MySQL";
		}

		if (zbx_is_callable(CArray.array("pg_pconnect", "pg_fetch_array", "pg_fetch_row", "pg_exec", "pg_getlastoid"))) {
			$allowedDb[ZBX_DB_POSTGRESQL] = "PostgreSQL";
		}

		if (zbx_is_callable(CArray.array("oci_connect", "oci_error", "oci_parse", "oci_execute", "oci_fetch_assoc",
		...
		if (zbx_is_callable(CArray.array("db2_connect", "db2_set_option", "db2_commit", "db2_rollback", "db2_autocommit",
		...
		if (class_exists("SQLite3", false) && zbx_is_callable(CArray.array("ftok", "sem_acquire", "sem_release", "sem_get"))) {
