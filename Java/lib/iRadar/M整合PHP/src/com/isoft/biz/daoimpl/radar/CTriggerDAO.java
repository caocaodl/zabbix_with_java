...
public class CTriggerDAO extends CTriggerGeneralDAO<CTriggerGet> {
	...
	protected void createReal(CArray<Map> triggers) {
		...
		for (Entry<Object, Map> e : triggers.entrySet()) {
			...
			info(_s("Created: Trigger \"%1$s\" on \"%2$s\".", Nest.value(trigger,"description").$(), implode(", ", triggersAndHosts.get(triggerId))));
	...
	protected void updateReal(CArray<Map> triggers) {
		...
		for(Map trigger : triggers) {
			...
			if (expressionChanged) {
				...
				if (!isset(trigger,"dependencies")) {
					Nest.value(trigger,"dependencies").$(rda_objectValues(Nest.value(dbTrigger,"dependencies").$(), "triggerid"));
				}
			}
			...
			infos.add( _s("Updated: Trigger \"%1$s\" on \"%2$s\".", Nest.value(trigger,"description").$(), implode(", ", hosts)));
