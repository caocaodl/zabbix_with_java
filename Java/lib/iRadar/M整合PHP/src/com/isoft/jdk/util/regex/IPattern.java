package com.isoft.jdk.util.regex;
...
import com.isoft.jdk.lang.ICharacter;

public final class IPattern
    implements java.io.Serializable
{
     ...
     */
    public static IPattern compile(String regex) {
        return new IPattern(regex, 0);
    }
     ...
     *          If the expression's syntax is invalid
     */
    public static IPattern compile(String regex, int flags) {
        return new IPattern(regex, flags);
    }
     ...
     */
    public IMatcher matcher(CharSequence input) {
        if (!compiled) {
            synchronized(this) {
                if (!compiled)
                    compile();
        ...
        IMatcher m = new IMatcher(this, input);
        return m;
    }
    ...
    public static boolean matches(String regex, CharSequence input) {
        IPattern p = IPattern.compile(regex);
        IMatcher m = p.matcher(input);
        return m.matches();
    }
     ...
     */
    private IPattern(String p, int f) {
        pattern = p;
        flags = f;
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private static final boolean isSupplementary(int ch) {
        return ch >= Character.MIN_SUPPLEMENTARY_CODE_POINT ||
               ICharacter.isSurrogate((char)ch);
    }
    ...
    private CharProperty unicodeScriptPropertyFor(String name) {
        final ICharacter.UnicodeScript script;
        try {
            script = ICharacter.UnicodeScript.forName(name);
    ...
    static class Node extends Object {
        Node next;
        Node() {
            next = IPattern.accept;
        }
        /**
         * This method implements the classic accept node.
         */
        boolean match(IMatcher matcher, int i, CharSequence seq) {
            matcher.last = i;
    ...
    static class LastNode extends Node {
         ...
         */
        boolean match(IMatcher matcher, int i, CharSequence seq) {
            if (matcher.acceptMode == IMatcher.ENDANCHOR && i != matcher.to)
                return false;
            matcher.last = i;
            matcher.groups[0] = matcher.first;
            matcher.groups[1] = matcher.last;
    ...
    static final class Script extends CharProperty {
        final ICharacter.UnicodeScript script;
        Script(ICharacter.UnicodeScript script) {
            this.script = script;
        }
        boolean isSatisfiedBy(int ch) {
            return script == ICharacter.UnicodeScript.of(ch);
        }
    }
    ...
    static final class Curly extends Node {
        ...
        boolean match0(IMatcher matcher, int i, int j, CharSequence seq) {
            if (j >= cmax) {
        ...
        // i is the index to start matching at
        // j is the number of atoms that have matched
        boolean match1(IMatcher matcher, int i, int j, CharSequence seq) {
            for (;;) {
        ...
        boolean match2(IMatcher matcher, int i, int j, CharSequence seq) {
            for (; j < cmax; j++) {
                if (!atom.match(matcher, i, seq))
                    break;
                if (i == matcher.last)
    ...
    static final class GroupHead extends Node {
        ...
        boolean matchRef(IMatcher matcher, int i, CharSequence seq) {
            int save = matcher.locals[localIndex];
    ...
    static class Loop extends Node {
        ...
        boolean matchInit(IMatcher matcher, int i, CharSequence seq) {
    ...
    static final class Bound extends Node {
        ...
        int check(IMatcher matcher, int i, CharSequence seq) {
            int ch;
            boolean left = false;
            int startIndex = matcher.from;
            int endIndex = matcher.to;
     ...
     */
    private static boolean hasBaseCharacter(IMatcher matcher, int i,
                                            CharSequence seq)
    {
    ...
    private static class CharPropertyNames {
        ...
        static {
            ...
            defClone("javaAlphabetic", new CloneableProperty() {
                boolean isSatisfiedBy(int ch) {
                    return ICharacter.isAlphabetic(ch);}});
            defClone("javaIdeographic", new CloneableProperty() {
                boolean isSatisfiedBy(int ch) {
                    return ICharacter.isIdeographic(ch);}});
