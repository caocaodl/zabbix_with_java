	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		if (!empty(Nest.value(data,"hostid").$())) {
			if (!empty(Nest.value(data,"parent_discoveryid").$())) {
				itemWidget.addItem(get_header_host_table(idBean, executor, !empty(Nest.value(data,"is_discovery_rule").$()) ? "discoveries" : "items",
					Nest.value(data,"hostid").asLong(true), Nest.value(data,"parent_discoveryid").asLong(true)));
			} else {
				itemWidget.addItem(get_header_host_table(idBean, executor, !empty(Nest.value(data,"is_discovery_rule").$()) ? "discoveries" : "items",
					Nest.value(data,"hostid").asLong(true)));
			}
		}

		itemWidget.addPageHeader(Nest.value(data,"page_header").$());
		...
		if (Nest.value(data,"is_discovery_rule").asBoolean()) {
			...
			CListBox applicationComboBox = new CListBox("applications[]", Nest.array(data,"applications").asString(), 6);
			...
			if (!Nest.value(data,"limited").asBoolean()) {
				if (Nest.value(data,"is_discovery_rule").asBoolean()) {
					array_push(buttons, new CButtonDelete(_("Delete discovery rule?"),
						url_params(idBean, array("form", "groupid", "itemid", "parent_discoveryid", "hostid")))
					);
				} else {
					buttons.add(new CButtonDelete(
						Nest.value(data,"parent_discoveryid").asBoolean() ? _("Delete item prototype?") : _("Delete item?"),
						url_params(idBean, array("form", "groupid", "itemid", "parent_discoveryid", "hostid"))
					));
				}
			}
