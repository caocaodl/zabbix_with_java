...
public class CConfigurationMaintenanceEdit extends CViewSegment {
	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		maintenanceFormList.addRow(_("Description"), new CTextArea("description", Nest.value(data,"description").asString()));

		/* Maintenance period tab */
		CFormList maintenancePeriodFormList = new CFormList("maintenancePeriodFormList");
		...
		CTweenBox hostTweenBox = new CTweenBox(maintenanceForm, "hostids", Nest.value(data,"hostids").$(), 10);
		...
		if (empty(Nest.value(data,"maintenanceid").$())) {
			...
			maintenanceForm.addItem(makeFormFooter(
				...
				array(
					...
					new CButtonDelete(_("Delete maintenance period?"), url_param(idBean, "form")+url_param(idBean, "maintenanceid")),
					new CButtonCancel()
				)
			));
		}
