...
public class TrLogformAction extends RadarBaseAction {
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"expressions",	array(T_RDA_STR, O_OPT,	 null,			NOT_EMPTY,			"isset({save_trigger})"),
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"save_trigger")) {
			...
			if(!empty(_exprs) && !empty(_expression = construct_expression(getIdentityBean(), executor, Nest.value(_REQUEST,"itemid").asString(), _exprs))){
				...
				if(isset(_REQUEST,"triggerid")){
					...
					DBstart(executor);
					CArray<Long[]> ctTriggers = API.Trigger(getIdentityBean(), executor).create(array(_trigger));					
					if(!empty(ctTriggers)){
						CTriggerGet toptions = new CTriggerGet();
				...
				if(_result){
					add_audit(getIdentityBean(), executor, _audit_action, AUDIT_RESOURCE_TRIGGER, _("Trigger")+" ["+_triggerid+"] ["+Nest.value(_trigger,"description").asString()+"]");
					unset(_REQUEST,"sform");

					rda_add_post_js("closeForm(\"items.action\");");
				}
			}
		}
		//------------------------ </ACTIONS> --------------------------
		...
		//EGEXP_INCLUDE
			...
			String _script = "javascript: return PopUp(\"popup.action?dstfrm="+_frmTRLog.getName()+"&dstfld1=itemid&dstfld2=item&srctbl=items&srcfld1=itemid&srcfld2=name\",800,450);";
			CSubmit _cbtn = new CSubmit("select_item",_("Select"),_script);
			...
			CButton _cb = new CButton("add_exp",_("Add"),"javascript: add_logexpr();");
			CButton _cbAdd = new CButton("add_key_and", _("AND"), "javascript: add_keyword_and();");
			CButton _cbOr = new CButton("add_key_or", _("OR"), "javascript: add_keyword_or();");
			CCheckBox _cbIregexp = new CCheckBox("iregexp", false, null,1);


			_frmTRLog.addRow(_("Expression"), array(_ctb,BR(),_cbIregexp,"iregexp",SPACE,_cbAdd,SPACE,_cbOr,SPACE,_exp_select,SPACE, _cb));

			CTableInfo _keyTable = new CTableInfo(null);
			_keyTable.setAttribute("id","key_list");
			_keyTable.setHeader(array(_("Keyword"), _("Type"), _("Action")));

			CTableInfo _table = new CTableInfo(null);
			_table.setAttribute("id","exp_list");
			_table.setHeader(array(_("Expression"), _("Type"), _("Position"), _("Action")));
			...
			_frmTRLog.addItemToBottomRow(new CSubmit("save_trigger", _("Save"), "javascript: document.forms[0].action += \"?saction=1\";"));
			_frmTRLog.addItemToBottomRow(SPACE);
			_frmTRLog.addItemToBottomRow(new CButton("cancel", _("Cancel"), "javascript: self.close();"));
