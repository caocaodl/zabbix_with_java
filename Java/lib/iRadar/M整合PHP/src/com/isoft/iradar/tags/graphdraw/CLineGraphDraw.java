...
public class CLineGraphDraw extends CGraphDraw {
	...
	private CArray<Map> triggers = array();
	private int ymin_type = GRAPH_YAXIS_TYPE_CALCULATED;
	private int ymax_type = GRAPH_YAXIS_TYPE_CALCULATED;
	private int yaxisright = 0;
	private int yaxisleft = 0;
	private int skipLeftScale = 0; // in case if left axis should be drawn but doesn't contain any data
	private int skipRightScale = 0; // in case if right axis should be drawn but doesn't contain any data
	private String ymin_itemid = "0";
	private String ymax_itemid = "0";
	private int legendOffsetY = 90;
	private CArray<CArray<Float>> percentile = map(
	 	"left", map(
	...
	private int m_showWorkPeriod = 1;
	private int m_showTriggers = 1;
	private CArray<Integer> zero = array();
	private CArray<String> graphOrientation = map(
	 	GRAPH_YAXIS_SIDE_LEFT, "",
	 	GRAPH_YAXIS_SIDE_RIGHT, ""
	 );
	private CArray grid = array(); // vertical & horizontal grids params
	private CArray<Integer> gridLinesCount = array(); // How many grids to draw
	private CArray<Double> gridStep = array(); // grid step
	private int gridPixels = 25; // optimal grid size
	private int gridPixelsVert = 40;
	
	private CArray<Double> unit2px = array();
	private CArray<Double> oxy = array();
	private CArray<Double> gridStepX = array();
	private long diffTZ;
	private Object itemsHost;
				...
				for(; _cj > 0; _cj--) {
					...
					for(String _var_name: array("clock", "min", "max", "avg")) {
						...
						if (_first_idx == _ci && "clock".equals(_var_name)) {
							_var.put(_ci - (_dx - _cj), Nest.value(_var, _first_idx).asInteger() - ((p / sizeX) * (_dx - _cj)));
							continue;
						}

						int _dy = Nest.value(_var, _ci).asInteger() - Nest.value(_var, _first_idx).asInteger();
						_var.put(_ci - (_dx - _cj), bcadd(Nest.value(_var, _first_idx).asInteger() , bcdiv((_cj * _dy) , _dx)));
					}
				}
			}
				...
				for(;_cj > 0; _cj--) {
					for(String _var_name: array("clock", "min", "max", "avg")) {
						...
						if ("clock".equals(_var_name)) {
							_var.put(_first_idx + (_dx - _cj), Nest.value(_var, _first_idx).asInteger() + ((p / sizeX) * (_dx - _cj)));
							continue;
						}
				...
				for (int j = i - 1; j >= 0; j--) {
					...
					for (int _ci = 0; _ci < sizeX; _ci++) {
						for(String _var_name: array("min", "max", "avg")) {
							...
							_curr_shift.put(_ci, Nest.value(_prev_var, _ci).asInteger() + Nest.value(_prev_shift, _ci).asInteger());
						}
					}
					break;
				}
...
//			}
			...
			if (isset(_data) && isset(Nest.value(_data,"min").$())) {
				...
				_legend.addCell(_rowNum, map(
					"text" , convert_units(map(
						"value" , Nest.value(_data,"avg_orig").$(),
						"units" , items.getNested(i, "units"),
						"convert" , ITEM_CONVERT_NO_UNITS
					)),
					"align" , 2
...
//		}
		...
		for (int _item = 0; _item < num; _item++) {
			...
			if (type == GRAPH_TYPE_STACKED) {
				...
				_max_color = getColor("ValueMax", GRAPH_STACKED_ALFA);
				_avg_color = getColor(Nest.value(items, _item, "color").asString(), GRAPH_STACKED_ALFA);
				_min_color = getColor("ValueMin", GRAPH_STACKED_ALFA);
				_minmax_color = getColor("ValueMinMax", GRAPH_STACKED_ALFA);
