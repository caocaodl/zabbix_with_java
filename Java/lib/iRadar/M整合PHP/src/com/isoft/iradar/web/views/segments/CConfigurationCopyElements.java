		...
		if (Nest.value(data,"copy_type").asInteger() == 0) {
			...
			for(Map group :(CArray<Map>)Nest.value(data,"groups").asCArray()) {
				array_push(
					targets,
					array(
						new CCheckBox("copy_targetid["+Nest.value(group,"groupid").asString()+"]", uint_in_array(Nest.value(group,"groupid").$(), Nest.value(data,"copy_targetid").asCArray()), null, Nest.value(group,"groupid").asInteger()),
						SPACE,
