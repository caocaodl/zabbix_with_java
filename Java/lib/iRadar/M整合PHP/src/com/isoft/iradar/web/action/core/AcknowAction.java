...
public class AcknowAction extends RadarBaseAction {
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"message",			array(T_RDA_STR, O_OPT, null,	bulk ? null : NOT_EMPTY, "isset({save})||isset({saveandreturn})"),
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (bulk) {
			...
			if (!empty(acknowledges)) {
				...
				for(Map acknowledge : acknowledges) {
					...
					acknowledgesTable.addRow(new CCol(rda_nl2br(Nest.value(acknowledge,"message").asString()), null, 2), "msg");
				}
		...
		CFormTable messageTable = new CFormTable(title+" \""+getUserFullname(CWebUser.data())+"\"");
		...
		messageTable.addItemToBottomRow(new CButtonCancel(url_params(getIdentityBean(), array("backurl", "eventid", "triggerid", "screenid"))));
