...
public class GraphsAction extends RadarBaseAction {
	...
	protected void doCheckFields(SQLExecutor executor) {
		...
		CArray fields = map(
			...
			"yaxismin",					array(T_RDA_DBL, O_OPT, null,		null,			"isset({save})&&({graphtype}==0||{graphtype}==1)"),
			"yaxismax",					array(T_RDA_DBL, O_OPT, null,		null,			"isset({save})&&({graphtype}==0||{graphtype}==1)"),
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"clone") && isset(_REQUEST,"graphid")) {
			...
			_REQUEST.putAll(array_merge(_REQUEST, graph));
			...
			if (result) {
				if (hasRequest("graphid")) {
					add_audit(getIdentityBean(), executor,
						...
						"Graph ID ["+Nest.value(graph,"graphid").$()+"] Graph ["+get_request("name")+"]"
					);
				} else {
					add_audit(getIdentityBean(), executor,AUDIT_ACTION_ADD, AUDIT_RESOURCE_GRAPH, "Graph ["+get_request("name")+"]");
				}
				unset(_REQUEST,"form");
			}
			...
			if (hasRequest("parent_discoveryid")) {
				...
				boolean result = Call(new Wrapper<Boolean>() {
					@Override
					protected Boolean doCall() throws Throwable {
						return !empty(API.Graph(getIdentityBean(), executor).delete(graphIds).valuesAsLong());
					}
				});
		...
		if ("copy_to".equals(Nest.value(_REQUEST,"go").asString()) && isset(_REQUEST,"group_graphid")) {
			...
			if (!empty(Nest.value(data,"graphid").$()) && !isset(_REQUEST,"form_refresh")) {
						...
						if (!empty(get_request("parent_discoveryid"))) {
							...
							if (!empty(parentGraphPrototypes)) {
								...
								link = new CLink(Nest.value(parentTemplate,"name").asString(),
									"graphs.action?form=update&graphid="+Nest.value(parentGraphPrototype,"graphid").asString()+"&hostid="+Nest.value(parentTemplate,"templateid").asString()+"&parent_discoveryid="+Nest.value(parentGraphPrototype,"discoveryRule","itemid").asString()
								);
							}
							...
							link = new CLink(Nest.value(parentTemplate,"name").asString(),
								"graphs.action?form=update&graphid="+Nest.value(parentGraph,"graphid").asString()+"&hostid="+Nest.value(parentTemplate,"hostid").asString()
							);
						}
						if (isset(link)) {
				...
				Nest.value(data,"yaxismin").$(get_request("yaxismin", 0));
				Nest.value(data,"yaxismax").$(get_request("yaxismax", 100));
