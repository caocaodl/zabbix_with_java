...
public class HttpmonAction extends RadarBaseAction {
	...
	public void doAction(SQLExecutor executor) {
		...
		if (!empty(this.pageFilter.$("hostsSelected").$())) {
			...
			options.setFilter("status", String.valueOf(HTTPTEST_STATUS_ACTIVE));
			...
			resolveHttpTestMacros(this.getIdentityBean(), executor, httpTests, true, false);
