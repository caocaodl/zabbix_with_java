...
public class ItemsUtil {
	...
	public static CArray<String> item_type2str() {
		CArray<String> types = map(
			ITEM_TYPE_IRADAR, _("iRadar agent"),
			ITEM_TYPE_IRADAR_ACTIVE, _("iRadar agent (active)"),
			...
			ITEM_TYPE_INTERNAL, _("iRadar internal"),
			ITEM_TYPE_TRAPPER, _("iRadar trapper"),
			ITEM_TYPE_AGGREGATE, _("iRadar aggregate"),
	...
	public static boolean delete_trends_by_itemid(SQLExecutor executor, Long[] itemIds) {
		...
		String sql =  "DELETE FROM trends WHERE " + segmentSql;
		boolean r1 = executor.executeInsertDeleteUpdate(sql, params)>0;
		sql =  "DELETE FROM trends_uint WHERE " + segmentSql;
		boolean r2 = executor.executeInsertDeleteUpdate(sql, params)>0;
		return r1 && r2;
	}
