...
public class HostsAction extends RadarBaseAction {
	...
	public void doAction(final SQLExecutor executor) {
		...
		if (isset(_REQUEST,"unlink") || isset(_REQUEST,"unlink_and_clear")) {
			...
			try {
				...
				final CArray<Map> hosts = map("hosts", API.Host(getIdentityBean(), executor).get(hoptions));

				String[] properties = new String[]{"proxy_hostid", "ipmi_authtype", "ipmi_privilege", "ipmi_username", "ipmi_password", "status"};
				final CArray newValues = array();
				...
				if (isset(visible,"new_groups") && !empty(Nest.value(_REQUEST,"new_groups").$())) {
					if (CWebUser.getType() == USER_TYPE_SUPER_ADMIN) {
						final CArray<Map> newGroups = array(); 
						...
						if (isset(newGroups)) {
							CArray<Long[]> createdGroups = Call(new Wrapper<CArray<Long[]>>() {
								@Override
								protected CArray<Long[]> doCall() throws Throwable {
									return API.HostGroup(getIdentityBean(), executor).create(newGroups);
								}
							},null);
				...
				boolean result = Call(new Wrapper<Boolean>() {
					...
					protected Boolean doCall() throws Throwable {
						return !empty(API.Host(getIdentityBean(), executor).massUpdate(array_merge(hosts, newValues)));
					}
				});
				if (!result) {
					throw new Exception();
				...
				if (!empty(add)) {
					...
					result = Call(new Wrapper<Boolean>() {
						@Override
						protected Boolean doCall() throws Throwable {
							return !empty(API.Host(getIdentityBean(), executor).massAdd(add));
						}
					});
					if (!result) {
						throw new Exception();
			...
			try {
				...
				if (!createNew && Nest.value(hostOld,"flags").asInteger() == RDA_FLAG_DISCOVERY_CREATED) {
					...
					if (!rda_empty(Nest.value(_REQUEST,"newgroup").$())) {
						CArray<Long[]> newGroup = Call(new Wrapper<CArray<Long[]>>() {
							@Override
							protected CArray<Long[]> doCall() throws Throwable {
				...
				final Long hostId;
				if (createNew) {
					CArray<Long[]> chostIds = Call(new Wrapper<CArray<Long[]>>() {
						@Override
						protected CArray<Long[]> doCall() throws Throwable {
							return API.Host(getIdentityBean(), executor).create(array(host));
						}
					},null);
					if (!empty(chostIds)) {
					...
					CArray<Long[]> chostIds = Call(new Wrapper<CArray<Long[]>>() {
						@Override
						protected CArray<Long[]> doCall() throws Throwable {
							return API.Host(getIdentityBean(), executor).update(array(host));
						}
					},null);
				...
				if ("full_clone".equals(Nest.value(_REQUEST,"form").asString())) {
					final Long srcHostId = Nest.as(get_request("hostid")).asLong();

					boolean result = Call(new Wrapper<Boolean>() {
						@Override
						protected Boolean doCall() throws Throwable {
							return copyApplications(getIdentityBean(), executor,srcHostId, hostId);
						}
					});
					if (!result) {
						throw new Exception();
					...
					result = Call(new Wrapper<Boolean>() {
						@Override
						protected Boolean doCall() throws Throwable {
							return copyItems(getIdentityBean(), executor,srcHostId, hostId);
						}
					});
					if (!result) {
						throw new Exception();
					...
					result = Call(new Wrapper<Boolean>() {
						@Override
						protected Boolean doCall() throws Throwable {
							return copyHttpTests(getIdentityBean(), executor,srcHostId, hostId);
						}
					});
					if (!result) {
						throw new Exception();
					...
					final CArray<Map> triggers = API.Trigger(getIdentityBean(), executor).get(toptions);
					if (!empty(triggers)) {
						result = Call(new Wrapper<Boolean>() {
							@Override
							protected Boolean doCall() throws Throwable {
								return copyTriggersToHosts(getIdentityBean(), executor,rda_objectValues(triggers, "triggerid").valuesAsLong(), new Long[]{hostId}, srcHostId);
							}
						});
						if (!result) {
							throw new Exception();
					...
					final CArray<Map> discoveryRules = API.DiscoveryRule(getIdentityBean(), executor).get(droptions);
					if (!empty(discoveryRules)) {
						boolean copyDiscoveryRules = Call(new Wrapper<Boolean>() {
							@Override
							protected Boolean doCall() throws Throwable {
								return API.DiscoveryRule(getIdentityBean(), executor).copy(map(
					...
					for(final Map graph : graphs) {
						...
						CArray cgs = Call(new Wrapper<CArray>() {
							@Override
							protected CArray doCall() throws Throwable {
								return copyGraphToHost(getIdentityBean(), executor, Nest.value(graph,"graphid").asLong(), hostId);
							}
						},null);
			...
			boolean result = Call(new Wrapper<Boolean>() {
				@Override
				protected Boolean doCall() throws Throwable {
					return !empty(API.Host(getIdentityBean(), executor).delete(Nest.value(_REQUEST,"hostid").asLong()));
				}
			});
			...
			hostIds = get_request("hosts", array());
			final Long[] fhostIds = hostIds.valuesAsLong();
			...
			boolean goResult = Call(new Wrapper<Boolean>() {
				@Override
				protected Boolean doCall() throws Throwable {
					return !empty(API.Host(getIdentityBean(), executor).delete(fhostIds));
				}
			});
			...
			final int status = enable ? TRIGGER_STATUS_ENABLED : TRIGGER_STATUS_DISABLED;
			...
			final Long[] actHostIds = actHosts.valuesAsLong();
			...
			if (!empty(actHosts)) {
				...
				boolean result = Call(new Wrapper<Boolean>() {
					@Override
					protected Boolean doCall() throws Throwable {
						return updateHostStatus(getIdentityBean(), executor, actHostIds, status);
					}
				});
		...
		if ("massupdate".equals(Nest.value(_REQUEST,"go").asString()) && isset(_REQUEST,"hosts")) {
			...
			if (!empty(hostId  = get_request("hostid", 0L))) {
				hostsWidget.addItem(get_header_host_table(getIdentityBean(), executor, "", Nest.value(_REQUEST,"hostid").asLong(true)));
			...
			for (Map host : hosts) {
				...
				CArray applications = array(new CLink(_("Applications"), "applications.action?groupid="+groupid+"&hostid="+hostid),
					" ("+Nest.value(host,"applications").$()+")");
				CArray items = array(new CLink(_("Items"), "items.action?filter_set=1&hostid="+hostid),
				...
				CArray graphs = array(new CLink(_("Graphs"), "graphs.action?groupid="+groupid+"&hostid="+hostid),
				...
				description.add(new CLink(CHtml.encode(Nest.value(host,"name").asString()), "hosts.action?form=update&hostid="+(Nest.value(host,"hostid").asString()+url_param(getIdentityBean(), "groupid"))));
				...
				switch (Nest.value(host,"status").asInteger()) {
					case HOST_STATUS_MONITORED:
						...
						statusUrl  = "hosts.action?hosts"+SQUAREBRACKETS+"="+Nest.value(host,"hostid").asString()+"&go=disable"+url_param(getIdentityBean(), "groupid");
						break;

					case HOST_STATUS_NOT_MONITORED:
						statusCaption = _("Not monitored");
						statusUrl = "hosts.action?hosts"+SQUAREBRACKETS+"="+Nest.value(host,"hostid").asString()+"&go=activate"+url_param(getIdentityBean(), "groupid");
