...
//			exit();
	...
	public void doAction(SQLExecutor executor) {
		...
		// Main widget header
		CArray<String> text = array(_("EVENTS")+": \""+CMacrosResolverHelper.resolveTriggerName(this.getIdentityBean(), executor, trigger)+"\"");
		...
		left_col.add(eventDetails);

		CArray right_col = array();
		...
		right_col.add(events_histry);

		CDiv leftDiv = new CDiv(left_col, "column");
		CDiv middleDiv = new CDiv(right_col, "column");
