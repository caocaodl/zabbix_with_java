...
public class CConfigurationTemplateEdit extends CViewSegment {
	...
	public CForm doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		if (templateid > 0 && !hasRequest("form_refresh")) {
			...
			groups = Nest.value(data,"dbTemplate","groups").asCArray();
			groups = rda_objectValues(groups, "groupid");
			...
			if (isset(_REQUEST,"groupid") && (Nest.value(_REQUEST,"groupid").asInteger() > 0) && !uint_in_array(Nest.value(_REQUEST,"groupid").$(), groups)) {
				array_push(groups, Nest.value(_REQUEST,"groupid").$());
			}
		...
		// get all Groups
		CTweenBox group_tb = new CTweenBox(frmHost, "groups", groups, 10);
		...
		for(Map group : all_groups) {
			group_tb.addItem(Nest.value(group,"groupid").$(), Nest.value(group,"name").asString());
		}
		...
		templateList.addRow(SPACE, array(tmp_label, BR(), newgroupTB), false, null, "new");
		...
		if ((templateid > 0) && (!"full_clone".equals(Nest.value(_REQUEST,"form").asString()))) {
			...
			others.add(new CButtonDelete(_("Delete template?"), url_param(idBean, "form")+url_param(idBean, "templateid")+url_param(idBean, "groupid")));
			others.add(new CButtonQMessage("delete_and_clear", _("Delete and clear"), _("Delete and clear template? (Warning: all linked hosts will be cleared!)"), url_param(idBean, "form")+url_param(idBean, "templateid")+url_param(idBean, "groupid")));
		}
