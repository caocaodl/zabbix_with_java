...
public class PopupAction extends RadarBaseAction {
	...
	protected void doAction(SQLExecutor executor) {
		...
		show_table_header(Nest.value(page,"title").$(), frmTitle);
		...
		if ("usrgrp".equals(srctbl)) {
			...
			table.setHeader(array(
				(multiselect>0 ? new CCheckBox("all_users", false, "javascript: checkAll(\""+form.getName()+"\", \"all_users\", \"users\");") : null),
				_("Alias"),
				_("Name"),
				_("Surname")
			));
			...
			CArray<Map> users = API.User(getIdentityBean(), executor).get(params);
			if(users!=null && !users.isEmpty()){
				for (Map user : users) {
					...
					if (isset(srcfld2) && "fullname".equals(srcfld2)) {
						Nest.value(user, srcfld2).$(getUserFullname(user));
					}
					
					String js_action = null;
					if (multiselect>0) {
						js_action  = "javascript: addValue("+rda_jsvalue(reference)+", "+rda_jsvalue(user.get("userid"))+");";
					} else {
						CArray values = map(
							dstfld1 , user.get(srcfld1)
						);
						if (isset(srcfld2)) {
							Nest.value(values, dstfld2).$(user.get(srcfld2));
						}
						js_action = "javascript: addValues("+rda_jsvalue(dstfrm)+", "+rda_jsvalue(values)+"); close_window(); return false;";
					}
					...
					table.addRow(array(
						multiselect>0 ? new CCheckBox("users["+rda_jsvalue(user.get(srcfld1))+"]", false, null, Nest.value(user,"userid").asString()) : null,
						alias,
			...
			if (multiselect>0) {
				CButton button = new CButton("select", _("Select"), "javascript: addSelectedValues(\"users\", "+rda_jsvalue(reference)+");");
				table.setFooter(new CCol(button, "right"));
			...
			if (templates != null && !templates.isEmpty()) {
				for(Map template:templates) {
					CSpan name = new CSpan(Nest.value(template,"name").$(), "link");
			...
			for (Map trigger : triggers) {
				...
				if (count(Nest.value(trigger,"dependencies").$()) > 0) {
					...
					for (Map dependentTrigger : dependentTriggers) {
						((CArray)description).add(array(CMacrosResolverHelper.resolveTriggerName(this.getIdentityBean(), executor, CArray.valueOf(dependentTrigger)), BR()));
					}
				}
			...
			if (multiselect>0) {
				CButton button = new CButton("select", _("Select"), "addSelectedValues(\"triggers\", "+rda_jsvalue(reference)+", "+parentId+");");
				table.setFooter(new CCol(button, "right"));
			...
			if (Nest.as(multiselect).asBoolean()) {
				CButton button = new CButton("select", _("Select"), "javascript: addSelectedValues(\"graphs\", "+rda_jsvalue(reference)+");");
				table.setFooter(new CCol(button, "right"));
			...
			CArray<Map> sysmaps = API.Map(getIdentityBean(), executor).get(moptions);
			...
			if (Nest.as(multiselect).asBoolean()) {
				CButton button = new CButton("select", _("Select"), "javascript: addSelectedValues(\"sysmaps\", "+rda_jsvalue(reference)+");");
				table.setFooter(new CCol(button, "right"));
			...
			for(Map dbSlideshow : dbSlideshows) {
				if (!slideshow_accessible(this.getIdentityBean(), executor, Nest.value(dbSlideshow,"slideshowid").asLong(), PERM_READ)) {
					continue;
				}
			...
			if (Nest.as(multiselect).asBoolean()) {
				CButton button = new CButton("select", _("Select"), "javascript: addSelectedValues(\"screens\", "+rda_jsvalue(reference)+");");
				table.setFooter(new CCol(button, "right"));
