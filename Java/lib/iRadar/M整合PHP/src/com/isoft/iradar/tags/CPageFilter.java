...
public class CPageFilter {
	...
	public final static String GROUP_LATEST_IDX = "web.latest.groupid";
	public final static String HOST_LATEST_IDX = "web.latest.hostid";
	public final static String GRAPH_LATEST_IDX = "web.latest.graphid";
	public final static String TRIGGER_LATEST_IDX = "web.latest.triggerid";
	public final static String DRULE_LATEST_IDX = "web.latest.druleid";
	...
	private void _getProfiles(IIdentityBean idBean, SQLExecutor executor, CArray options) {
		...
		Object profileSection = Nest.value(config,"individual").asBoolean() ? Nest.value(page,"file").$() : Nest.value(page,"menu").$();
		...
		if (Nest.value(config,"select_latest").asBoolean()) {
			Nest.value(_profileIds,"groupid").$(CProfile.get(idBean, executor, GROUP_LATEST_IDX));
			Nest.value(_profileIds,"hostid").$(CProfile.get(idBean, executor, HOST_LATEST_IDX));
			Nest.value(_profileIds,"graphid").$(CProfile.get(idBean, executor, GRAPH_LATEST_IDX));
			Nest.value(_profileIds,"triggerid").$(null);
			Nest.value(_profileIds,"druleid").$(CProfile.get(idBean, executor, DRULE_LATEST_IDX));
	...
	private void _initGroups(IIdentityBean idBean, SQLExecutor executor, Long groupid, CArray options, Long hostid) {
		...
		for(Map group: groups) {
			Nest.value(data,"groups", group.get("groupid")).$(group);
		}
		...
		CProfile.update(idBean, executor, GROUP_LATEST_IDX, groupid, PROFILE_TYPE_ID);
	...
	private void _initHosts(IIdentityBean idBean, SQLExecutor executor, Long hostId, CArray options) {
		...
		if (!is_null(Nest.value(_requestIds,"hostid").$())) {
			...
			CProfile.update(idBean, executor, HOST_LATEST_IDX, hostId, PROFILE_TYPE_ID);
		}
	...
	private void _initGraphs(IIdentityBean idBean, SQLExecutor executor, Long graphid, CArray options) {
		...
		if (!is_null(Nest.value(_requestIds,"graphid").$())) {
			...
			CProfile.update(idBean, executor, GRAPH_LATEST_IDX, graphid, PROFILE_TYPE_ID);
		}
	...
	private void _initDiscoveries(IIdentityBean idBean, SQLExecutor executor, Long druleid, CArray options) {
		...
		CProfile.update(idBean, executor, DRULE_LATEST_IDX, druleid, PROFILE_TYPE_ID);
	...
	public CComboBox getGraphsCB() {
		...
		natcasesort(graphs);
		graphs = array_add(map(0, _("not selected")), graphs);

		CComboBox graphComboBox = new CComboBox("graphid", this.$("graphid").asString(), "javascript: submit();");
		for(Entry<Object, Object> entry: graphs.entrySet()) {
			Object id = entry.getKey();
			String name = Nest.as(entry.getValue()).asString();
