...
import static com.isoft.iradar.core.g.RDA_SERVER_RUNNING;
import static com.isoft.iradar.core.g.RDA_SERVER_RUNNING_TS;
...
public class JsrpcAction extends RadarBaseAction {
	...
	protected boolean doAjax(SQLExecutor executor) {
		...
		if("host.get".equals(method)) {
			...
			result = map(
				"result", RDA_SERVER_RUNNING,
				"message", RDA_SERVER_RUNNING ? "" : _s("iRadar server is not running: the information displayed may not be current.", rda_date2str(_("d M Y H:i:s"), RDA_SERVER_RUNNING_TS))
			);
