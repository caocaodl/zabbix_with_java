...
public class CMonitoringScreen extends CViewSegment {
	...
	public CWidget doWidget(IIdentityBean idBean, SQLExecutor executor, Map data) {
		...
		// header form
		CComboBox configComboBox = new CComboBox("config", "screens.action", "javascript: redirect(this.options[this.selectedIndex].value);");
		...
		if (empty(Nest.value(data,"screens").$())) {
			...
			screenWidget.addPageHeader(_("SCREENS"), array(
				headerForm,
				SPACE,
				get_icon(idBean, executor, "favourite", map("fav", "web.favorite.screenids", "elname", "screenid", "elid", Nest.value(screen,"screenid").$())),
				SPACE,
			...
			for(Map dbScreen : (CArray<Map>)Nest.value(data,"screens").asCArray()) {
				elementsComboBox.addItem(Nest.value(dbScreen,"screenid").$(),
					htmlspecialchars(Nest.value(dbScreen,"name").asString()));
			}
			...
			screenWidget.addHeader(Nest.value(screen,"name").$(), headerForm);
