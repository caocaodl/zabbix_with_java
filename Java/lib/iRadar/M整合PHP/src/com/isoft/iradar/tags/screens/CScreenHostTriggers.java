...
public class CScreenHostTriggers extends CScreenBase {

	public CScreenHostTriggers(IIdentityBean idBean, SQLExecutor executor) {
		super(idBean, executor);
	}

	public CScreenHostTriggers(IIdentityBean idBean, SQLExecutor executor, Map options) {
	...
	public CDiv get() {
		...
		if (Nest.value(screenitem,"resourceid").asLong() > 0) {
			...
			hoptions.setHostIds(Nest.value(screenitem,"resourceid").asLong());
			hoptions.setOutput(API_OUTPUT_EXTEND);
			CArray<Map> hosts = API.Host(this.idBean, executor).get(hoptions);
			Map host = reset(hosts);

			item = new CSpan(_("Host")+NAME_DELIMITER+Nest.value(host,"host").$(), "white");
			Nest.value(params,"hostids").$(Nest.value(host,"hostid").$());
		} else {
			...
			CComboBox hostsComboBox = new CComboBox("tr_hostid", hostid, "submit()");
			hostsComboBox.addItem(0, _("all"));
			for(Map host : hosts) {
				hostsComboBox.addItem(Nest.value(host,"hostid").$(), Nest.value(host,"host").asString());
			}
		...
		CUIWidget output = new CUIWidget("hat_trstatus", make_latest_issues(idBean, executor, params));
		output.setDoubleHeader(array(_("HOST ISSUES"), SPACE, rda_date2str(_("[H:i:s]")), SPACE), item);
