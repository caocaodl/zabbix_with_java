...
public class TriggersAction extends RadarBaseAction {
	...
	public void doAction(final SQLExecutor executor) {
		...
		if ("massupdate".equals(Nest.value(_REQUEST,"go").asString()) && isset(_REQUEST,"g_triggerid")) {
			...
			CView triggersView = new CView("configuration.copy.elements", getCopyElementsFormData(this.getIdentityBean(), executor, "g_triggerid", _("CONFIGURATION OF TRIGGERS")));
			triggersView.render(getIdentityBean(), executor);
			triggersView.show();
		} else {
			CArray data = map(
			...
			CPageFilter pageFilter = new CPageFilter(getIdentityBean(), executor, map(
				"groups", map("not_proxy_hosts", true, "editable", true),
			...
			if ("status".equals(sortfield)) {
				orderTriggersByStatus(triggers, getPageSortOrder(getIdentityBean(), executor));
			} else {
