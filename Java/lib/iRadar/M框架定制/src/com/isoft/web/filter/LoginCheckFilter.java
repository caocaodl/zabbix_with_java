...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (Feature.enableLoginCheck) {
			doLoginCheck(request, response, filter);
		} else {
			filter.doFilter(request, response);
		}
	}
	
	public void doLoginCheck(ServletRequest request, ServletResponse response,
			FilterChain filter) throws IOException, ServletException {
		...
		if(!pass){
			if ("/index.action".equals(viewId) 
			 || "/checkcode.action".equals(viewId)
			 || "/login.action".equals(viewId)
			 || "/logout.action".equals(viewId) 
			 || "/platform/ProfForgotPwd.action".equals(viewId)
...
//			}
			if (hasPerm) {
				...
				if (httpRequest.getHeader("x-requested-with") != null
					...
					httpRequest.getRequestDispatcher("/reindex.html").forward(request, response);
				}
			}
		} else {
			if (httpRequest.getHeader("x-requested-with") != null
