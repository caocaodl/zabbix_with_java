package com.isoft.iaas.openstack.connector;
...
public class JaxRs20Connector implements OpenStackClientConnector {
	...
	private LoggingFilter logger = new LoggingFilter(Logger.getLogger("os"),
			10000);

	@Override
	public <T> OpenStackResponse request(OpenStackRequest<T> request) {
		WebTarget target = client.target(request.endpoint()).path(
				request.path());
		...
		try {
			if (entity != null) {
				return new JaxRs20Response(invocation.method(request.method()
						.name(), entity));
			} else {
				if (HttpMethod.PUT == request.method()) {
					return new JaxRs20Response(invocation.method(request
