package com.isoft.iaas.openstack.console.utils;
			...
			for (String column : row) {
				...
				if (column != null) {
					if (Column.ALIGN_RIGHT == columnModel.getAlign()) {
						for (int j = 0; j < columnModel.getSize()
								- column.length(); j++) {
							sb.append(" ");
						}
					}
					sb.append(column.length() <= columnModel.getSize() ? column
							: column.substring(0, columnModel.getSize()));
